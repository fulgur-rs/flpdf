...
fn qpdf_full_rewrite_emits_touched_object_and_keeps_pages() {
    ...
    let original_root = pdf.resolve_canonical_object(root).unwrap();
...
fn qpdf_full_rewrite_object_stream_member_rewrite_stays_qpdf_compatible() {
    ...
    for candidate in candidates.drain(..) {
        let original = pdf.resolve_canonical_object(candidate).unwrap();
    ...
    let touched_value = rewritten.resolve_canonical_object(touched).unwrap();
    assert_ne!(touched_value, Object::Null);

    let mut original = Pdf::open(BufReader::new(File::open(&source).unwrap())).unwrap();
    let original_value = original.resolve_canonical_object(touched).unwrap();
