...
fn coalesce_contents_merges_array_to_single_stream() {
    ...
    let out_page_obj = out_pdf.resolve_canonical_object(out_page_ref).unwrap();
...
fn extract_page_resource_keys(path: &Path, category: &str) -> Vec<Vec<u8>> {
    ...
    let resources_obj = match page_dict.get("Resources").cloned() {
        Some(Object::Reference(r)) => pdf.resolve_canonical_object(r).unwrap(),
        Some(obj) => obj,
        None => return vec![],
    };
...
fn compress_streams_y_applies_flatedecode_and_roundtrips() {
    ...
    for page_ref in &pages {
        let page_obj = out_pdf.resolve_canonical_object(*page_ref).unwrap();
        let page_dict = match &page_obj {
            Object::Dictionary(d) => d.clone(),
            other => panic!("page must be a Dictionary, got {other:?}"),
        };
        ...
        let content_stream = match out_pdf.resolve_canonical_object(contents_ref).unwrap() {
            Object::Stream(s) => s,
...
fn combination_normalize_coalesce_compress_succeeds() {
    ...
    let page_obj = out_pdf.resolve_canonical_object(pages[0]).unwrap();
    let page_dict = match page_obj {
        Object::Dictionary(d) => d,
        other => panic!("page must be a Dictionary, got {other:?}"),
    };
