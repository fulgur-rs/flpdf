...
/// Lift a test-only materialized dictionary into the canonical filter input
/// shape. Production callers already own an ObjectHandle; this helper keeps
/// fixture construction out of the public compatibility boundary.
...
fn value_handle(value: &Object) -> ObjectHandle {
    match value {
        ...
        // Filter fixtures do not resolve references. Keeping an unsupported
        // scalar shape preserves the old reader's rejection/empty-parameter
        // behavior without requiring a document context.
