...
fn image_length(out: &[u8]) -> Object {
    ...
    let pages = pdf.resolve_canonical_object(pages_ref).expect("pages");
    ...
    let page = pdf.resolve_canonical_object(page_ref).expect("page");
    let resources = page
        .as_dict()
        .and_then(|d| d.get("Resources").cloned())
        .expect("/Resources");
    let resources = match resources {
        Object::Reference(r) => pdf.resolve_canonical_object(r).expect("resources"),
        other => other,
    };
    ...
    let xobject = match xobject {
        Object::Reference(r) => pdf.resolve_canonical_object(r).expect("xobject dict"),
        other => other,
    };
    ...
    let image = pdf.resolve_canonical_object(image_ref).expect("image");
    image
        .as_stream()
...
fn keep_holder_is_live_integer(out: &[u8]) -> bool {
    ...
    matches!(
        pdf.resolve_canonical_object(holder_ref),
        Ok(Object::Integer(_))
    )
}
