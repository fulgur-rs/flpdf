...
fn default_id_random_first_save_then_preserves_element1_on_resave() {
    ...
    write_default(&mut pdf1, &mut out1).unwrap();
    ...
    write_default(&mut pdf2, &mut out2).unwrap();
...
fn default_id_random_xref_stream_full_rewrite_resave_preserves_element1() {
    ...
    let mut out1 = Vec::new();
    write_with_settings(&mut pdf1, &mut out1, &settings).unwrap();
    ...
    let mut pdf2 = Pdf::open(Cursor::new(out1.clone())).unwrap();
    let mut out2 = Vec::new();
    write_with_settings(&mut pdf2, &mut out2, &settings).unwrap();
...
fn pdf_writer_rewrites_flate_object_stream_member_and_recomputes_first() {
    ...
    .unwrap();
    let mapped_member_2 = mapping[&ObjectRef::new(2, 0)];
    ...
    let Object::Stream(rewritten_obj_stream) = rewritten.resolve(mapped_obj_stream).unwrap() else {
    ...
    assert_eq!(members[0].0, mapped_member_2.number);
...
fn pdf_writer_rewrites_members_across_two_object_streams() {
    ...
    .unwrap();
    let first_stream_ref = mapping[&ObjectRef::new(4, 0)];
    let second_stream_ref = mapping[&ObjectRef::new(7, 0)];

    let loaded = load_xref_and_trailer(&mut Cursor::new(&output)).unwrap();
    assert!(matches!(
        loaded.entries.get(&first_stream_ref),
        Some(flpdf::XrefEntry::Uncompressed { .. })
    ));
    assert!(matches!(
        loaded.entries.get(&second_stream_ref),
        Some(flpdf::XrefEntry::Uncompressed { .. })
    ));

    let mut rewritten = Pdf::open(Cursor::new(output)).unwrap();
    let (decoded_first_stream, first_stream_first) =
        decoded_objstm(&mut rewritten, first_stream_ref.number);
    ...
    let (decoded_second_stream, second_stream_first) =
        decoded_objstm(&mut rewritten, second_stream_ref.number);
...
fn pdf_writer_rewrites_two_members_in_one_object_stream() {
    ...
    .unwrap();
    let mapped_member_2 = mapping[&ObjectRef::new(2, 0)];
    let mapped_member_4 = mapping[&ObjectRef::new(4, 0)];
    let mapped_obj_stream = mapping[&ObjectRef::new(5, 0)];

    let mut rewritten = Pdf::open(Cursor::new(output)).unwrap();
    assert_eq!(
        rewritten.resolve(mapped_member_2).unwrap(),
        member_2_rewritten
    );
    assert_eq!(
        rewritten.resolve(mapped_member_4).unwrap(),
        member_4_rewritten
    );
...
fn pdf_writer_rewrites_member_declared_in_extended_object_stream() {
    ...
    .unwrap();
    let mapped_base_stream = mapping[&ObjectRef::new(4, 0)];
    let mapped_extension_stream = mapping[&ObjectRef::new(5, 0)];
    let mapped_member_2 = mapping[&ObjectRef::new(2, 0)];
    let mapped_member_3 = mapping[&ObjectRef::new(3, 0)];
    ...
    assert!(output_text.contains(&format!(
        "/Extends {} {} R",
        mapped_base_stream.number, mapped_base_stream.generation
    )));

    let mut rewritten = Pdf::open(Cursor::new(output)).unwrap();
    let (decoded, first) = decoded_objstm(&mut rewritten, mapped_extension_stream.number);
    let members = parse_objstm_members(&decoded, first);
    assert_eq!(members[0].0, mapped_member_2.number);
    assert_eq!(members[1].0, mapped_member_3.number);
    assert_eq!(
    ...
    assert!(matches!(
        rewritten.resolve(mapped_extension_stream).unwrap(),
        Object::Stream(_)
    ));
}
...
fn pdf_writer_preserves_extends_when_rewriting_extension_stream_member() {
    ...
    let mut rewritten = Pdf::open(Cursor::new(output)).unwrap();
    let Object::Stream(extension_stream) = rewritten.resolve(mapped_extension_stream).unwrap()
    else {
        panic!("expected rewritten extension object stream");
    };
    assert_eq!(
        extension_stream.dict.get_ref("Extends"),
        Some(mapped_base_stream)
    );
    let (decoded, first) = decoded_objstm(&mut rewritten, mapped_extension_stream.number);
    let members = parse_objstm_members(&decoded, first);
    assert_eq!(members[0].0, mapped_member_2.number);
...
fn pdf_writer_rewrites_null_object_revision() {
    ...
    assert_eq!(count_substrings(&output, b"2 0 obj"), 0);
}
...
#[test]
fn rewrites_linearized_input_to_fresh_valid_pdf() {
...
fn write_qdf_header_has_binary_marker() {
    ...
    let mut pdf = Pdf::open(Cursor::new(bytes)).unwrap();
    let mut output = Vec::new();
    write_qdf_output(&mut pdf, &mut output).unwrap();
...
fn qdf_goes_through_canonical_pdf_writer_serializers() {
    ...
    assert!(
        !settings.no_original_object_ids,
    ...
    assert!(
        ...
        "canonical QDF writer must emit the %QDF-1.0 header marker"
    );
    assert!(
        rendered.contains("%% Original object ID:"),
        "canonical QDF writer must emit %% Original object ID: comments"
    );
    assert!(
        rendered.contains("\ntrailer <<"),
        "canonical QDF writer must use the `trailer <<` dict layout"
    );
    assert!(
        rendered.contains("\nxref\n"),
        "canonical QDF writer must use a classic xref table"
    );
    assert!(
        !rendered.contains("/Type /XRef"),
        "canonical QDF writer must not emit a cross-reference stream"
    );
    assert!(
        !rendered.contains("/Type /ObjStm"),
        "canonical QDF writer must not emit object streams"
    );
}
...
fn write_qdf_indirect_contents_array_is_byte_identical_to_qpdf() {
    ...
    write_with_settings(&mut pdf, &mut actual, &settings).unwrap();
...
/// qpdf 11.9.0 byte gate for a stream whose logical payload does not end in
/// LF. qpdf adds one framing LF, emits `%QDF: ignore_newline` after the stream
/// object, and keeps the indirect `/Length` holder at the raw payload length.
#[test]
fn write_qdf_non_eol_stream_is_byte_identical_to_qpdf() {
    let file = File::open("../../tests/fixtures/compat/qdf-ignore-newline.pdf").unwrap();
    let mut pdf = Pdf::open(BufReader::new(file)).unwrap();
    let settings = WriterTestSettings {
        qdf: true,
        static_id: true,
    ...
    let expected =
        fs::read("../../tests/golden/references/qdf-ignore-newline/qdf-static-id.pdf").unwrap();
    if actual != expected {
        let common = actual.len().min(expected.len());
        let first_diff = (0..common)
            .find(|&idx| actual[idx] != expected[idx])
            .unwrap_or(common);
        panic!(
            "non-EOL QDF output must be byte-identical to qpdf 11.9.0: \
             first diff at byte {first_diff}, flpdf length {}, qpdf length {}",
            actual.len(),
            expected.len()
        );
...
/// verify that the qpdf writer does not propagate stale filter
