...
fn plain_thumbnail_before_container_thumbnail_pdf() -> Vec<u8> {
    // Mirror of `source_objstm_thumbnail_before_plain_rest_pdf`: here the *plain*
    // object is itself a thumbnail and carries the lower source number, while the
    // container holding the other page's thumbnail is numbered higher. qpdf keeps
    // both in the thumbnail category and orders them by page then object number
    // (`QPDF_linearization.cc:1279-1340`), so the plain stream must stay first.
    let mut pdf = Vec::new();
    pdf.extend_from_slice(b"%PDF-1.5\n");
    let mut offsets = [0u64; 12];
    ...
    append_object(
        ...
        &mut offsets,
        3,
        b"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 10 10] /Contents 6 0 R /Thumb 5 0 R >>",
    );
    append_object(
        &mut pdf,
        &mut offsets,
    ...
    append_object(
        ...
        &mut offsets,
        5,
        b"<< /Type /XObject /Subtype /Image /Width 1 /Height 1 /Length 0 >>\nstream\n\nendstream",
    );
    append_object(
        &mut pdf,
        &mut offsets,
...
#[test]
fn preserve_part9_plain_thumbnail_precedes_a_higher_numbered_container() {
    let directory = tempfile::tempdir().expect("plain thumbnail order tempdir");
    let input = directory.path().join("input.pdf");
    std::fs::write(&input, plain_thumbnail_before_container_thumbnail_pdf())
        .expect("write plain thumbnail order input");

    let expected = qpdf_linearized_objstm_preserve_at(&input);
    let actual = flpdf_linearized_objstm_preserve_at(&input);
    report(
        "plain thumbnail before a higher-numbered thumbnail container",
        &mask_id1(&actual),
        &mask_id1(&expected),
        "Preserve plain thumbnail order (ignoring /ID[1])",
    );
    report(
        "plain thumbnail before a higher-numbered thumbnail container",
        &actual,
        &expected,
        "Preserve plain thumbnail order",
    );
}
