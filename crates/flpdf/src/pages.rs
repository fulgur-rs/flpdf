...
mod tests {
    ...
    #[test]
    fn no_materialized_inherited_resources_projection_remains() {
        let source = include_str!("pages.rs");
        let legacy = |suffix: &str| ["pub fn resolve_inherited_", suffix].concat();
        assert!(!source.contains(&legacy("resources<")));
        assert!(!source.contains(&legacy("resources_with_max_depth<")));
    }
