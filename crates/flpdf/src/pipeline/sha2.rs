//! Stream SHA-256, SHA-384, or SHA-512 digests through a reusable pipeline
//! stage. Calling `finish()` resets the selected digest, so the next `write()`
//! starts a fresh cycle with the same bit size.
//!
