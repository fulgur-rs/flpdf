...
impl<R: Read + Seek> Pdf<R> {
    ...
    ///
    /// The qpdf method returns `void`; this Rust port returns `Result<()>` only
    /// to propagate failures from live-handle resolution and mutation.
    ///
    /// # Errors
    ///
    ...
    pub fn remove_security_restrictions(&mut self) -> Result<()> {
        ...
        if acroform.try_is_dictionary()? && acroform.try_has_key(b"/SigFlags")? {
            ...
            // succeeds, including an already-zero direct integer.
