//! Discover resource names through the live `ObjectHandle` content callback
//! route before the exact-byte token filter rewrites source names. A
//! document-owned scan preserves structural errors; only the detached
