...
if [[ ! -f "$FIX/attachment-two-page.pdf" ]]; then
    ...
    # Use a fixed path + deterministic key/filename/dates so the embedded
    # attachment metadata is byte-identical regardless of where this script
    # runs (mktemp would inject a random basename into the PDF).
    TMPFILE="/tmp/flpdf-fixture-attachment.txt"
