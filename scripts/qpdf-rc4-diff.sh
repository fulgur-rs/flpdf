...
check_source_state() {
  ...
  if ! actual_commit="$(git -C "${qpdf_source}" rev-parse --verify HEAD)"; then
    echo "qpdf-rc4-diff.sh: unable to verify pinned source HEAD" >&2
    return 1
  fi
  ...
  if ! source_status="$(
    ...
    echo "qpdf-rc4-diff.sh: unable to verify pinned source cleanliness" >&2
    return 1
  fi
  if [[ -n "${source_status}" ]]; then
    echo "qpdf-rc4-diff.sh: ${dirty_message}" >&2
    return 1
  fi
}

unsafe_temp_artifact() {
  echo "qpdf-rc4-diff.sh: unsafe temp artifact: $*" >&2
  return 1
}
...
select_temp_base() {
  ...
  for root in "${roots[@]}"; do
    if ! physical_root="$(safe_temp_base_path "${root}")"; then
      if [[ "${root}" == "${requested_root}" ]]; then
        echo \
          "qpdf-rc4-diff.sh: unsafe or unusable TMPDIR; using an external fallback" \
          >&2
      fi
      continue
    fi
    if path_is_within "${physical_root}" "${repo_root}" ||
      ...
      if [[ "${root}" == "${requested_root}" ]]; then
        echo \
          "qpdf-rc4-diff.sh: unsafe TMPDIR build path; using an external fallback" \
          >&2
      fi
      continue
    fi
  ...
  echo \
    "qpdf-rc4-diff.sh: no usable temp base outside the repository and pinned source" \
    >&2
  return 1
}
...
verify_build_directory_path() {
  ...
  if [[ -z "${build_dir}" ||
    ...
    "$(basename -- "${build_dir}")" != flpdf-qpdf-rc4.* ]]; then
  ...
  if [[ -n "${build_dir}" ]]; then
    if [[ -n "${build_dir_inode}" ]] &&
      ...
      if ! rm -rf -- "${build_dir}"; then
        echo "qpdf-rc4-diff.sh: unable to remove validated build directory" >&2
        status=1
      fi
    else
      echo \
        "qpdf-rc4-diff.sh: refusing to clean an unvalidated lexical build path" \
        >&2
      status=1
    fi
  fi
...
for required_command in git mktemp realpath stat; do
  if ! command -v "${required_command}" >/dev/null; then
    echo "qpdf-rc4-diff.sh: ${required_command} is required" >&2
    exit 1
  fi
done
if ! read -r root_anchor_owner root_anchor_device < <(
  ...
  echo "qpdf-rc4-diff.sh: unable to identify the filesystem root anchor" >&2
  exit 1
fi
trap cleanup EXIT
...
if ! check_source_state ||
  ...
  echo "qpdf-rc4-diff.sh: compiled probe is not a trusted build artifact" >&2
  exit 1
fi
...
assert_rejected_argument() {
  ...
  if output="$("${probe}" "$@" 2>&1)"; then
    echo "qpdf-rc4-diff.sh: invalid probe input was accepted: $*" >&2
    exit 1
  fi
  if [[ "${output}" != "qpdf_rc4_probe: ${expected}" ]]; then
    echo "qpdf-rc4-diff.sh: invalid probe input reached qpdf: ${output}" >&2
    exit 1
  fi
}

assert_rejected_argument "empty explicit key" explicit "" "" 0
assert_rejected_argument "empty C-string key" cstr "" "" 0
