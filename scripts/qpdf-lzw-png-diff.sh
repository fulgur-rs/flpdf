...
check_source_state() {
  ...
  if ! actual_commit="$(git -C "${qpdf_source}" rev-parse --verify HEAD)"; then
    echo "qpdf-lzw-png-diff.sh: unable to verify pinned source HEAD" >&2
    return 1
  fi
  if [[ "${actual_commit}" != "${qpdf_commit}" ]]; then
    echo "qpdf-lzw-png-diff.sh: pinned source is not at ${qpdf_commit}" >&2
    return 1
  fi
  if ! source_status="$(
    git -C "${qpdf_source}" status --porcelain --untracked-files=all --ignored
  )"; then
    echo "qpdf-lzw-png-diff.sh: unable to verify pinned source cleanliness" >&2
    return 1
  fi
  if [[ -n "${source_status}" ]]; then
    echo "qpdf-lzw-png-diff.sh: ${dirty_message}" >&2
    return 1
  fi
}

unsafe_temp_artifact() {
  echo "qpdf-lzw-png-diff.sh: unsafe temp artifact: $*" >&2
  return 1
}
...
select_temp_base() {
  ...
  for root in "${roots[@]}"; do
    if ! physical_root="$(safe_temp_base_path "${root}")"; then
      if [[ "${root}" == "${requested_root}" ]]; then
        echo "qpdf-lzw-png-diff.sh: unsafe or unusable TMPDIR; using an external fallback" >&2
      fi
      continue
    fi
    if path_is_within "${physical_root}" "${repo_root}" || path_is_within "${physical_root}" "${qpdf_source}"; then
      if [[ "${root}" == "${requested_root}" ]]; then
        echo "qpdf-lzw-png-diff.sh: unsafe TMPDIR build path; using an external fallback" >&2
      fi
      continue
    fi
    printf '%s\n' "${physical_root}"
  ...
  echo "qpdf-lzw-png-diff.sh: no usable temp base outside the repository and pinned source" >&2
  return 1
}
...
verify_build_directory_path() {
  ...
  if [[ -z "${build_dir}" || "$(dirname -- "${build_dir}")" != "${temp_base}" ||
    "$(basename -- "${build_dir}")" != flpdf-qpdf-lzw-png.* ]]; then
...
cleanup() {
  ...
  if [[ -n "${build_dir}" ]]; then
    if [[ -n "${build_dir_inode}" ]] && verify_build_directory_identity; then
      ...
      echo "qpdf-lzw-png-diff.sh: refusing to clean an unvalidated lexical build path" >&2
      status=1
    fi
  fi
...
for required_command in git mktemp realpath stat c++ cargo; do
  command -v "${required_command}" >/dev/null || {
    echo "qpdf-lzw-png-diff.sh: ${required_command} is required" >&2; exit 1;
  }
done
if ! read -r root_anchor_owner root_anchor_device < <(stat -c '%u %d' -- /); then
  echo "qpdf-lzw-png-diff.sh: unable to identify filesystem root anchor" >&2
  exit 1
fi
trap cleanup EXIT

temp_base="$(select_temp_base)"
build_dir="$(mktemp -d "${temp_base}/flpdf-qpdf-lzw-png.XXXXXXXX")"
verify_build_directory_path
open_build_directory
verify_build_directory_identity
check_source_state "pinned source has tracked-file changes"

probe="${build_dir_fd_path}/qpdf_lzw_png_probe"
c++ -std=c++17 -DQPDF_DISABLE_QTC \
  "-I${qpdf_source}/include" \
  "-I${qpdf_source}/libqpdf" \
  "${repo_root}/tests/oracle/qpdf_lzw_png_probe.cc" \
  "${qpdf_source}/libqpdf/Pipeline.cc" \
  "${qpdf_source}/libqpdf/Pl_LZWDecoder.cc" \
  "${qpdf_source}/libqpdf/Pl_PNGFilter.cc" \
  "${qpdf_source}/libqpdf/Buffer.cc" \
  -o "${probe}"

if ! check_source_state || ! verify_build_directory_identity ||
  [[ -L "${probe}" || ! -f "${probe}" || ! -x "${probe}" ]] ||
  [[ "$(stat -c '%u' -- "${probe}")" != "${UID}" ]]; then
  echo "qpdf-lzw-png-diff.sh: compiled probe is not a trusted build artifact" >&2
  exit 1
fi

cd "${repo_root}"
QPDF_LZW_PNG_PROBE="${probe}" \
  cargo test -p flpdf --lib \
  pipeline::lzw_png_oracle::qpdf_lzw_png_differential \
  -- --ignored --exact
