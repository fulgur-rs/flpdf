...
if [[ "$#" != 1 ]]; then
  echo "usage: qpdf-objecthandle-dereference-diff.sh INPUT.pdf" >&2
  exit 2
fi
...
if [[ "$(git -C "${qpdf_source}" rev-parse HEAD)" != "${qpdf_commit}" ]]; then
  echo "qpdf-objecthandle-dereference-diff.sh: pinned source commit mismatch" >&2
  exit 1
fi
...
check_source_clean() {
  ...
  if [[ -n "${source_status}" ]]; then
    echo "qpdf-objecthandle-dereference-diff.sh: pinned source has tracked-file changes" >&2
    return 1
  fi
}

check_source_clean
build_dir="$(mktemp -d -t flpdf-qpdf-objecthandle-XXXXXXXX)"
cleanup() {
  rm -rf -- "${build_dir}"
}
trap cleanup EXIT
...
probe_binary="${build_dir}/qpdf-objecthandle-dereference-probe"
c++ -std=c++17 \
  -DPOINTERHOLDER_TRANSITION=4 \
  -I"${qpdf_source}/include" \
  -I"${qpdf_source}/libqpdf" \
  "${repo_root}/tests/oracle/qpdf_objecthandle_dereference_probe.cc" \
  -L"${build_dir}/libqpdf" \
...
case "${resolved_libqpdf}" in
  "${probe_lib_dir}"/*) ;;
  *)
    echo "qpdf-objecthandle-dereference-diff.sh: probe resolved an unpinned libqpdf" >&2
    exit 1
    ;;
esac

LD_LIBRARY_PATH="${probe_library_path}" "${probe_binary}" "${input_pdf}"
check_source_clean
