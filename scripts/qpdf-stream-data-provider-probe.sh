...
if [[ "$#" != 0 ]]; then
  echo "usage: qpdf-stream-data-provider-probe.sh" >&2
  exit 2
fi
...
if [[ "${actual_commit}" != "${qpdf_commit}" ]]; then
  echo "qpdf-stream-data-provider-probe.sh: pinned source commit mismatch" >&2
  exit 1
fi
if [[ -n "$(git -C "${qpdf_source}" status --porcelain --untracked-files=no)" ]]; then
  echo "qpdf-stream-data-provider-probe.sh: pinned source has tracked-file changes" >&2
  exit 1
fi

build_dir="$(TMPDIR=/tmp mktemp -d -t flpdf-qpdf-stream-data-provider-XXXXXXXX)"
build_dir="$(realpath -e -- "${build_dir}")"
case "${build_dir}" in
  /tmp/flpdf-qpdf-stream-data-provider-*) ;;
  *)
    echo "qpdf-stream-data-provider-probe.sh: unsafe build directory" >&2
    exit 1
    ;;
esac
cleanup() {
  case "${build_dir:-}" in
    /tmp/flpdf-qpdf-stream-data-provider-*) rm -rf -- "${build_dir}" ;;
  esac
}
trap cleanup EXIT
...
probe_binary="${build_dir}/qpdf-stream-data-provider-probe"
c++ -std=c++17 \
  -DPOINTERHOLDER_TRANSITION=4 \
  -I"${qpdf_source}/include" \
  -I"${qpdf_source}/libqpdf" \
  "${repo_root}/tests/oracle/qpdf_stream_data_provider_probe.cc" \
  -L"${build_dir}/libqpdf" \
...
case "${resolved_libqpdf}" in
  "${probe_lib_dir}"/*) ;;
  *)
    echo "qpdf-stream-data-provider-probe.sh: probe resolved an unpinned libqpdf" >&2
    exit 1
    ;;
esac
...
if [[ "${actual_commit}" != "${qpdf_commit}" || -n "$(git -C "${qpdf_source}" status --porcelain --untracked-files=no)" ]]; then
  echo "qpdf-stream-data-provider-probe.sh: pinned source changed during probe" >&2
  exit 1
fi
