...
if [[ "$#" != 0 ]]; then
  echo "usage: qpdf-stream-filter-registry-probe.sh" >&2
  exit 2
fi
...
check_source_pin() {
  ...
  if [[ "${actual_commit}" != "${qpdf_commit}" ]]; then
    echo "qpdf-stream-filter-registry-probe.sh: pinned source commit mismatch" >&2
    return 1
  fi
}
...
check_source_clean() {
  ...
  if [[ -n "${source_status}" ]]; then
    echo "qpdf-stream-filter-registry-probe.sh: pinned source has tracked-file changes" >&2
    return 1
  fi
}
...
build_dir="$(mktemp -d -t flpdf-qpdf-stream-filter-registry-XXXXXXXX)"
build_dir="$(realpath -e -- "${build_dir}")"
case "${build_dir}" in
  /tmp/flpdf-qpdf-stream-filter-registry-*) ;;
  *)
    echo "qpdf-stream-filter-registry-probe.sh: unsafe build directory" >&2
    exit 1
    ;;
esac
cleanup() {
  case "${build_dir:-}" in
    /tmp/flpdf-qpdf-stream-filter-registry-*) rm -rf -- "${build_dir}" ;;
  esac
}
trap cleanup EXIT
...
probe_binary="${build_dir}/qpdf-stream-filter-registry-probe"
c++ -std=c++17 \
  -DPOINTERHOLDER_TRANSITION=4 \
  -I"${qpdf_source}/include" \
  -I"${qpdf_source}/libqpdf" \
  "${repo_root}/tests/oracle/qpdf_stream_filter_registry_probe.cc" \
  -L"${build_dir}/libqpdf" \
...
case "${resolved_libqpdf}" in
  "${probe_lib_dir}"/*) ;;
  *)
    echo "qpdf-stream-filter-registry-probe.sh: probe resolved an unpinned libqpdf" >&2
    exit 1
    ;;
esac
