...
- *(job)* expose image config setters for json layering
- *(xref)* add load_xref_state_through_canonical_owner primitive

### Fixed

- *(xref)* keep undelivered warnings pending when the sink fails
- *(xref)* unify insertReconstructedXrefEntry guard/insert/suppress (B20)
- *(pages)* promote direct kids in forward order like qpdf
- *(pages)* keep non-dictionary /Kids leaves in the page walk like qpdf
- *(xref)* apply the reconstruction free-row filter on every recovery handoff
- *(cli)* layer image options after job JSON
- *(writer)* capture PCLm Generate ObjStm membership during setup
...
- Keep B25 mixed: the guard is armed after the scan, not before it
- write-through reconstructed_xref via CanonicalTrailerOwner (flpdf-3yn9.48.176, B25)
- Merge pull request #2201 from fulgur-rs/mixed/flpdf-3yn9-48-206
- Merge pull request #2200 from fulgur-rs/mixed/flpdf-3yn9-48-205
- Merge pull request #2199 from fulgur-rs/mixed/flpdf-3yn9-48-204
- keep the audit out of published rustdoc and fix the shifted citations
- Merge remote-tracking branch 'origin/main' into HEAD
- correct the B27 audit to what the code actually does
- *(xref)* drop internal issue-ID reference from published module doc
- *(xref)* record B27 repair_diagnostics buffer as (B), not (C)
- Merge remote-tracking branch 'origin/main' into HEAD
- allowlist the wrapper's calls instead of banning accessors
- count conversions, and track helper and parameter provenance
- cover async blocks, destructuring shadows, and the constructor's inputs
- fix the shadowing false positive and four traversal gaps
- gate attributed expressions at any depth, and fix the cited test name
- judge only the wrapper the contract is about
- Merge remote-tracking branch 'origin/main' into HEAD
- match forbidden calls, and scope helper bindings per function
- check the delegated call's shape and normalize raw identifiers
- Merge remote-tracking branch 'origin/main' into HEAD
- close four more gaps in the route contract's traversal
- Merge remote-tracking branch 'origin/main' into HEAD
- Merge pull request #2183 from fulgur-rs/mixed/flpdf-3yn9-48-189
- Merge pull request #2180 from fulgur-rs/mixed/flpdf-3yn9-48-187
- Merge pull request #2186 from fulgur-rs/mixed/flpdf-3yn9-48-193
- describe the linearized normalize set as the raw set it is
- Merge remote-tracking branch 'origin/main' into HEAD
- Merge pull request #2181 from fulgur-rs/mixed/flpdf-3yn9-48-183
- Merge pull request #2177 from fulgur-rs/mixed/flpdf-3yn9-48-188
- Merge pull request #2182 from fulgur-rs/mixed/flpdf-3yn9-48-182
- Merge main into the branch for #2182
- mark the last uncovered line in the moved qpdf-key oracle test
- fix cov:ignore marker false-positive in explanatory prose
- mark genuinely uncovered lines in the moved JSON tests
- narrow QPDFJob::write_json(_with_version) to pub(crate)
- Merge pull request #2171 from fulgur-rs/mixed/flpdf-3yn9-48-168
- Merge pull request #2170 from fulgur-rs/mixed/flpdf-3yn9-48-167
- Merge main into the branch for #2170
- unify plain writer ObjStm exclusion into filter_objstm_batches_for_output (D23)
- remove Endpoint/PageRangeEntry/Parity pub debt (E-15)
- Merge pull request #2159 from fulgur-rs/mixed/flpdf-6j90n-current
- Merge pull request #2158 from fulgur-rs/mixed/flpdf-b0cjh-current
- Merge main into the nondict-leaf branch, keeping both oracle fixtures
- Merge pull request #2156 from fulgur-rs/flpdf-3yn9.48.160-page-refs-leaf
- *(pages)* cover a direct /Kids-bearing kid in the page walk
- Merge pull request #2151 from fulgur-rs/mixed/flpdf-ihyup-2-current
- *(pages)* satisfy strict lint for coverage probe
- *(pages)* keep coverage mapping on live frame call
- *(pages)* cover live handle boundary branches
- *(pages)* retain repaired page handles through optimization
- Merge pull request #2140 from fulgur-rs/flpdf-3yn9.48.164-cutover
- *(object)* route string callers through asString
- Merge pull request #2138 from fulgur-rs/perf/flpdf-xom94
...
- add qpdf runtime stream-filter registry
...
- *(pages)* invalidate prepared cache before splice
- place later page objects before their map entries
- repair page trees before multi-source page planning
- make shallow copy resolve its receiver
- propagate JSON attachment donor policies
- *(test)* use raw user set length assertion
- *(linearization)* keep raw projection route lint clean
- *(linearization)* place classic thumbnails before outlines
- *(xref)* preserve qpdf recovery diagnostics on live source
- *(xref)* preserve diagnostics during live recovery
- *(xref)* preserve live recovery boundaries
- *(xref)* scan recovery entries from the live source
- preserve raw foreign-copy retry state
- route raw object identity consumers through QpdfObjGen
- *(linearization)* honor qpdf user gates for part7 owners
- *(writer)* branch child references on the raw identity
- *(identity)* settle object zero as null instead of leaving it unresolved
- *(identity)* normalize object number zero handles
- *(encryption)* accept plaintext copy donors as no-op
- *(qtest)* preserve ObjStm DecodeParms warning context
- *(attachments)* accept direct catalog roots
- *(cli)* apply transformations before JSON output
- *(object-handle)* allow boxed containment collection
- reconcile ADBE on indirect PCLm roots
- preserve qpdf linearized ObjStm within-part order
- *(writer)* reserve xref stream before late refs
- *(writer)* number specialized late trailer references
...
- *(linearization)* reuse prepared page sequence
- Merge pull request #2009 from fulgur-rs/perf/flpdf-ymuj-6-41-inline-boundary
- bound linearization code generation footprint
- Merge pull request #2004 from fulgur-rs/perf/flpdf-ymuj-6-34-page-user-reuse
- reuse page-user map for later linearization pages
- document linearization coverage terminator
- cover linearization reachability fallbacks
- reuse optimization reachability for linearization
- Merge pull request #1997 from fulgur-rs/perf/flpdf-ymuj-6-31-object-count-bound
- Merge pull request #1994 from fulgur-rs/fix/flpdf-lrm3u-primary-document-graph
- Merge pull request #1995 from fulgur-rs/fix/flpdf-fwqm1-rustdoc-summary
- Merge pull request #1996 from fulgur-rs/fix/flpdf-3yn9-48-112-shallow-copy
- *(linearization)* cover Preserve identity guards
- *(linearization)* retain Preserve ObjStm container identity
- Merge pull request #1983 from fulgur-rs/perf/flpdf-ymuj-6-28-cached-raw-route
- *(linearization)* cache raw projection route decision
- Document stream dictionary errors
- Test stream dictionary resolver error propagation
- Add resolving stream dictionary accessor
- *(linearization)* consolidate hint generation boundary
- Remove test 31 Pdf::resolve bridge
- Merge pull request #1970 from fulgur-rs/perf/flpdf-ymuj-6-25-qpdf-object-users
- *(linearization)* query the inverse user map per closure object
- *(optimization)* cover invalid ObjectRef projection
- *(linearization)* remove persistent ObjectRef user projections
- Merge pull request #1968 from fulgur-rs/fix/flpdf-jo21-one-page-objstm
- Merge pull request #1967 from fulgur-rs/fix/flpdf-3yn9-48-101-page-label-renderer
- remove qpdf-less page-label renderer
- Merge pull request #1964 from fulgur-rs/fix/flpdf-3yn9-48-100-page-plan-surface
- Merge pull request #1963 from fulgur-rs/fix/flpdf-9zbro-linearized-objstm-extensions
- Fix linearized Generate ObjStm setup membership
- Merge pull request #1961 from fulgur-rs/fix/flpdf-pk64o-linearize-thumbnail-order
- *(linearization)* document unreachable thumbnail branches
- Merge pull request #1957 from fulgur-rs/fix/flpdf-3yn9-48-97-attachmentinfo
- Merge pull request #1956 from fulgur-rs/fix/flpdf-pwyo2-linearization-raw-followups
- Merge pull request #1955 from fulgur-rs/fix/flpdf-1j3p-remove-typed-label-bridge
- Merge pull request #1953 from fulgur-rs/perf/flpdf-ymuj-6-20-live-source
- *(xref)* update canonical live source route contract
- *(qpdf)* record live xref source ownership
- *(xref)* document defensive coverage edges
- *(xref)* cover source-window retries
- *(xref)* cover live source boundaries
- *(xref)* keep canonical sections source-windowed
- *(xref)* load canonical source windows through resolver
- *(xref)* guard canonical live source loading route
- Merge pull request #1952 from fulgur-rs/fix/flpdf-8q38-xref-ownerless
- normalize source guards across platforms
- strengthen xref oracle coverage
- prove canonical xref candidate reachability
- Merge pull request #1950 from fulgur-rs/fix/flpdf-3yn9-48-46-c11
- cover canonical TIFF predictor decode path
- remove qpdf-less recovering filter API
- remove qpdf-less passthrough codec label bridge
- cover raw preserve seed boundaries
- cover unprojectable writer-order keys
- cover raw identity projection boundaries
- *(linearization)* initialize fallback plan directly
- *(linearization)* cover manual part7 owner fallback
- *(linearization)* derive ObjStm page owners from object users
- *(linearization)* derive page reach from object users
- *(object-handle)* move tree ownership to NNTree
- *(linearization)* move writer-owned renumber map
- *(linearization)* cover selected hint setup errors
- *(linearization)* retain only selected hint payload
- Merge pull request #1933 from fulgur-rs/perf/flpdf-ymuj-6-11-token
- Merge pull request #1939 from fulgur-rs/fix/flpdf-kod35-raw-renumber
- Merge pull request #1931 from fulgur-rs/refactor/flpdf-3yn9-48-93-c28
- Merge pull request #1926 from fulgur-rs/test/flpdf-kirsw-qpdf-api-golden
- *(identity)* use one raw qpdf object generation
- Merge pull request #1932 from fulgur-rs/refactor/flpdf-3yn9-48-94-e12-page-transform
- route pages transforms through QPDFJob
- Merge pull request #1924 from fulgur-rs/fix/flpdf-3sbf-object-zero-normalization
- Merge pull request #1921 from fulgur-rs/forward/flpdf-ymuj-6-9-to-main
- *(optimization)* compact small object-user sets
- *(linearization)* derive page users from optimization maps
- Merge pull request #1912 from fulgur-rs/refactor/flpdf-ymuj-6-7-xref-map
- *(linearization)* reuse xref offsets across passes
- Merge pull request #1910 from fulgur-rs/refactor/flpdf-ymuj-6-6-linear-final-sink
- *(job)* update plaintext copy donor expectation
- Merge pull request #1904 from fulgur-rs/refactor/flpdf-ymuj-6-5-qpdfobjgen-width
- Merge pull request #1905 from fulgur-rs/fix/flpdf-nrulb-status-precedence
- *(resolver)* move source extents into cache entries
- Merge pull request #1899 from fulgur-rs/fix/flpdf-hy1g2-show-encryption
- Merge pull request #1898 from fulgur-rs/fix/flpdf-x8bje-parser-description
- *(object-handle)* remove reverse containment bookkeeping
- Merge pull request #1894 from fulgur-rs/fix/flpdf-xpch-empty-overlay
- Merge pull request #1893 from fulgur-rs/refactor/flpdf-ymuj-8-xref-table
- Merge pull request #1891 from fulgur-rs/fix/flpdf-tdf8-direct-root-attachments
- Merge pull request #1885 from fulgur-rs/refactor/flpdf-ydwg-json-transform
- Merge pull request #1883 from fulgur-rs/refactor/flpdf-ymuj-3-10-identity-tags
- *(object-handle)* remove stale mutation generation mapping
- *(object-handle)* guard against mutation counter reintroduction
- *(object-handle)* compact large descriptions
- Merge pull request #1876 from fulgur-rs/refactor/flpdf-ymuj-3-8-mutation-state
- Merge pull request #1875 from fulgur-rs/refactor/flpdf-ymuj-3-7-state-owners
- Merge pull request #1873 from fulgur-rs/refactor/flpdf-ymuj-5
- Merge pull request #1871 from fulgur-rs/refactor/flpdf-ymuj-3-6-canonical-cell
- *(object-handle)* keep empty stream filter state lazy
- *(object-handle)* cover dead containment aliases
- *(object-handle)* compact containment parent storage
- *(object-handle)* cover unmatched owner removal
- *(object-handle)* cover state owner transitions
- *(object-handle)* inline single state owner
- Merge pull request #1864 from fulgur-rs/refactor/flpdf-ymuj-3-4
- Merge pull request #1865 from fulgur-rs/refactor/flpdf-ymuj-6-live-map
- cover removed source ObjStm Preserve planner
- Merge pull request #1862 from fulgur-rs/refactor/flpdf-dz48
- correct xref diagnostic ownership assertions
- Merge pull request #1857 from fulgur-rs/refactor/flpdf-ymuj-4
- move QDF Generate to live queue
- pin QDF Generate live route
- cut over plain Generate to live queue
- *(writer)* annotate unreachable outside-route branch
- *(writer)* cover outside plain route guard
- *(writer)* classify plain routes by behavior
- apply rustfmt to late trailer reservation
- Merge pull request #1846 from fulgur-rs/test/flpdf-jvud-preserve-no-objstm
- *(writer)* resolve startxref to classify the fixture xref form
- *(writer)* pin the fixture's source xref form
- *(writer)* pin the whole plain route region as one expression
- *(writer)* pin the complete Disable-shaped binding expression
- *(writer)* pin the eligibility predicate and the standalone xref header
- *(writer)* pin the full Preserve guard and ungate local xref-form checks
- *(writer)* normalize route contract source line endings
- *(writer)* bind the Preserve route check and fix the xref fixture free entry
- *(writer)* pin empty Preserve object-stream membership
