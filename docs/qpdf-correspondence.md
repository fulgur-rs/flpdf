...
| `qdf_fix.rs` | 1,204 | qpdf の counterpart は `qpdf/fix-qdf.cc`（libqpdf 外の別バイナリ）。ordinary stream は exact な LF-terminated `N 0 obj` の successor に対し、`st_after_stream` が各 exact `%QDF: ignore_newline` 行を1 byteずつ減算し、次行の bare integer を置換する。`/Length M G R` は読まず、successor が無ければ tail xref/trailer も再生成しない（`fix-qdf.cc:87-90,240-264,294-303`）。flpdf は隣接する top-level `ObjectSpan` で同じ遷移を行う。object stream (`/Type /ObjStm`) / cross-reference stream (`/Type /XRef`) は別状態 `st_in_ostream_*` / `st_in_xref_stream_dict` で処理する（flpdf-9hc.43）。 |
