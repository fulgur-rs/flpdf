...
| `QPDFObjectHandle::typeWarning` / `warnIfPossible` / `objectWarning` / `warn` / `getKey` / `getKeys` / `getIntValue` / `getIntValueAsInt`（行数は上段に計上済み） | — | `object_handle.rs` の `type_warning` / `warn_if_possible` / `object_warning` / `warn_through_context` / `context` と `DocumentResolver::warn`、`try_get_key` / `try_get_keys` / `try_get_int_value` / `try_get_int_value_as_int`、`reader/resolver.rs` の `push_object_warning`（`QPDFObjectHandle.cc:502-543,978-1003,2168-2212,2385-2396`; `QPDF.cc:487-494`） | 🔀 メッセージ文言は qpdf と完全一致。context のある handle は `DocumentResolver::warn` → `push_object_warning` で `Pdf::repair_diagnostics` と同じ収集先へ同順に届く。no-context 分岐は qpdf のまま 2 通り — `typeWarning`/`objectWarning` は `throw QPDFExc`（`std::runtime_error` 派生、`QPDFExc.hh:29`）に対応する `Error::System`、`warnIfPossible` は `QPDFLogger::defaultLogger()->getError()` へ素の文言を書いて正常復帰する。**逸脱 2 件**（いずれも object description 伝播で解消）: (1) object description が未実装のため location が空で、qpdf の `"object N G: "` 接頭辞が付かない（`QPDF.cc:1725` が空 description で読むため qpdf 側も filename は付かない。`ResolverHandle::push_warning` の input-source description は `damagedPDF` 用の枠であり、流用すると qpdf が出さない filename を出すため使わない）。(2) context を持つのは canonical indirect handle のみで、direct child は no-context 分岐を通る（qpdf は `QPDFValue::qpdf` を description setter で設定する、`QPDFValue.hh:60-83`）。`asDictionary`/`asInteger` に対応する `try_as_dictionary`/`try_as_integer` は qpdf 同様 warning を出さない |
