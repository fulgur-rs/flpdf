...
| `QPDFObjGen.cc` / `QPDFObjGen.hh` | 68 | `qpdf_obj_gen.rs::QpdfObjGen`（raw xref registration の signed object/generation identity）+ `object_ref.rs::ObjectRef`（qpdf parser の valid indirect-reference surface） | ✅ `QpdfObjGen` は `QPDFObjGen` の int identity、object-number-wide free-row境界、`isIndirect`を担当し、`ObjectRef`へは `1 <= object` と `0 <= generation < 65535` の境界で変換する。linearization synthetic identity は raw xref primitive に混入させない。 |
...
接続する。`showXRefTable` は raw viewを表示し、通常のresolver/writer consumerは
valid viewを使う。`QPDF.cc:689-718,1149-1184,1212-1236` に対応し、raw viewの
