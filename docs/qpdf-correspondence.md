...
| `QPDF_Stream::registerStreamFilter` / `QPDF_Stream::filterable` | `QPDF_Stream.cc:33-50,72-94,148-152,378-485` | `stream_filter.rs` の `stream_filter_for` / `FilterSpec` / `decode_filter_specs_from_handle` と `filters.rs` の `decode_prepared_specs` | 🔀 qpdfのfilter factoryは`filter_factories`の`std::map`を名前引きに使い、`registerStreamFilter`でruntime登録できる。flpdfは既存の`match` registryを使い、runtime登録APIは親issue `.48.46` の後続scopeで扱う。`FilterSpec`はqpdfがchainへ渡す`QPDFObjectHandle`に対応するnative `ObjectHandle`を保持し、縮約`DecodeParams`/`ParamValue`/retention snapshot層は削除済み。`QPDF_Stream::filterable`のfactory lookup、`/DecodeParms` shape・長さ検証、`setDecodeParms`順序はcanonicalなObjectHandle readerと`decode_prepared_specs`で共有する。 | 🔀 |
| `QPDFObjectHandle::isArray` / `isDictionary` / `isNameAndEquals` / `isDictionaryOfType` / `isStreamOfType` / `getArrayNItems` / `getArrayItem` / `isOrHasName`（行数は上段に計上済み） | `QPDFObjectHandle.hh:366-374` | `object_handle.rs` の `try_is_array` / `try_is_dictionary` / `try_is_name_and_equals` / `try_is_dictionary_of_type` / `try_is_stream_of_type` / `try_array_len` / `try_array_item` / `try_is_or_has_name`（`QPDFObjectHandle.cc:240-267,426-471,759-785,1027-1039`） | ✅ holder を qpdf 順に lazy resolveし、`try_is_array` / `try_is_dictionary` は value kind だけを検査して child collectionをsnapshotしない。`isStreamOfType` は qpdf の `isStream() && getDict().isDictionaryOfType(...)` をそのまま stream 内辞書へ委譲し、container borrow は resolver 再入前に解放する。配列全体をsnapshotせず、`try_array_len` は長さだけを読み、`try_array_item` はvalid-index面のみを契約に含める。owned collectionが必要な `try_as_array` / `try_as_dictionary` と `as_*` snapshot accessorは別責務として残す |
...
| `QPDFObject.cc` / `QPDFValue.cc` | `libqpdf/qpdf/QPDFObject_private.hh:19-151,176-180`; `libqpdf/qpdf/QPDFValue.hh:18-153`; `libqpdf/QPDF.cc:1843-1858` | `object_handle.rs` の outer `ObjectSlot` + `SharedValueState` / `ObjectValue` / `ValueIdentity` | 🔀 `SharedValueState` が qpdf の一つの `QPDFValue` pointer に対応し、payload、active identity、parsed offset、optional description state を共有する。qpdf の `Description` は `std::shared_ptr` で保持され、flpdf は parser-owned Template を parse call ごとに `Rc<Vec<u8>>` で共有し、より大きい Json/Child variant を box 化して description-free value の inline layout を小さく保つ。stream-local token-filter list と normalization marker は ObjectValue::Stream に置き、qpdf に対応しない mutation-generation counter は保持しない。outer `ObjectSlot` は別個の qpdf `QPDFObject` counterpart として handle/cache/provenance identity を保ち、`initialized`、cache の `end_before_space` / `end_after_space`、`tree_pdf_unique_id`、per-slot `containment_parents` は value pointer の外に残す。これは `assign` の value-pointer assignment、`swapWith` の value-pointer swap と ObjGen 復元、`updateCache` の per-cache span 更新に対応する。`state_owners` は shared value aliases に per-slot containment edge を伝播するため `SharedValueState` 内に残る flpdf-only bookkeeping であり、qpdf に reverse containment index があるという主張ではない。 |
| `QPDFObjGen.cc` / `QPDFObjGen.hh` | 68 | `qpdf_obj_gen.rs::QpdfObjGen`（raw xref registration・`ObjCache`・`ObjectHandle` の signed object/generation identity）+ `object_ref.rs::ObjectRef`（qpdf parser の valid indirect-reference surface）+ `reader/resolver.rs::ResolverCore::object_cache` / `object_handle.rs::ValueIdentity` | ✅ `QpdfObjGen` は raw xref と canonical cache/handle の identity、object-number-wide free-row境界、`isIndirect`を担当する。`ObjectRef` は `1 <= object` と `0 <= generation < 65535` の parser/public projection に限定し、object header の generation gate と混同しない。linearization synthetic identity は raw xref primitive に混入させない。⚪ `QpdfObjGen` のフィールドは `int` ではなく `i64`（`qpdf_obj_gen.rs` の `qpdf-deviation` マーカー参照）: qpdf に対応物のない Rust 側 `ObjectRef::new(u32, u16)` からの射影を全域にするための拡幅で、header 由来の値は `new`/`try_from_object_ref` の signed-int 境界を通るため qpdf の範囲を出ない。`ObjectRef` 側の narrowing は別途追跡。 |
...
| `QPDFStreamFilter.cc` | 19 | `stream_filter.rs`（`set_decode_params`、decode pipeline factory、specialized / lossy の既定分類）。`QPDFStreamFilter::getDecodePipeline`（`QPDFStreamFilter.hh:46-49`）に対応する`StreamFilter`はnative full handleをsetterへ渡し、`None`はqpdfの`nullptr`（11.9.0でこれを返すのは`SF_Crypt`だけ、`QPDF_Stream.cc:52-56`）。qpdf-shaped production callerは`ObjectHandle::pipe_stream_data`に接続済みで、`.48.49` で非qtestの public whole-buffer decode/encode helperは撤去した。qtest exceptionのrecovering adapterだけは別セッションに残る。旧`DecodeParams`/`ParamValue` snapshot責務は削除済みで、runtime registryは親issue `.48.46` の後続scopeである。 | 🔀 |
...
canonical な `decode_filter_specs_from_handle` route で保持している。旧
Dictionary/value adapter と object-shaped production reader は legacy object-model route
の削除（`d18ce346`）で除去済みである。未知フィルタと長さ不整合を組み合わせた場合も、
同じ `unsupported stream filter` エラーを先に返す。qpdf 11.9.0 の既存 fixture
...
| `xref.rs::load_xref_state_with_options` の `startxref` offset-0 経路 | qpdf の `xref_offset == 0` チェック（`QPDF.cc:450-452`）は、`startxref` が解析できない場合と、構文的に正しい `startxref` が明示的に offset 0 を指す場合の両方で、即座に `damagedPDF("can't find startxref")` を投げ `read_xref` を一切呼ばない。flpdf はどちらの経路でも `startxref == 0` のまま実際に `parse_xref_from_start` を再試行する — qpdf に対応物のない retry-at-offset-0 detour。`push_repair_diagnostics` 自体は qpdf の `reconstruct_xref` 3行警告シーケンスを忠実に再現するだけで、対応物のない挙動はこの detour 側にある。 | `parse_startxref` の解析失敗フォールバックと明示的 offset-0 の両方、およびその後続の `parse_xref_from_start` 呼び出し |
| `object_handle.rs::ObjectSlot::pdf_unique_ids`（`flpdf-ymuj.3.2` で削除） | qpdf は document-level `unique_id`（`QPDF.hh:1454`, `QPDF.cc:2294-2296`）と各 value の `QPDF*` back-pointer（`QPDFValue.hh:60-80,149-152`）を持つが、別の per-object numeric id set は持たない。`active_pdf_unique_id`（単一値）は qpdf の `QPDF*` back-pointer への container 表現代替（CLAUDE.md 分類 (B)）であり、direct value のownership判定もこの単一値または未所有状態に揃えた。 | 旧history setと `attach_child_to_parent` のsubtree stampingを撤去。現行の `containment_parents` は qpdf ownershipではなく、現在のdirect containment rootとteardownのためのweak edgeだけを保持する。 |
...
shared value alias ごとの propagation を保つ `SharedValueState::state_owners` も
flpdf-only である。これは `ObjectSlot` ごとに残る reverse edge を同じ shared value
の各 outer slot に反映する弱い owner list であって、qpdf の `QPDFObject` / `QPDFValue`
に reverse containment index を見出すものではない。qpdf 側の `QPDFValue::ChildDescr`
weak parent は description 用だけであり、array/dictionary の child storage は forward
handles のみである（`QPDFValue.hh:41-58,74-84`; `QPDF_Array.cc:39-58,103-119`;
`QPDF_Dictionary.cc:11-19,50-56`）。
