...
view. The canonical flpdf writer keeps the same separation: `qpdf_null_visibility`
...
| `QPDFNameTreeObjectHelper` / `QPDFNumberTreeObjectHelper` / `NNTree.cc` | 1394 (`34-75,106-168,216-390,391-520,560-700`) | `nntree.rs`（shared canonical `ObjectHandle` engine + handle-native public `NameTree`/`NameTreeCursor` and the remaining `NumberTree` wrapper）+ consumer adapters。qpdf の live `QPDFObjectHandle`/`QPDF_Array` mutation（`NNTree.cc:34-75` の iterator value 更新、`:106-168` の limits、`:216-390` の split/insert、`:391-520` の remove/deepen、`:560-700` の find）に対応し、`ResolvedArray` は `ObjectHandle::set_array_items` で alias を保持したまま更新、direct kid の indirect 化は `Pdf::make_indirect_from_object_handle`、root split は既存 root slot を維持する。dirty propagation で canonical mutation を観測する。`NameTree`/`NameTreeCursor` はこのsliceでroot・key/value・cursor mutationをraw `Object` projectionなしに切り替えた。`NumberTree`のraw root projectionとgeneric `materialize_cursor_value`は後続のbounded cutoverに残る。`legacy_terminal_handle`/`root_handle` の bare-reference chase は同種の bridge パターン（本表 §7、`pages.rs`/`page_annotation_flatten.rs`/`outline_document_helper.rs` 行を参照）で、qpdf の `QPDF::replaceObject` は indirect replacement を拒否する（`QPDF.cc:1986-1991`）ため対応物は無い | 🔀 |
...
`job/rotate_spec.rs`(204) / `job/page_collate.rs`(145)
