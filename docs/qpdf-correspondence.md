...
進捗計測の準備境界は `writer/pdf_writer.rs:499-517` に固定する。QDF/content-normalization
の `PageDocumentHelper::get_all_pages()` による page-tree 修復を先に実行してから
`get_object_count()` を取得し、qpdf の `doWriteSetup`→progress snapshot
(`QPDFWriter.cc:2114-2115,2189-2193`) と同じ順序で、修復が mint した indirect object も
`events_expected` に含める。linearized writer は既存の準備後 snapshot を維持する。
