...
| `Pl_SHA2.cc` | 75 | `pipeline/sha2.rs`（SHA-256/384/512 の bit 選択、`resetBits`/`write`/`finish` lifecycle、optional next への forwarding。二重 `finish()` は `next` への転送のみ qpdf 同様毎回実行し、digest 自体は error にする — 詳細は次行）。⚪ 2種の逸脱を defined logic error に変換する。(1) メモリ安全性: `bits=0`（未 `resetBits`）または `write`-after-`finish` 再利用時に null/stale な crypto provider ポインタを非検査で dereference する（`libtests/sha2.cc` は未検証、対応する byte 列が存在しない）。(2) 決定的だが再現不能: 二重 `finish()` は qpdf では UB ではなく、`sph_sha2` の `md_helper.c`（"The context is NOT reinitialized by this function"）が示す通り、既に finalize 済みの running state に同一の padding block を再度 `RFUN` で圧縮し、1回目と異なる決定的な digest を生成する。SHA-384 はこれの再現が `sha2` crate の公開APIでは原理的に不可能（`sph_sha384_close` は 8×u64 語の running state を 48 バイト=6語に truncate して出力するが（`sha2big.c`: `sha384_close(cc, dst, 6)`）、`Sha384::finalize()` も同じ48バイトしか返さず、残り2語を再構築する手段が無い）ため、256/512 だけ再現するのではなく3種とも一律 error にする。出力バイトに影響しない（このパスへ到達する production consumer が存在しない） | ✅ |
