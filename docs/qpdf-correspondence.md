...
| `QPDFObjectHandle::typeWarning` / `warnIfPossible` / `objectWarning` / `warn` / `getIntValue` / `getIntValueAsInt`（行数は上段に計上済み） | — | `object_handle.rs` の `type_warning` / `warn_if_possible` / `object_warning` / `warn_through_context` / `context` と `DocumentResolver::warn`、`try_get_int_value` / `try_get_int_value_as_int`、`reader/resolver.rs` の `push_object_warning`（`QPDFObjectHandle.cc:502-543,2168-2212,2385-2396`; `QPDF.cc:487-494`） | 🔀 メッセージ文言は qpdf と完全一致。context のある handle は `DocumentResolver::warn` → `push_object_warning` で `Pdf::repair_diagnostics` と同じ収集先へ同順に届く。no-context 分岐は qpdf のまま 2 通り — `typeWarning`/`objectWarning` は `throw QPDFExc`（`std::runtime_error` 派生、`QPDFExc.hh:29`）に対応する `Error::System`、`warnIfPossible` は `QPDFLogger::defaultLogger()->getError()` へ素の文言を書いて正常復帰する。**逸脱 2 件**: (1) object description が未実装のため location が空で、qpdf の `"object N G: "` 接頭辞が付かない（`QPDF.cc:1725` が空 description で読むため qpdf 側も filename は付かない。`ResolverHandle::push_warning` の input-source description は `damagedPDF` 用の枠であり、流用すると qpdf が出さない filename を出すため使わない）。(2) context を持つのは canonical indirect handle のみで、direct child は持たない — qpdf は `QPDFParser` が生成する全 value に `QPDFValue::qpdf` を設定する（`QPDFParser.cc:416-442`）ため、qpdf の direct child は常に context を持つ。**この (2) のため `getKey`/`getKeys` の `typeWarning` は未移植** — 移植すると `SF_FlateLzwDecode::setDecodeParms`（型ガード無しで `getKeys()` を呼ぶ、`SF_FlateLzwDecode.cc:28`）に対応する `stream_filter.rs` の consuming `/DecodeParms` 経路で、非辞書 `/DecodeParms` が qpdf の警告ではなく `Error::System` になる。`asDictionary`/`asInteger` に対応する `try_as_dictionary`/`try_as_integer` はqpdf 同様 warning を出さない |
