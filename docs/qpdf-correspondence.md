...
| `InputSource` 系 5 ファイル | 625 | `Read + Seek` ジェネリクスで代替。所有者は `reader/resolver.rs` の `ResolverCore`（`m->file` 相当）で、`OffsetInputSource`（`QPDF.cc:406`）が担う header shift は `ResolverCore::read_window` が適用する | ⚪ |
