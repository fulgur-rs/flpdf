...
| `qdf_fix.rs` | 1,214 | qpdf の counterpart は `qpdf/fix-qdf.cc`（libqpdf 外の別バイナリ）。ordinary stream は exact な LF-terminated `N 0 obj` の successor に対し、`st_after_stream` が各 exact `%QDF: ignore_newline` 行を1 byteずつ減算し、次行の bare integer を置換する。`/Length M G R` は読まず、successor が無ければ tail xref/trailer も再生成しない（`fix-qdf.cc:87-90,240-264,294-303`）。flpdf は `endstream` 行の直後を記録し、次の認識済み top-level `ObjectSpan` header までを同じ marker 範囲として走査するため、`endobj` より前の marker も含める（`flpdf-8cmfr`）。object stream (`/Type /ObjStm`) / cross-reference stream (`/Type /XRef`) は別状態 `st_in_ostream_*` / `st_in_xref_stream_dict` で処理する（flpdf-9hc.43）。 |
