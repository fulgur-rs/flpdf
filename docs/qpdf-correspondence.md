...
| `QPDFPageDocumentHelper.cc` | 158 | `page_document_helper.rs`(236) + `page_extract.rs`(385-402: `minimal_target_bytes()`。`emptyPDF()` + `addPage()` 経路、doc に明記。`Pdf::empty()` へは未委譲 — `flpdf-jq0z` のバージョン伝播修正が先) | 🔀 `emptyPDF()` 自体（`QPDF.cc:34-51,290-293`）の canonical 実装は `reader.rs`(3451-3502: `EMPTY_PDF_BYTES` + `Pdf::empty()`)。⚪ qpdf の `emptyPDF()` は default-construct 済み `QPDF` を遅延初期化する `void` メンバー関数だが、flpdf の `Pdf` に「未初期化」状態が無いため static factory（`Result<Self>` を返す）に置き換えている。バイト列・parse 経路（`open_mem_owned` = `processMemoryFile` 相当）は同一 |
