...
| `std::shared_ptr<QPDFValue>` → `Rc<RefCell<..>>`（`object_handle.rs`） | 79 | 無し（共有 identity の内部所有権機構のみ。live direct containment の weak reverse index は qpdf の現在の forward membership から派生する incremental dirty bookkeeping で、共有 identity や emitted bytes を変更しない。Pdf identity provenance は別フィールドで保持。byte-identical suite で確認済み） |
