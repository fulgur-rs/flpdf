...
flpdf 側の repository-level oracle coverage は `crates/flpdf-cli/tests/cli_inline_images_transform.rs` が pinned qpdf 11.9.0 を直接実行し、8通りの externalize/optimize/keep 条件、`--pages`、nested Form、named colorspace、画像なし、damaged inline image、inclusive threshold を flpdf の page-image JSON と比較する。fixture は flpdf-authored で、qpdf-qtest の vendor copy や qtest-only shim は追加しない。
