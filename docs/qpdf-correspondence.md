...
| `QPDFObjGen.cc` / `QPDFObjGen.hh` / `QPDFObjectHandle::getObjGen` | 68 | `qpdf_obj_gen.rs::QpdfObjGen`（crate root から公開 re-export、raw xref registration・`ObjCache`・`ObjectHandle` の signed object/generation identity）+ `object_handle.rs::ObjectHandle::get_obj_gen` + `object_ref.rs::ObjectRef`（qpdf parser の valid indirect-reference surface）+ `reader/resolver.rs::ResolverCore::object_cache` / `object_handle.rs::ValueIdentity` | ✅ root re-export `QpdfObjGen` と `ObjectHandle::get_obj_gen()` が raw identity を公開する。qpdf の `getObjGen` は `include/qpdf/QPDFObjectHandle.hh:1145-1152` / `libqpdf/QPDFObjectHandle.cc:2563-2567` の通り direct/uninitialized handle に `(0, 0)` を返し、indirect handle の raw identity を返す。`QpdfObjGen` は qpdf と同じ signed `i32` の object/generation pair と raw xref・canonical cache/handle identity、object-number-wide free-row境界、`isIndirect`を担当する。`ObjectRef` は Rust 側の広い public/parser projection であり、raw identity へ入るときは `try_from_object_ref` の checked signed-int 境界を通す。`ObjectRef` の `1 <= object` と `0 <= generation < 65535` projection、object header の generation gate、linearization synthetic identity は raw QpdfObjGen と混同しない。object number 0 は qpdf の raw xref/free identity としてのみ保持し、`ObjectHandle` の indirect/projection にはしない。ただし解決は行う — qpdf は `QPDF::getObject`（`libqpdf/QPDF.cc:1952-1959`）でも `QPDF::resolve` でも `isIndirect()` や generation 範囲で分岐せず、xref に行のない identity を null に帰着させる（`libqpdf/QPDF.cc:1745-1748`）。`N G R` 範囲外の generation も同じ扱い。 |
...
The public `PageDocumentHelper::get_all_pages` returns the same raw handle
sequence; `pages::page_refs` remains the separate checked `Vec<ObjectRef>`
projection for consumers that require valid `N G R` references. No second
page-tree traversal is performed. `update_all_pages_cache` forces regeneration as qpdf's
...
1621-1740`). flpdf's production `QPDFJob::handlePageSpecs` now keeps that same
primary as its page-operation target (`job/page_specs.rs` and
`page_document_helper.rs`), so the original type-2 member-to-container map and
the complete primary object cache remain in the live document. Foreign page
graphs are copied into that primary through `object_copy.rs::copy_foreign_object`
in occurrence order. The former source-container placeholders in
`job/page_merge.rs` remain only for the generic fresh-target merge helper and
are not on the Job `--pages` route. qpdf-zlib differentials cover the Job
Preserve/Generate, QDF, normalization, and linearized page-merge paths,
including indirect `/Extends`.
...
`writer::ObjectWriterEmission::{unparse_object_with_string_writer,unparse_object_qdf_with_string_writer}`
...
`flpdf-ihyup.1` では、page-selection のAcroForm field pruningを qpdf の raw identity境界へ寄せた。`QPDFFormFieldObjectHelper::getTopLevelField` は `QPDFObjGen::set` を使って live `QPDFObjectHandle` を返し、`N G R` のgeneration範囲へ射影しない（`QPDFFormFieldObjectHelper.cc:35-46`）。flpdf は `FormFieldObjectHelper::get_top_level_field` と `AcroFormDocumentHelper::top_level_field_handles` をcanonical routeとし、`job/page_specs.rs::collect_primary_fields` は `QpdfObjGen`集合で候補と `/Fields`順を照合する。top-level field lookup は `ObjectHandle` を返し、ObjectRef projectionやそのcompatibility viewを介さずraw identityを維持する。
...
| `QPDFFormFieldObjectHelper.cc` | 852 | `form_field_object_helper.rs` + `form_field_object_helper/rendering.rs` + `default_appearance.rs`（field lookup/mutation と Tx/Ch appearance generation。`QPDFFormFieldObjectHelper.cc:472-478` に従い Btn appearance は production dispatch から除外）。既存 `/AP/N` は qpdf の `ValueSetter` 相当を同じ streamに登録する `AppearanceTokenFilter`（qpdf `QPDFFormFieldObjectHelper.cc:766-852`）で更新し、state dictionaryの`/AS`選択も`AnnotationObjectHelper`へ委譲する。新規APはqpdfどおり`/ProcSet`だけを初期Resourcesに置き、fontは既存AP `/Resources`→`/AcroForm /DR`で実際に見つかった場合だけ同じhandleを追加する（qpdf `:779-849`）；見つからないFont合成・`/FormType`追加は行わない。encodingはqpdf `QUtil`のASCII/WinAnsi/MacRomanを選ぶ。CLI の `generate_missing_appearances` は non-`/Btn` を `/AP/N` の有無で skip せず canonical helper へ渡す（qpdf `QPDFAcroFormDocumentHelper.cc:393-415`）。`crates/flpdf-cli/tests/cli_acroform_transforms.rs::generate_appearances_tx_reuses_existing_ap` は `/NeedAppearances true` の既存 stream を `--compress-streams=y` で再書き込み、`DecodeLevel::Generalized` 後の `/Tx BMC`/`Tf` と no-wrapper source preservation を確認する。qpdf 11.9.0 pinned source と `/usr/bin/qpdf` の live probe でも同じ入力の既存AP、無`/DR`、MacRoman入力を確認済み。token-filter primitive自体の変更は本 issueのscope外。計算・解決を行う公開 getter 名はqpdfの`get*`識別子を保持し、`get_inheritable_field_value`/`get_inheritable_field_value_as_string`/`get_inheritable_field_value_as_name`/`get_field_type`/`get_fully_qualified_name`/`get_partial_name`/`get_alternative_name`/`get_mapping_name`/`get_value`/`get_value_as_string`/`get_default_value`/`get_default_value_as_string`/`get_default_appearance`/`get_default_resources`/`get_quadding`/`get_flags`/`get_choices`へ写す。qpdf deviation class (C): `QPDFObjGen::set` は direct nodes を無視するため、flpdf の `mark_direct_node_seen` / `direct_parent_cycle_error` は in-memory direct `/Parent` cycle を `Error::Unsupported` で止める safety deviation（`include/qpdf/QPDFObjGen.hh:108-121`; `libqpdf/QPDFFormFieldObjectHelper.cc:35-46`）。`mark_direct_node_seen` は `#[deprecated(note = "no qpdf counterpart; ...")]` で記録し、3 production caller と test caller に局所 `#[allow(deprecated)]` を付ける。 | 🔀 |
| `QPDFPageDocumentHelper.cc` | 158 | `page_document_helper.rs::get_all_pages` (public raw `ObjectHandle` page list) + `pages.rs::page_refs` (separate checked `ObjectRef` projection) + `page_document_helper.rs` page mutation APIs that still take valid references + `page_extract.rs`(`extract_pages`/`extract_page`) + `job/page_merge.rs`(`merge_documents`)。`job/overlay.rs` の source/destination page snapshot は現在 `pages::page_refs()` で raw repaired list を明示的に `ObjectRef` へ射影し、`QPDF_pages.cc:39-138` 相当の repair（欠落 `/MediaBox` の Letter fallback と warning）を Form 化・placement 前に適用する。両モジュールとも `Pdf::empty()` へ委譲（`emptyPDF()` + `addPage()` の library-level 経路、doc に明記）。`Pdf::uninitialized()` は qpdf の `QPDF()` の未処理状態、`Pdf::close_input_source()` は `closeInputSource()` の入力ソース差し替えをそれぞれ canonical な resolver state として公開する |
...
| `QPDFJob_config` / `_argv` / `_json` / `QPDFArgParser` | 3164 | `flpdf-cli/src/arg_parser.rs` + `SegmentHandler` / `QpdfSegmentHandler` in `main.rs` + clap | ⚪。QPDFJobの使用エラー分類は [`UsageError`](../crates/flpdf/src/error.rs) + `Error::Usage` として job lifecycle から CLI の `usage_exit` へ伝播し、`QPDFUsage` の別catch経路（`qpdf/qpdf.cc:10-23,34-39`）を再現する。CLI の入口は `std::env::args_os()` とし、qpdf argv grammar の residual/segment tokens、`--pages`/`--overlay`/attachment の path、`QPDFJob` の input description を `OsString`/raw bytes のまま保持する。UTF-8 が必要な selector・range・日付などだけを各 option parser の境界で検証し、非UTF-8 argv を `std::env::args()` の unwrap で失わない。`flpdf-v1xw` では argv token を raw bytes と `OsString` 投影の二重キャリア `RawArg` で運び、clap の parse 後に `raw_option_value` / `apply_raw_overrides` が byte-oriented な値（password 系）を raw 側で上書きする。qpdf は argv を 1 度しか走査する（`QPDFArgParser.cc:433-569`）ため、`SegmentHandler` は各 named-segment token と終端 callback を同じ走査中に `QpdfSegmentHandler` へ届け、typed state を downstream consumer へ渡す。この (B) の入れ物の差を含め、受理するコマンドラインと出力は qpdf と同じことを `cli_arg_parsing_segments.rs` と qtest `arg-parsing` 25 ケースで検証する。**Classification (B) argv carrier alternative (`flpdf-g25ac`, 2026-09-23)**: qpdf's `QPDFArgParser` invokes `jobJsonFile` and named-segment Config callbacks in one left-to-right pass (`libqpdf/QPDFArgParser.cc:433-569`; `QPDFJob_config.cc:774-784`). flpdf's `retain_in_residual` is the carrier split for its staged parser: the ordinary qpdf-style route applies typed segment values collected by `QpdfSegmentHandler`, while `--job-json-file` replays the raw named-segment tokens through `QPDFJob::initialize_from_expanded_raw_argv`, updating the same partial job state in argv order. This transport split preserves qpdf-visible output: `cli_job_json.rs::overlay_underlay_transport_matches_qpdf_with_and_without_job_json` compares qpdf 11.9.0 and flpdf output bytes for overlay and underlay both with and without an empty job-json file using `--compress-streams=n`. Native subcommands are a separate flpdf CLI surface and are not part of this qpdf-style argv carrier mapping. |
...
（toolchain実装詳細の再現対象外）であることを明記した。

`tests/fixtures/compat/*.pdf` 178件を `--static-id --pages <f> 1 -- <f> out`
で掃引した実測（qpdf 11.9.0 / flpdf、2026-09-20）:

- **exit code は178件すべて一致**。
- **出力バイトは、決定的に比較できる162件すべて一致**。両者が出力を書かない
  13件（0ページ文書12件と1件）は比較対象外。encryptedの3件
  （`encrypted-r4-three-page`/`encrypted-recovered-eol`/
  `encrypted-recovered-eol-valid`）は `--static-id` でも
  **qpdf自身が実行ごとに異なるバイトを出す**ため、この掃引では比較しない。
- **stderr が一致しないのは33件**で、内訳は2つだけ:
  - **12件**が本項のdeviation（0ページ文書。qpdfのlibstdc++ artifact vs
    flpdfの安定診断）。
  - 残り**21件**は `operation succeeded with warnings` 行の先頭が
    `qpdf: ` か `flpdf: ` かの違いのみ。qpdfは `QUtil::getWhoami(argv[0])`
    を前置する（`qpdf/qpdf.cc:29,40`）ので、flpdfが自分のprogram nameを
    前置するのは対応関係として正しい。WARNING行自体は全件byte一致で、
    prefixを揃えると21件すべて完全一致する。

新しいparser・bridge・side-file cacheは追加しない。
...
flpdf 側の repository-level oracle coverage は `crates/flpdf-cli/tests/cli_inline_images_transform.rs` が qpdf 11.9.0（PATH 上の実行ファイル、`--version` で版を検証）を直接実行し、8通りの externalize/optimize/keep 条件、`--pages`、nested Form、named colorspace、画像なし、damaged inline image、inclusive threshold、2枚の画像を使う混在 threshold (0/10/100)、qpdf の `large-inline-image.pdf` による delimiter-surrounded `EI`、EOF-in-inline-image の exit 3（通常と externalize）を比較する。比較は 2 層で、page-image JSON（qpdf 出力・flpdf 出力の両方を qpdf で読む）に加えて、`--pages` / nested Form / damaged / mixed threshold / large EI / EOF の各ケースでは `--qdf --static-id` の **whole-file byte 比較**と stderr・exit code の一致も確認する。JSON だけでは 2 種類の穴が塞げない。(1) nested Form: qpdf の `doJSONPages` は page 直下の XObject しか列挙しない（`QPDFJob.cc:1044` → `QPDFPageObjectHelper.cc:376-384`）のに対し `externalizeInlineImages` は Form XObject の resources へも再帰する（`QPDFPageObjectHelper.cc:430-434`）ため、変換の有無にかかわらず両側とも `[]` になる。inline のまま残る truth table 行と閾値超えのケースも、XObject が生成されないため同じく `[]` になる。(2) `--pages` の選択違い: こちらは配列が空なのではなく、**どちらのページを残しても同一の正規化メタデータ 1 件**になる（object 参照は正規化で落とし、異なる pixel payload は JSON に現れない）ため区別できない。いずれも whole-file byte 比較でのみ検出できる。`--qdf` は stream 圧縮を無効化するので DEFLATE 逸脱 (A) が出力に現れず、この byte 比較に `qpdf-zlib-compat` feature は不要である（qpdf 自身の `inline-images.test:93-94` も whole-file 比較を使う）。合成 fixture は flpdf-authoredで、qpdf の `large-inline-image.pdf` は Rust test fixture にコピーして使う。qpdf-qtest の vendor files と qtest-only shim は変更しない。
...
| `qdf_fix.rs` | 1,290 | qpdf の counterpart は `qpdf/fix-qdf.cc`（libqpdf 外の別バイナリ）。`st_top` は exact な `xref\n` 行で `st_at_xref` へ遷移し、object が0件でも subsection `0 1` と free object-0 行を出力し、trailer `/Size` を1へ書き換える（`fix-qdf.cc:130,265-287`; `flpdf-bcryr`）。ordinary stream は exact な LF-terminated `N 0 obj` の successor に対し、`st_after_stream` が各 exact `%QDF: ignore_newline` 行を1 byteずつ減算し、次行の bare integer を置換する。`/Length M G R` は読まず、xref/trailer/`startxref`/`%%EOF` でも state は変わらないため、後続 header は tail の後でも positional holder になる。input の終端まで successor が無い場合だけ tail xref/trailer を再生成せずコピーする（`fix-qdf.cc:87-90,240-264,294-303`; `flpdf-nc7jn`）。flpdf は通常走査では最初の exact `xref\n` で停止するが、最後の通常streamが `st_after_stream` に残る場合は tail 全体から後続 header を探し、同じ object-number / integer-line 順序と marker count を適用する（`flpdf-nc7jn`）。flpdf は `endstream` 行の直後を記録し、次の認識済み top-level `ObjectSpan` header までを同じ marker 範囲として走査するため、`endobj` より前の marker も含める（`flpdf-8cmfr`）。flpdf は successor header を認識した時点で object number を検証し、後続 body の parse より先に expected-integer 条件を判定する（`flpdf-14zs3`）。object stream (`/Type /ObjStm`) / cross-reference stream (`/Type /XRef`) は別状態 `st_in_ostream_*` / `st_in_xref_stream_dict` で処理する（flpdf-9hc.43）。 |
...
| `xref.rs::LoadedXref::repair_diagnostics`（`flpdf-3yn9.48.177` で監査、コードマーカーなし。~~`XrefStreamFailure::diagnostics`~~ は 2026-09-20 `flpdf-po4te` で撤去） | qpdf の `m->warnings`（`QPDF.cc:487-494`）は push_back のみで `warn()` 呼び出し順に即時配送される単一 sink。flpdf は複数の xref candidate（/Prev chain・hybrid section・reconstruction fallback）を試すため、Rust の関数境界を跨いで分割生成される診断を `Diagnostics` 値として一旦保持し、`deliver_canonical_diagnostics` で drain する。**`LoadedXref::repair_diagnostics` については** container 代替（分類 (B)）: `merge_recovered_qpdf_state`（`xref.rs:2157`）が `accumulated`（先行）→`recovered`（後発）の順で結合し、`:1049` と `:1138` の `mem::take` は 1 まとまりを次段へ渡すだけで並べ替えない。**2026-09-19 訂正（レビュー指摘により実測し直した）**: 当初この行は根拠として `prepend_repair_diagnostics` と `XrefStreamFailure::diagnostics` も挙げ、「順序の入れ替えや内容破棄は無い」と書いていたが、3 点とも事実と違った——(1) `XrefStreamFailure::diagnostics`（`xref.rs:3307`）は**常に空**で、`XrefStreamFailure::new`（`:3313`）が `Diagnostics::default()` を入れるだけ、push する箇所が存在せず、2 つの consumer は空集合を回している（冗長な state であって診断を運ぶ container ではない）。(2) `prepend_repair_diagnostics`（`:2146`）は production では**一度も実体動作しない**——唯一の呼び出し元 `:1038` が渡す `initial_diagnostics` は `:837` で既に drain 済みで、`load_xref_state_from_window` の他の呼び出し元（`:849`・`:871`・`:917`）は fresh default を渡すため、常に空入力の early return を取る。(3) `deliver_canonical_diagnostics`（`:181`）は先にバッファ全体を drain してから push していたため、owner の warning sink がバッチ途中で失敗すると以降の警告が失われていた。qpdf は `warn()` 1 回ごとに `m->warnings` へ append するので未到達分は残る——これは container ではなく**挙動の差**であり (B) の条件 2（処理順序は qpdf のまま）を満たさないため、**この PR で未配送分をバッファへ戻すよう修正した**（回帰テスト `xref.rs::a_failing_warning_sink_leaves_the_undelivered_tail_pending`。tail 復帰を撤去する mutation で FAILED になることを確認済み）。**2026-09-20 解消（`flpdf-po4te`）**: (1) と (2) の冗長機構は、production で非空になり得ないことを probe（`prepend_repair_diagnostics` の early-return 側を一時的に `panic!` へ、`XrefStreamFailure::diagnostics` の 2 つの consumer を一時的に `assert!(...is_empty())` へ差し替え）で実測してから削除した——workspace 全体のテスト、qpdf qtest fixture 589 件、`tests/fixtures/compat/*.pdf` 178 件を `--check` と `rewrite --qdf --object-streams=generate` の両方で掃引し、いずれも発火しなかった。`prepend_repair_diagnostics` 関数と `xref.rs:1073` の呼び出し、`XrefStreamFailure::diagnostics` フィールドと 2 箇所の forwarding ループ、forwarding のためだけに存在していた `hybrid_error_diagnostics` ローカル変数を削除した。よって CLAUDE.md 分類 (C) のコードマーカー（`// qpdf-deviation`）は付けない。 | `docs/qpdf-route-matrix/b-parser-recovery-diagnostics.md` の B27 行を対応させて更新済み。B29 の snapshot consumer（CLI lazy warning emission 等）は別issue範囲として着手せず — 単一 `ResolverCore::repair_diagnostics` collection への正当な public accessor consumer であり別 sink ではないため |
| `xref.rs::LoadedXrefState::already_reconstructed`（`flpdf-3yn9.48.176` の B25 write-through で削除） | qpdf の `m->reconstructed_xref`（`QPDF.hh:1480`）は `reconstruct_xref` が成功した瞬間に `this` へ直接書く（`QPDF.cc:518-522`）。flpdf は同じ書き込みを `xref.rs::recover_xref_from_linear_scan` の成功地点で `CanonicalTrailerOwner::set_reconstructed_xref`（新規、`ResolverHandle::set_reconstructed_xref` へ委譲 — 既存の `set_header_offset` と同じ `&self` + 内部可変性シーム）経由で owner（resolver）へ即座に書き込むよう揃えた。従来は `LoadedXrefState::already_reconstructed` という戻り値フィールドへ一旦貯め、`engine.rs::open_with_repair_mode_as` がロード完了後にまとめて `resolver.set_reconstructed_xref(...)` へ転記していた——qpdf に無い「バッチ適用」ステップで、これが `docs/qpdf-route-matrix/b-parser-recovery-diagnostics.md` の B25 行を mixed 判定させていた。write-through 化により戻り値フィールドと転記コード、および冗長だった `engine.rs` 側の `header_offset` 再書き込み（`set_header_offset` は既に `xref.rs:850` で write-through 済みだった）を削除した。**分類 (B) の条件1 検証**: candidate-xref re-entry（`read_xref(max_offset)`, `xref.rs:3288`）経由で `resolver.rs::reconstruct_xref_and_retry` の二重再構築ガードが新たに armed される観測可能な経路が無いかを一時的な panic probe で検証済み——`cargo test -p flpdf --features qpdf-zlib-compat`（181件の compat fixture 統合テスト込み）+ `cargo test -p flpdf-cli --features qpdf-zlib-compat` の全件で、ガード自体を直接検証する既存 unit テスト 1 件以外に到達なし（no-op）。 | **2026-09-19 再訂正**: B25 は `mixed` のまま（canonical 化を撤回）。write-through 化そのもの（戻り値フィールドと転記ステップの削除）は有効だが、書き込み地点が qpdf と違う——qpdf は `reconstruct_xref` の入口で warn と scan の前に立てる（`QPDF.cc:518-524`）のに対しこちらは scan 成功後。入口へ移すと `cargo test -p flpdf --lib` が 9 件落ちる（flpdf の candidate-xref re-entry が実際に scan 中へ再入するため）。予算の適用範囲の差として `flpdf-4wenf` で追跡する。B26（`attempt_recovery`、同根の mirror）は `XrefLoadOptions::allow_repair` の参照箇所が15箇所超で規模が大きく、別途対応する |
| `xref.rs::XrefLoadOptions::allow_repair`（`flpdf-3yn9.48.176` の B26 write-through で削除） | qpdf の `m->attempt_recovery`（`QPDF.hh:1461`）は `parse()` エントリ（`QPDF.cc:463`）と resolve 時 retry（`QPDF.cc:1614-1637`）の両方が同じ `QPDF` インスタンス上の 1 bit を読むだけ。flpdf は同じ値を `xref.rs` の xref 読み込みロジック（`load_xref_state_from_source`/`load_xref_state_from_window`）が `XrefLoadOptions::allow_repair` という独自コピーとして別途受け取っており、`engine.rs::open_with_repair_mode_as` が resolver 構築時の `options.repair`（resolver 自身が保持する値）とは別に、この構造体へも `allow_repair: options.repair` として二重に構築していた。B25 と同じ `CanonicalTrailerOwner` の `&self` + 内部可変性 write-through seam（`set_header_offset` が既に確立）にならい `attempt_recovery(&self) -> bool` を追加し、`ResolverHandle::attempt_recovery`（既存の pub(crate) アクセサ）へ委譲。xref 読み込みの4箇所（`load_xref_state_from_source` 2箇所、`load_xref_state_from_window` のローカル変数初期化 1箇所、その派生1箇所）を owner から直接読むよう揃え、`XrefLoadOptions::allow_repair` フィールドと `engine.rs` 側の二重構築を削除した。同じ owner を経由する他の xref primitive（`merge_previous_xref_sections`/`parse_xref_stream_with_canonical_owner`/`recover_trailer_from_xref_stream_candidate` 等）は元々 owner 引数を持っていたため、`XrefLoadOptions` 経由の値を読んでいなかった箇所は変更不要——`allow_repair` フィールド自体の削除に伴うテスト側の構築リテラル修正のみで済んだ。 | B26 は canonical へ再分類（`docs/qpdf-route-matrix/b-parser-recovery-diagnostics.md` 参照）。B22 の open 側/resolve 側 2 実装分割はこの統合では解消しない（別軸） |

2026-09-23 update（`flpdf-4wenf`）: B25 の guard timing は現行実装で qpdf と一致した。`ResolverHandle::begin_xref_reconstruction` を open-time / resolve-time の共通 entry helper とし、open-time は warning と scan の前に呼ぶ。入口へ移した際の9 test failureは nested reconstruct の到達ではなく、早期 guard が同じ recovery scan 内の candidate table install まで stale snapshot と誤認したためだった。scan / candidate re-entry 中だけ table update scope を許し、最終 raw table を owner に同期した上で外側の stale snapshot を拒否する。indirect `/Type` candidate の live probe では nested reconstruct を観測しなかった。B22 は open / resolve の再構築処理が別実装という独立軸で mixed のまま。
...
The post-writer page-content pass follows the same handle boundary in flpdf: `job/check.rs::check_document_with_suppression` obtains raw page handles through the public `PageDocumentHelper::get_all_pages()` and constructs `PageObjectHelper::from_object_handle`, preserving `QpdfObjGen` identity for this qpdf-shaped loop. `get_all_pages()` mirrors `QPDFPageDocumentHelper::getAllPages()` and returns raw handles; `pages::page_refs()` remains the separate checked `ObjectRef` projection for callers that require valid `N G R` references (`flpdf-ihyup.9`; `QPDFPageDocumentHelper.cc:13-20`, `QPDFJob.cc:745-790`).
...
`pages::page_refs` now returns its valid prepared cache or obtains raw handles
from `PageDocumentHelper::get_all_pages` and applies the checked
`page_ref_from_handle` projection through the existing live-handle
