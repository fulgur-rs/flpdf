...
| `QPDF_Stream::registerStreamFilter` / `QPDF_Stream::filterable` | `QPDF_Stream.cc:33-50,72-94,148-152,378-485` | `stream_filter.rs` の `stream_filter_for` と `object_handle.rs` の `prepare_stream_filter_plan` | 🔀 qpdfのfilter factoryは`filter_factories`の`std::map`を名前引きに使い、`registerStreamFilter`でruntime登録できる。flpdfは既存の`match` registryを使い、runtime登録APIは親issue `.48.46` の後続scopeで扱う。`QPDF_Stream::filterable`のfactory lookup、`/DecodeParms` shape・長さ検証、`setDecodeParms`順序はcanonicalなObjectHandle readerで共有する。旧 `FilterSpec`/縮約`DecodeParams`/`ParamValue`/retention snapshot層とwhole-buffer recovering routeは `.48.96` までに撤去済み。 | 🔀 |
