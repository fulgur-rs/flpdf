...
| `QPDFJob_config` / `_argv` / `_json` / `QPDFArgParser` | 3164 | `flpdf-cli/src/arg_parser.rs` + `SegmentHandler` / `QpdfSegmentHandler` in `main.rs` + clap | ⚪。QPDFJobの使用エラー分類は [`UsageError`](../crates/flpdf/src/error.rs) + `Error::Usage` として job lifecycle から CLI の `usage_exit` へ伝播し、`QPDFUsage` の別catch経路（`qpdf/qpdf.cc:10-23,34-39`）を再現する。CLI の入口は `std::env::args_os()` とし、qpdf argv grammar の residual/segment tokens、`--pages`/`--overlay`/attachment の path、`QPDFJob` の input description を `OsString`/raw bytes のまま保持する。UTF-8 が必要な selector・range・日付などだけを各 option parser の境界で検証し、非UTF-8 argv を `std::env::args()` の unwrap で失わない。`flpdf-v1xw` では argv token を raw bytes と `OsString` 投影の二重キャリア `RawArg` で運び、clap の parse 後に `raw_option_value` / `apply_raw_overrides` が byte-oriented な値（password 系）を raw 側で上書きする。qpdf は argv を 1 度しか走査する（`QPDFArgParser.cc:433-569`）ため、`SegmentHandler` は各 named-segment token と終端 callback を同じ走査中に `QpdfSegmentHandler` へ届け、typed state を downstream consumer へ渡す。この (B) の入れ物の差を含め、受理するコマンドラインと出力は qpdf と同じことを `cli_arg_parsing_segments.rs` と qtest `arg-parsing` 25 ケースで検証する。**CLAUDE.md の逸脱分類 (C)**（`flpdf-g9uy9`）: `SegmentKind::retain_in_residual` は `--pages`/`--encrypt`/`--add-attachment`/`--copy-attachments-from`/`--set-page-labels` の raw segment token を無条件に `residual_args`（`--job-json-file` 経路の統一 argv パーサーが読む）へ再挿入するが、`--overlay`/`--underlay` だけは `has_job_json` で条件分岐する。qpdf の `QPDFArgParser` は単一パス・単一文法で `--job-json-file` も他の Config コールバックと同列（`QPDFJob_config.cc:774`）のため、この job-json/非job-json 分岐自体に qpdf 側の対応物は無い。分岐が必要な理由は qpdf 側にはなく flpdf-cli 固有の構造にある: 非job-json 経路の overlay/underlay 適用は `QpdfSegmentHandler` が収集する `overlay_specs` を経由し `residual_args` を見ないため、無条件保持しても出力には影響しない——ただし flpdf-cli 独自のサブコマンドルーティング（qpdf に対応物なし）が `--overlay`/`--underlay` を `rewrite` サブコマンド以外で拒否する検証（`--overlay/--underlay can only be used with rewrite output`、`main.rs`）を持ち、この検証は clap の parse 成功を前提にするため、`residual_args` に retain した生トークンを clap がサブコマンド未定義の flag として拒否してしまうと、この検証より先に clap 自身の汎用エラーで exit してしまう。他の5 segment kind にはこの種のサブコマンド制限が無いため無条件保持のままで問題が起きない。`crates/flpdf-cli/src/arg_parser.rs` の `retain_in_residual` 宣言直前に `// qpdf-deviation:` マーカーを置いた。 |
