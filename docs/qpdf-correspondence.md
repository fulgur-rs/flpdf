...
| `QPDFParser::parse` / `QPDF::readObject`（indirect handle生成とstream framingの境界） | `QPDFParser.cc:155-172`; `QPDF.cc:1331-1349` | `parser.rs::parse_qpdf_file_object_handle_with_diagnostics` はtokenize中にindirect `ObjectHandle`を生成する。初段classic trailerは `engine.rs` がparse前に構築したcanonical resolverへ直接生成し、xref stream/ObjStmの残bootstrapは `xref.rs::BootstrapHandleDocument` に限定する。stream tailのframingはcaller側で継続する。 | 🔀 classic trailerのdocument identityはcanonicalへ移行済み。xref stream/ObjStmのpre-`Pdf` bootstrap ownerとpost-open resolverの分離、およびrebind/replayの撤去は後続slice (`flpdf-3yn9.48.13`–`.15`) |
...
| `QPDF::resolve` / `QPDF::resolveObjectsInStream`（bootstrap cache completion） | `QPDF.cc:1700-1857` | `xref.rs::parse_xref_stream` は xref stream の file-object を `read_file_object_handle` で一度だけ parse し、`parser.rs` の ObjStm member と同じ bootstrap handle graphへ installする。初段classic trailerだけは `engine.rs` のcanonical resolverへ直接生成される。 | 🔀 `.2` で raw+handle の二重 parse と live parser の finished-tree conversionを除去。`.3` では post-open `ObjectCache::Resolved` と ObjStm reconciliationをhandle identityへ切り替えた。xref stream/ObjStm bootstrapのcanonical移行とLoadedXref handoffは後続sliceに残る |
