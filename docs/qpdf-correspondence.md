...
| `qdf_fix.rs` | 1,221 | qpdf の counterpart は `qpdf/fix-qdf.cc`（libqpdf 外の別バイナリ）。ordinary stream は `st_after_stream` が各 exact `%QDF: ignore_newline` 行を1 byteずつ減算し、直後の `N 0 obj` の次行を bare integer として置換する。`/Length M G R` は読まない（`fix-qdf.cc:87-90,240-264,294-303`）。flpdf は隣接する top-level `ObjectSpan` から同じ holder を選び、整数行を検証して置換する。object stream (`/Type /ObjStm`) / cross-reference stream (`/Type /XRef`) は別状態 `st_in_ostream_*` / `st_in_xref_stream_dict` で処理する（flpdf-9hc.43）。 |
