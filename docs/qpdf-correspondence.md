...
| `QPDFJob.cc` `createQPDF` / `handlePageSpecs` + `QPDFJob_config.cc` `jsonInput` / `updateFromJson` | `459-463,1695-1714; 305-309,328-332` | `flpdf-cli/src/main.rs` の `open_job_pdf`、`run_json_document`、`run_json_input_inspection`、`run_page_extraction_from_repeated_pdf` + `check.rs::check_pdf_with_limits` + `page_combine.rs::CombinedPlan::build_repeated` | ✅ `flpdf-3yn9.16`。`--json-input` は rootless `Pdf::create_from_json` を、`--update-from-json=...` は通常PDF/JSON作成後の直後に呼び、rewrite・JSON output・read-only inspection（`--check`/`--show-npages`/`--show-pages`）・rotate・page-tree選択を同じ canonical `Pdf<R>` consumerへ渡す。JSON主入力の `--pages` は一時PDFを経由せず、同じ文書の ObjectHandle/xref を `build_repeated` で計画化する。qpdf 11.9.0 の update-before-transform 順序と inspection の受理を `cli_json_input.rs` で固定する。 |
