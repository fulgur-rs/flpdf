...
| `xref.rs::LoadedXref::repair_diagnostics` / `XrefStreamFailure::diagnostics`（`flpdf-3yn9.48.177` で監査、コードマーカーなし） | qpdf の `m->warnings`（`QPDF.cc:487-494`）は push_back のみで `warn()` 呼び出し順に即時配送される単一 sink。flpdf は複数の xref candidate（/Prev chain・hybrid section・reconstruction fallback）を試すため、Rust の関数境界を跨いで分割生成される診断を `Diagnostics` 値として一旦保持し、`deliver_canonical_diagnostics` で drain する。**`LoadedXref::repair_diagnostics` については** container 代替（分類 (B)）: `merge_recovered_qpdf_state`（`xref.rs:2160`）が `accumulated`（先行）→`recovered`（後発）の順で結合し、`:1052` と `:1141` の `mem::take` は 1 まとまりを次段へ渡すだけで並べ替えない。**2026-09-19 訂正（レビュー指摘により実測し直した）**: 当初この行は根拠として `prepend_repair_diagnostics` と `XrefStreamFailure::diagnostics` も挙げ、「順序の入れ替えや内容破棄は無い」と書いていたが、3 点とも事実と違った——(1) `XrefStreamFailure::diagnostics`（`xref.rs:3310`）は**常に空**で、`XrefStreamFailure::new`（`:3315`）が `Diagnostics::default()` を入れるだけ、push する箇所が存在せず、2 つの consumer は空集合を回している（冗長な state であって診断を運ぶ container ではない）。(2) `prepend_repair_diagnostics`（`:2149`）は production では**一度も実体動作しない**——唯一の呼び出し元 `:1041` が渡す `initial_diagnostics` は `:840` で既に drain 済みで、`load_xref_state_from_window` の他の呼び出し元（`:852`・`:874`・`:920`）は fresh default を渡すため、常に空入力の early return を取る。(3) `deliver_canonical_diagnostics`（`:184`）は**先にバッファ全体を drain してから**push するので、owner の warning sink がバッチ途中で失敗すると以降の警告は失われる。この経路は caller が見る警告集合を変えるが、書き出しバイトは変えないので (B) の前提（出力バイト不変）自体は崩れない。(1) と (2) の冗長機構は `flpdf-po4te` で追跡する。よって CLAUDE.md 分類 (C) のコードマーカー（`// qpdf-deviation`）は付けない。 | `docs/qpdf-route-matrix/b-parser-recovery-diagnostics.md` の B27 行を対応させて更新済み。B29 の snapshot consumer（CLI lazy warning emission 等）は別issue範囲として着手せず — 単一 `ResolverCore::repair_diagnostics` collection への正当な public accessor consumer であり別 sink ではないため |
