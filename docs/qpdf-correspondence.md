...
| qpdf | 行 | flpdf | 状態 |
|---|---|---|---|
| `QPDFJob.cc` | 3116 | `flpdf-cli/src/main.rs`(6796) + `job/lifecycle.rs`（JSON create/update/write と共有 completion、`QPDFJob.cc:429-563,1708`）+ `job/json.rs`（`QPDFJob::writeJSON` の出力選択部分、`QPDFJob.cc:3094-3115`）+ **`json_inspect.rs` の `doJSON*`**（下記。`QPDF::writeJSON` 相当は `document_json.rs` に分離済み — §8）+ `overlay*.rs` + `page_merge.rs`(1117) + `check.rs`(360) + `attachment_list.rs`(1074: `QPDFJob.cc:876-911` の `doListAttachments` 移植。`<file> has no embedded files` は infilename を要するため CLI 側に残す) + `acroform_field_prune.rs`(497: `QPDFJob.cc:2610-2632` の "Remove unreferenced form fields"。`prune_acroform_after_subset` が CLI から呼ばれる) + page 操作群 | 🔀 `job/lifecycle.rs` は現在サポートするJSON job routeのcanonical create/update/write境界を移植済み。argv/config、その他のtransform consumerは後続sliceで集約する。 |
| `QPDFJob.cc` `createQPDF` / `handlePageSpecs` + `QPDFJob_config.cc` `jsonInput` / `updateFromJson` | `459-463,1695-1714; 305-309,328-332` | `job/lifecycle.rs` のJSON create/update（`flpdf-25kg.5.2.1`）+ `flpdf-cli/src/main.rs` の `open_job_pdf`、`run_json_input_inspection`、`run_page_extraction_from_repeated_pdf` + `check.rs::check_pdf_with_limits` + `page_combine.rs::CombinedPlan::build_repeated` | 🔀 `--json-input` / `--update-from-json` の JSON output route は `QPDFJob` の create/update/write に移行済み。その他のrewrite・read-only inspection（`--check`/`--show-npages`/`--show-pages`）・rotate・page-tree選択は `flpdf-3yn9.16` の既存canonical `Pdf<R>` consumerを使用し、後続Job sliceで同じ状態へ接続する。JSON主入力の `--pages` は一時PDFを経由せず、同じ文書の ObjectHandle/xref を `build_repeated` で計画化する。qpdf 11.9.0 の update-before-transform 順序と inspection の受理を `cli_json_input.rs` で固定する。 |
...
flpdfは `job/lifecycle.rs::QPDFJob::write_json` と、未移行のCLI経路では
