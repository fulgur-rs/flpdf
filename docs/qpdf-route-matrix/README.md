...
| [D. writer — reachability, ObjStm planning / renumber / emission, xref / trailer, encryption, linearize](d-writer.md) | 31 | 17 | 0 | 14 | 0 |
...
| **object cache に「削除済み」の永続 tombstone は存在しない** — `removeObject` は cache cell ごと erase し、`deleted_objects` は xref 構築が終われば clear される | `libqpdf/QPDF.cc:1995-2005` / `libqpdf/QPDF.cc:706-708` / `libqpdf/QPDF.cc:575` | A2 は canonical（**2026-09-19 訂正**: `flpdf-3yn9.48.22` で qpdf に無い 6 状態の `CacheEntry` facade は削除済み。A2 行自身も `canonical` であり、この記述は cutover 前の stale だった）。A17 の tombstone 手動clear分岐は `CacheEntry` 定義元の `crates/flpdf/src/cache.rs` ごと `flpdf-3yn9.48.22` で削除済みで、A17 は canonical に再分類した（`flpdf-3yn9.48.172`） | `get_all_objects`（A9）と `live_object_refs`（A10）の列挙が食い違い、writer の到達性集合が経路ごとに変わる。同じ入力で出力 object 数が route 依存になる |
...
6. **C44** — public `getStreamJSON` facade と deferred `StreamBlobProvider` 相当は `.48.47` で実装済み。C-U2 の provider 回数・lifetime probe は 2026-09-18（`.48.154`）に `probe154/c44_probe.sh` と flpdf 側テストで完了し、乖離なし。C44 は public facade と deferred blob の責務分離を追跡する mixed 履歴行のまま維持する。canonical C24 `write_stream_json` を二重pipeへ変更しない。
