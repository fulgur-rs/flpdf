...
**調査日:** 2026-09-15（再監査 `flpdf-3yn9.48`、文書更新 `flpdf-3yn9.48.108`）。
...
**今回の変更を適用した tree で** 1296 qpdf citations / 912 flpdf citations /
259 logical rows、分類は
canonical 141 / mixed 118 / bridge 0 / unknown 0。A〜E の160行だけを数える
...
classified row は 259 行で、canonical 141 / mixed 118 / bridge 0 / unknown 0（bridge + mixed は118行）である。
...
7. **C4 / C8 / C9 / C25〜C29、E-27 / E-28** — provider/copy/decodeの不足primitiveを明示し、xrefとdriver test 0/1など既知consumerからbounded cutoverする。C28 の test 0/1 cutoverとqpdf-less public hardening/module撤去は `.48.93` / `.48.96` で完了した。C43 の qpdf-less public wrapperとinternal label helperは `.48.91` / `.48.95` で撤去した。E-28のtest 2/6 caller-side resolve bridgeは `.48.107` / `.48.108` で撤去した。
...
7. **E-27 / E-28** — `.48.93` で test 0/1 の既知stream warning bridgeを canonical pipe/loggerへ、`.48.107` / `.48.108` で test 2 / 6 の caller-side resolve bridgeを canonical accessorへ移行済み。残る mixed case/APIだけを追加調査し、A〜D全行の確定を待たない。
