...
| [C. stream data provider / decode / retry / filter / encryption / `/Length`](c-stream-pipeline-encryption.md) | 42 | 29 | 2 | 11 | 0 |
...
| **teardown は `xref_table.clear()` → `obj_cache` 全件 disconnect の 1 本** | `libqpdf/QPDF.cc:215-236` / `libqpdf/QPDFObject.cc:13-17` | A20 mixed。canonical `Pdf::open` の walk は `.48.15.1` で `ResolverHandle::disconnect_all` へ一本化したが、owner-less public loaderには `BootstrapCache::Drop` の第2 walkが残る（`.48.72` scope） | owner-less bootstrap側だけが持つ handle がcanonical teardownの正本に入らない。canonical側のclear-before-disconnect契約は固定済みで、残る差はpublic owner-less APIの第2 document state。probe A-4 |

### 5.B parser / xref recovery / warning・error・diagnostics

| 不変条件 / 境界 | qpdf の根拠 | flpdf の現状（該当行） | 壊すと何が変わるか |
|---|---|---|---|
| **warning sink は `m->warnings` 1 本で、順序は `warn` 呼び出し順そのもの**（`push_back` 以外に並べ替え・重複除去・分類は無い） | `libqpdf/QPDF.cc:487-494` / `include/qpdf/QPDF.hh:1475` | B28 canonical。B27 は canonical `Pdf::open` では `.48.73` で xref/recoveryのfile/trailer/candidate diagnosticsも `ResolverHandle::push_qpdf_warning`へ呼出順に合流し、replay/install/deferred bridgeを撤去済み。owner-less public loaderは `BootstrapHandleState` → `XrefReadContext` → `LoadedXref` の残staging（`.48.72`） | canonical `qpdf --check` のwarning行はqpdfのcall orderで一度だけ配送される。owner-less loaderの第2 document stateとそのdiagnostic stagingは別sliceで撤去する。probe B-P4 |
...
| **classic xref の欠番/type≠1 は通常出力で `std::logic_error`。object 0 と pass-1 `suppress_offsets` は別分岐** | `libqpdf/QPDFWriter.cc:2343-2379` / `libqpdf/QPDFXRefEntry.cc:27-32` | D12 mixed。plain first consumerは `writer/plain/xref.rs::write_xref_table` へ移行済み。specialized/PCLm/linearizedのtable loopは後続consumerとして残る | shared primitiveはentry 0、type-1 offset、generation 0、range、suppress/hint補正を保持し、missing/free/type2は `Error::Internal`。正常writerのgap producer調査はD12と分離する（D-U3） |
