...
**調査日:** 2026-09-16（再監査 `flpdf-3yn9.48`、文書更新 `flpdf-3yn9.48.118`）。
現行 `origin/main=84739f617bb20102f92245bfb308579be5e3d33c` と pinned qpdf source に基づき、領域別表の分類・責務境界・既存issueとの対応を更新した。
...
rowsを検証する。2026-09-16 の現行 `origin/main` (`d3c1cba3e16416f65aa31de8c12adb8e05ae95b2`) を基準に、
...
2026-09-16 の current-main audit anchor は `origin/main=d3c1cba3e16416f65aa31de8c12adb8e05ae95b2`、pinned qpdf は
11.9.0 commit `3b97c9bd266b7c32ea36d3536e22dab77412886d` である。checker の実測は
**今回の変更を適用した tree で** 1408 qpdf citations / 912 flpdf citations /
259 logical rows、分類は
canonical 151 / mixed 108 / bridge 0 / unknown 0。A〜E の160行だけを数える
...
classified row は 259 行で、canonical 151 / mixed 108 / bridge 0 / unknown 0（bridge + mixed は108行）である。
...
7. **E-27 / E-28** — `.48.93` で test 0/1 の既知stream warning bridgeを canonical pipe/loggerへ、`.48.107` / `.48.108` / `.48.109` / `.48.110` / `.48.111` で test 2 / 3 / 6 / 11 / 19 のcaller-side resolve/accessor bridgeを、`.48.112` / `.48.113` / `.48.114` で test21/test17のshallowCopy・root/array accessor bridgeを、`.48.115` で test73の`resolve_once` bridgeを、`.48.116` で test87のdirect-only key enumeration bridgeを、`.48.117` で test97のarray-count bridgeを、`.48.118` で test92のdestroyed-handle accessor bridgeを、`.48.120` で test85のgetValueAs bridgeを canonical routeへ移行済み。残る mixed case/APIだけを追加調査し、A〜D全行の確定を待たない。
