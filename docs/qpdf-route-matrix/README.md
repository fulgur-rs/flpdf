...
| C-U1 | C | **Resolved by `flpdf-zvjf` and `flpdf-hj7v`**: C42 の recovered span と show-stream出力が qpdf と一致するか | pinned qpdf source の `QPDF::recoverStreamLength` / `QPDF::pipeStreamData` / `QPDFJob::doShowObj` を照合し、AESv2とunencrypted recovered-length fixtureで qpdf と flpdf の raw bytes・warning・exit status を比較した。qpdf は recovered `length` を変更せず pipe するため、flpdf の AES/RC4 subtraction・PCLm-only workaround・show-stream の inspection trim を除去した。`RecoveredStreamEol` は `dump-object` 再シリアライズの framing 専用 metadata として残る | **C42**（mixed、qpdf full-length pipe + dump-object-only metadata） |
