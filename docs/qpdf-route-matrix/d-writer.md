...
| D14 | `QPDFWriter::writeTrailer` / `getTrimmedTrailer` | `libqpdf/QPDFWriter.cc:1160-1236,2009-2032` | `crates/flpdf/src/writer.rs::build_writer_trailer_handle`（private）と `crates/flpdf/src/writer/object.rs::write_trailer_with_ref_map`（`crates/flpdf/src/writer/object.rs:952`） | `build_writer_trailer_handle` centralizes trailer trimming; route-specific live-handle writers and xref framing consume it through OutputSink | mixed | `crates/flpdf/src/writer.rs::build_writer_trailer_handle` plus route-specific sink writers | qpdf trimming/key exclusion is centralized in `build_writer_trailer_handle`. 2026-09-19（`flpdf-3yn9.48.169`）re-audit: null visibility is verified canonical, not a residual bridge — both consumers apply qpdf's dereferencing null check (`QPDF_Dictionary::getKeys()`'s `!isNull()` filter, `QPDF_Dictionary.cc:117-127`, `QPDFObjectHandle.cc:352-356`) before emitting a trailer key. Confirmed against qpdf 11.9.0 with an object-0 trailer reference (`/CustomKey 0 0 R`): dropped identically from the trailer in classic, `--qdf`, and `--object-streams=generate` (xref-stream) output by both qpdf and flpdf. The remaining mixed component is solely the split between `crates/flpdf/src/writer/object.rs::unparse_trailer_entries_with_ref_map_and_kind`（classic, consumed via `writer/plain/xref.rs`）and `crates/flpdf/src/writer/serialize.rs::write_live_trailer_entries`（xref-stream, consumed via the same route's `write_object_dict_prefix`）against qpdf's single `writeTrailer(which, size, xref_stream, prev, pass)`（`libqpdf/QPDFWriter.cc:1160-1236`）. This split is output-byte-neutral — the audit found no behavior divergence — but two owners genuinely persist, so the row stays mixed rather than reclassifying to canonical; a same-session merge of two independently byte-gate-tested serializers would be high-risk, so the scoped unification (boundary audit: ID/Encrypt placement, `/Size` computed-value condition, `/Root` null/removed exemption, QDF layout) is tracked separately as `flpdf-3yn9.48.183`。2026-09-17（`flpdf-6gmnc`）: trailer parser handles retain qpdf's `trailer at offset` description so writer-side copy-encryption ID accessor warnings keep their null-chain context; this is separate from D14's `/Size` key-set follow-up（`flpdf-5jcwv`）。 |
| D15 | `QPDFWriter::writeEncryptionDictionary`（body 後・xref 前、`std::map` の key 昇順） | `libqpdf/QPDFWriter.cc:2244-2256`、`writeStandard` からの呼び出し位置は `libqpdf/QPDFWriter.cc:3017-3019`（他の呼び出し元は `writeLinearized` の `libqpdf/QPDFWriter.cc:2795`） | `crates/flpdf/src/writer/encrypted_strings.rs::write_encryption_dictionary_handle`（`pub(crate)`、`crates/flpdf/src/writer/encrypted_strings.rs:313`） | prod: 2 (`crates/flpdf/src/writer.rs:5163` legacy coordinator, `crates/flpdf/src/linearization/writer.rs:2360`) / test: 0 | canonical | `crates/flpdf/src/writer/encrypted_strings.rs::write_encryption_dictionary_handle` | 出力位置も qpdf と一致: legacy coordinator は body 全 object の後・`let xref_offset = bytes.len();` の直前（`crates/flpdf/src/writer.rs:5155-5170`）、linearized は part4 の直後。**この行の「plain pipeline は暗号化経路を持たないので暗号化出力は legacy coordinator を通る」という旧記述は失効している**: 根拠にしていた `plain::eligible` 述語は `.48.164` で撤去され、非 linearized の暗号化出力は本ファイル後半の route 表どおり shared plain live route を通る。prod caller の位置と数の再測定は本行の保守項目として残る |
