...
| D25 | `QPDFWriter::prepareFileForWrite`（`/Extensions` / `/ADBE` の direct 化 + `fixDanglingReferences`） | `libqpdf/QPDFWriter.cc:2036-2056`、`libqpdf/QPDF.cc:1259-1269` | graph preparation: `crates/flpdf/src/writer.rs::prepare_file_for_write`（`:1628`）、共通callerは `PdfWriter::write`（`:777`）。ADBEの追加削除は `inject_adbe_extension`（`:1977`）/ `strip_adbe_extension`（`:2059`）に残る | prepare prod: 1 / output mutation prod: 2 / test: 0 | mixed | `crates/flpdf/src/writer.rs::prepare_file_for_write` | qpdf と同じく、flpdfも `write()` の標準・linearized分岐前に graph preparation を一度だけ実行する。`snapshot_catalog_extensions`（`:2181`）/`restore_catalog_extensions`（`:2204`）は現時点では出力専用ADBE mutationの後処理として残り、`flpdf-3yn9.48.60` でroot `unparseObject`責務へ移行する。 |
