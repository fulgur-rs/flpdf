...
| D6 | `QPDFWriter::preserveObjectStreams`（source container map ∩ compressible set） | `libqpdf/QPDFWriter.cc:1939-1967` | `crates/flpdf/src/writer/object_streams/planning.rs::plan_qpdf_preserve_object_streams_with_unreferenced` | prod: plain Preserve 全体（live route も `writer/plain/plan.rs::build_live_object_stream_plan` でこの planner を呼び、QDF/normalize を含む全 Preserve route で返った group を live queue へ登録する。linearized も同じ関数を呼ぶ） / test: empty-map ordering | mixed | plain Preserve planner | document `get_object_stream_data` を compressible walk より先に呼び、空なら解決せず戻る。specialized coordinator と linearized Preserve は `.48.54` で同じ source-backed membership owner に移行した。live queue emission への接続は `.48.65` 第1 cohort で完了しており、`write_plain_live_disable` がこの planner を直接呼び、`LiveQueue::register_object_streams` が返された source-backed group を container-first 採番の membership として受け取る。QDF/normalize の Disable と source ObjStm なし Preserve は `flpdf-ay5b` で同じ live bodyへ接続した。**2026-09-19（`flpdf-3yn9.48.166` 調査）**: QDF/normalize Generate と source ObjStm-bearing Preserve が同じ `LiveQueue` に接続されていることを実測確認した（`tests/fixtures/compat/*.pdf` 175件 × `{--qdf, --normalize-content=y}` × `{generate, preserve}` の700実行で698件がqpdf 11.9.0とbyte-identical、残り2件は無関係な QDF trailer 辞書インデント差 `flpdf-6isbu`）——D2/D3/D5/D29（**2026-09-19 訂正**: D9 と D11 は同じ版で `mixed` に差し戻したためリストから除外） はこれを根拠に canonical へ再分類した。本行の分類を mixed のまま維持するのは、この live-queue接続とは別の未検証事項（`D-U1`、`docs/qpdf-route-matrix/README.md:991`）のためである: linearized Preserve consumer（D31、`linearization/plan.rs`）もこの同じ関数を呼ぶが、qpdf の `writeLinearized` は Preserve メンバーを source-index 順で消費するのに対し、この共有関数が返す membership の member 順序が同じ順序で足りているかを比較する actual probe がまだ無い。この順序差の検証は D6/D31 の共通スコープであり、本 issue（QDF/normalize の非 linearized live-queue接続）の対象外。 |
...
| D9 | `QPDF::getObjectStreamData`（入力 xref の type 2 entry → source container） | `libqpdf/QPDF.cc:2381-2390`、`include/qpdf/QPDF.hh:757-761` | `crates/flpdf/src/reader.rs::get_object_stream_data` → `crates/flpdf/src/reader/resolver.rs::get_object_stream_data` | prod: document entry → plain Preserve（QDF/normalize を含む全 route）/ tests: mixed rows, prefilled map, live rows, lazy IO | mixed | document-owned type-2 mapping |**2026-09-19 訂正（`canonical` 化を撤回）**: type-2 entry → source container の責務に production 経路が 2 本ある——`job/page_merge.rs:258-282 install_primary_object_stream_membership` が `source_xref_entries()` の生 xref から `XrefEntry::Compressed` を自前でマッチして member→container を再構築しており、`get_object_stream_data` を通らない（`mod tests` は `:1730` からなので production）。責務境界を切り直すまで `mixed`。 **2026-09-19（`flpdf-3yn9.48.166`）再分類**: source xref 正本から objnumber → container number を caller map へ追記・上書きし、clear や object 解決をしない。plain Preserve の再filterを撤去済みで、`writer/plain/mod.rs::write_plain_live` が `source_object_stream_data` をこのマップから受け取り `build_live_object_stream_plan` → `plan_qpdf_preserve_object_streams_with_source_membership`（D6）経由で `LiveQueue`（D2）に登録する経路は QDF/normalize を含め分岐しない。`source_xref_entries` は reader 自身の責務として維持する。D2 の byte-identical 実測（698/700）がこの source membership 供給経路にもそのまま当てはまる。 |
...
| D11 | `QPDFWriter::writeObject` / `unparseObject`（body 1 オブジェクトの emission） | `libqpdf/QPDFWriter.cc:1761-1809,1318-1603` | `crates/flpdf/src/writer/write_object.rs::WriteObject` → plain Disable/Preserve/Generate（QDF/normalize を含む全 non-linearized route）を担う `LiveObjectEmitter` | shared prod: live consumer 1（全 non-linearized route共通）/ tests: writer emission + live queue | mixed | `WriteObject::write_object` + plain live `LiveObjectEmitter` |**2026-09-19 訂正（`canonical` 化を撤回）**: linearized は独立した emission 実装を持つ——`crates/flpdf/src/linearization/writer.rs:692 append_body_object_with_raw_identity` と `write_object_with_qpdf_obj_gen_map_and_removed`。`rg 'WriteObject' crates/flpdf/src/linearization/` は 0 件で、plain 側の emission primitive を共有していない。qpdf は `QPDFWriter::writeObject`/`unparseObject` を standard と linearized で**共有**する（`README.md:196` の不変条件も同じことを述べている）。1 つの qpdf 責務に production 実装が 2 本ある以上 §3 の `mixed` に該当する。 **2026-09-19（`flpdf-3yn9.48.166`）再分類**: 共通 owner が progress、QDF framing、set/clear data key、open/unparse/close を所有し、`LiveObjectEmitter` は plain Disable・Preserve（source ObjStm-bearing を含む）・Generate・PCLm・QDF/normalize いずれの bounded cohort でも live queue で child を発見する。`.48.87` で非暗号・非QDF・非normalize Generate の fresh ObjStm が同じ live queue へ接続した後、`210cc3e8b`・`651e6cb40`・`48cbfb0a7`・`.48.164`（`c15ccc9c6`）で QDF/normalize Generate と source ObjStm-bearing Preserve もこの emitter に収束し、`PlainObjectEmitter` に相当する別実装は現在ソースに存在しない（`rg -n PlainObjectEmitter crates/flpdf` はゼロ件）。D2 の byte-identical 実測（698/700）がこの emission primitive にもそのまま当てはまる。 |
...
| D14 | `QPDFWriter::writeTrailer` / `getTrimmedTrailer` | `libqpdf/QPDFWriter.cc:1160-1236,2009-2032` | `crates/flpdf/src/writer.rs::build_writer_trailer_handle`（private）と `crates/flpdf/src/writer/object.rs::write_trailer_with_ref_map`（`crates/flpdf/src/writer/object.rs:952`） | `build_writer_trailer_handle` centralizes trailer trimming; route-specific live-handle writers and xref framing consume it through OutputSink | mixed | `crates/flpdf/src/writer.rs::build_writer_trailer_handle` plus route-specific sink writers | qpdf trimming/key exclusion is centralized in `build_writer_trailer_handle`. 2026-09-19（`flpdf-3yn9.48.169`）re-audit: null visibility stays `mixed`（**2026-09-19 再訂正**: 当初「canonical と検証済み」と書いたが、下記のとおり実証範囲が狭く、API 構築の object-0 では乖離が残る）。three consumers apply qpdf's dereferencing null check (`QPDF_Dictionary::getKeys()`'s `!isNull()` filter, `QPDF_Dictionary.cc:117-127`, `QPDFObjectHandle.cc:352-356`) before emitting a trailer key. Confirmed against qpdf 11.9.0 with an object-0 trailer reference (`/CustomKey 0 0 R`): dropped identically from the trailer in classic, `--qdf`, and `--object-streams=generate` (xref-stream) output by both qpdf and flpdf. The parse-derived fixture above never reaches the `object_ref.number == 0` predicate: both parsers null out `0 0 R` at parse time (qpdf `QPDFParser.cc:168`, flpdf `parser.rs:872`), so every run terminates on the `try_is_null()` arm. A library caller that installs a non-null value at object 0 and references it from a custom trailer key does diverge — qpdf's `getKeys()` keeps the resolved value and `unparseChild` (`QPDFWriter.cc:1149-1154`) writes it, while flpdf discards the key on its raw object-zero identity (and the classic child path serializes it as null, pinned by `writer/object.rs:6108`). That case belongs to this row's trailer-key visibility contract, not to a separate question, and is in scope for the unification issue. The structural split is three-way, not two: `crates/flpdf/src/writer/object.rs::unparse_trailer_entries_with_ref_map_and_kind`（classic, consumed via `writer/plain/xref.rs`）and `crates/flpdf/src/writer/serialize.rs::write_live_trailer_entries`（xref-stream, consumed via the same route's `write_object_dict_prefix`）against qpdf's single `writeTrailer(which, size, xref_stream, prev, pass)`（`libqpdf/QPDFWriter.cc:1160-1236`）. This split is output-byte-neutral — the audit found no behavior divergence — but two owners genuinely persist, so the row stays mixed rather than reclassifying to canonical; a same-session merge of two independently byte-gate-tested serializers would be high-risk, so the scoped unification (boundary audit: ID/Encrypt placement, `/Size` computed-value condition, `/Root` null/removed exemption, QDF layout) is tracked separately as `flpdf-3yn9.48.183`（**scope に追加（2026-09-19）**: 3 本目の `canonical_linearization_trailer_entries` の統合と、API 構築の object-0 trailer key の挙動）。2026-09-17（`flpdf-6gmnc`）: trailer parser handles retain qpdf's `trailer at offset` description so writer-side copy-encryption ID accessor warnings keep their null-chain context; this is separate from D14's `/Size` key-set follow-up（`flpdf-5jcwv`）。 |
| D15 | `QPDFWriter::writeEncryptionDictionary`（body 後・xref 前、`std::map` の key 昇順） | `libqpdf/QPDFWriter.cc:2244-2256`、`writeStandard` からの呼び出し位置は `libqpdf/QPDFWriter.cc:3017-3019`（他の呼び出し元は `writeLinearized` の `libqpdf/QPDFWriter.cc:2795`） | `crates/flpdf/src/writer/encrypted_strings.rs::write_encryption_dictionary_handle`（`pub(crate)`、`crates/flpdf/src/writer/encrypted_strings.rs:313`） | prod: 2（**2026-09-19 訂正**: 直接の `/Encrypt` emission は `crates/flpdf/src/writer/plain/mod.rs:207` と `crates/flpdf/src/linearization/writer.rs:2759`。旧記載の `writer.rs:5163` は `#[cfg(test)]` の unit test 内、`linearization/writer.rs:2360` は `do_write_pass` の `pdf` 引数で、いずれも emission の caller ではない） / test: 0 | canonical | `crates/flpdf/src/writer/encrypted_strings.rs::write_encryption_dictionary_handle` | 出力位置も qpdf と一致: legacy coordinator は body 全 object の後・`let xref_offset = bytes.len();` の直前（`crates/flpdf/src/writer.rs:5155-5170`）、linearized は part4 の直後。**この行の「plain pipeline は暗号化経路を持たないので暗号化出力は legacy coordinator を通る」という旧記述は失効している**: 根拠にしていた `plain::eligible` 述語は `.48.164` で撤去され、非 linearized の暗号化出力は本ファイル後半の route 表どおり shared plain live route を通る。prod caller の位置と数の再測定は本行の保守項目として残る |
| D16 | `QPDFWriter::setEncryptionParametersInternal` / `setEncryptionParameters` / `copyEncryptionParameters` | `libqpdf/QPDFWriter.cc:777-840,591-648,651-702` | `crates/flpdf/src/writer.rs::WriterSetupState`（`pub(crate)`、`crates/flpdf/src/writer.rs:2735`）→ `EncryptionParameters::into_context` | prod: shared setup 1 (`crates/flpdf/src/writer.rs`), route consumers 2 (`crates/flpdf/src/writer.rs`, `crates/flpdf/src/linearization/writer.rs`) / test: 1 | mixed | `crates/flpdf/src/writer.rs::build_writer_setup` | qpdf は 3 つの setter が `setEncryptionParametersInternal` 1 本へ収束し、以後は `m->encryption_dictionary` という単一 state。flpdf も qpdf 形状の option 正規化後に `WriterSetupState` が `EncryptionParameters` を一度だけ構築し、standard / linearized は各 route の `/Encrypt` slot だけを割り当てて `EncryptionParameters::into_context` から同じ辞書・file key・ID0・metadata state を使う。`EncryptionContext` は route-specific slot を含む出力 consumer state であり、setter の再構築 owner ではない。`/Encrypt` の最小版数決定（R6→1.7 ext8 等）は `crates/flpdf/src/writer.rs::encryption_version_floor` に分離されている。2026-09-17（`flpdf-67h8d`）に、copy path の qpdf `getIntValueAsInt` warning/zero fallbackを `Pdf::writer_copy_encryption_source` の pre-detach snapshot と canonical builder の `/Length 0`/zero-length V<5 keyへ接続した。reader の 128-bit malformed-length fallbackとは分離している。2026-09-18（`flpdf-3yn9.48.159` probe）: qpdf 11.9.0 をoracleに `--copy-encryption`/preserve-encryption/`--password-is-hex-key` を実測し、canonical builder（`crates/flpdf/src/writer.rs:2747-2800`）の鍵長・handler検証がqpdfに対応物のない独自述語であることを確認——qpdfは`/Length`由来の長さ（`key_len = /Length / 8`、`QPDFWriter.cc:662`）で鍵を再導出するだけで長さの整合検査をしない。**ただし V>=5 は例外**で、`copyEncryptionParameters` は`encryption_key = qpdf.getEncryptionKey()`（`QPDFWriter.cc:680-686`）として認証済みの鍵をそのまま `setEncryptionParametersInternal` へ渡す。したがって`d-v5-len128` が乖離するのは「qpdf が 128-bit 鍵を導出するから」ではなく、**qpdf が不整合な `/Length` を受理しつつ 256-bit 鍵を保持するから**である。follow-up で V5 の挙動を誤って実装しないこと。malformed `/Length`（4ケース）とhex-key経路（4ケース）でbyte/exit code差が実測された。CLI側のdonor-open失敗包装（`main.rs:5418-5508`）も独自文言でmessage framingが乖離。cutoverの前提としてこれらのparity修正が必要（follow-up issue化予定）。2026-09-18（`flpdf-3yn9.48.161`）に、canonical builder の鍵長・handler検証（qpdfに対応物のない独自述語）とCLI donor境界の `supported` matrix を撤去し、qpdf の `setEncryptionParametersInternal` と同じく V>=5 は認証済み鍵を保持、V<5 は `CopyEncryptionSource::padded_user_password` から `min(16, /Length / 8)` バイトで鍵を再導出する形に揃えた（`QPDFWriter.cc:832-839`）。malformed `/Length`（V2R3 032 / V2R3 240 / V5R6 128）と `--password-is-hex-key`（4 長）が `--copy-encryption` / preserve-encryption の両経路でqpdf 11.9.0とbyte一致し、`flpdf-cli/tests/cmp_copy_encryption_key_derivation_tests.rs` が固定する。分類は **mixed のまま** — V=4 donor の `/Length 040` だけは 14 バイトのper-object AES keyを生み、qpdfのcrypto providerがそのバッファを16バイト読むover-read（`QPDFCrypto_gnutls.cc:197-213`）に依存するため、flpdfは既存の `qpdf-deviation` 方針どおり捏造せず拒否する（`docs/qpdf-correspondence.md` の `QPDF_encryption.cc` 行を参照）。CLI donor-open失敗のmessage framing乖離は 2026-09-18（`flpdf-3yn9.48.162`）に解消した——qpdf は donor を primary input と同じ `QPDFJob::processFile`（donor: `libqpdf/QPDFJob.cc:2891-2899` / primary: `libqpdf/QPDFJob.cc:434`）で開き、失敗は `realmain` の単一 catch（`qpdf/qpdf.cc:36-40`）が `whoami: e.what()` として出すため、`--copy-encryption:` のようなオプション固有の接頭辞は付かない。flpdf-cli の donor 境界もprimary input と同じ `open_error_with_file` / `error_with_file` / `actionable_password_error` を通し、donor 失敗を CLI の単一 fatal-error renderer へ回すよう揃えた。missing-donor / wrong-pw / no-pw / donor-is-dir / donor-not-pdf の stderr が qpdf 11.9.0 と byte 一致し、`flpdf-cli/tests/encrypt_cli_tests.rs` が固定する。分類は **mixed のまま**（上記 V=4 `/Length 040` のover-read 拒否は未変更） |
...
**D14 re-audit (2026-09-19, `flpdf-3yn9.48.169`):** the issue's premise —
...
trailer key is emitted. A targeted fixture with a trailer key set to an
...
and risky to do safely in the same session as this audit; that merge is
tracked as `flpdf-3yn9.48.183`.
