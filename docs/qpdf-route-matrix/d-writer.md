...
| D5 | `QPDFWriter::assignCompressedObjectNumbers`（container enqueue 時に member 範囲を即時予約） | `libqpdf/QPDFWriter.cc:1057-1069` | `crates/flpdf/src/writer/plain/body.rs::LiveQueue::register_object_streams`（emission-time、全 non-linearized route共通）。planned consumer は `crates/flpdf/src/writer/object_streams/planning.rs::ObjectStreamGroup` を `ObjectStreamRenumber::build_with_stream_policy` に渡す | prod: 19 (`crates/flpdf/src/writer/plain/plan.rs`=10, `crates/flpdf/src/writer/rewrite_renumber.rs`=6, `crates/flpdf/src/writer/object_streams/planning.rs:100,244`, `crates/flpdf/src/writer.rs:4005`) / test: 0 | canonical | `LiveQueue::register_object_streams`（plain の live consumer、全 non-linearized route共通） | **2026-09-19（`flpdf-3yn9.48.166`）再分類**: Preserve は `SourceBacked { source, members }`、Generate は qpdf の `makeIndirectObject(newNull())` に対応する `Generated { source, members }` を作り、両方の source identity を `LiveQueue::register_object_streams`（D2）へ渡す。2026-09-13（`flpdf-3yn9.48.87`）で非暗号・非QDF・非normalize の Generate、続く同日の `210cc3e8b`/`651e6cb40`/`48cbfb0a7` と `.48.164`（`c15ccc9c6`）で QDF/normalize Generate と source ObjStm-bearing Preserve もこの登録経路へ収束した。生成した null placeholder は出力後に writer-owned allocation として除去し、再書き込みの preserve-unreferenced seed に混入させない。`crates/flpdf/src/writer/rewrite_renumber.rs::ObjectStreamRenumber` の Synthetic group は linearized/`#[cfg(test)]` の `PlainWritePlan` 専用として残る。D2 の byte-identical 実測（698/700）がこの割当経路にもそのまま当てはまる。 |
| D6 | `QPDFWriter::preserveObjectStreams`（source container map ∩ compressible set） | `libqpdf/QPDFWriter.cc:1939-1967` | `crates/flpdf/src/writer/object_streams/planning.rs::plan_qpdf_preserve_object_streams_with_unreferenced` | prod: plain Preserve 全体（live route も `writer/plain/mod.rs::build_live_object_stream_plan` でこの planner を呼び、QDF/normalize を含む全 Preserve route で返った group を live queue へ登録する。linearized も同じ関数を呼ぶ） / test: empty-map ordering | mixed | plain Preserve planner | document `get_object_stream_data` を compressible walk より先に呼び、空なら解決せず戻る。specialized coordinator と linearized Preserve は `.48.54` で同じ source-backed membership owner に移行した。live queue emission への接続は `.48.65` 第1 cohort で完了しており、`write_plain_live_disable` がこの planner を直接呼び、`LiveQueue::register_object_streams` が返された source-backed group を container-first 採番の membership として受け取る。QDF/normalize の Disable と source ObjStm なし Preserve は `flpdf-ay5b` で同じ live bodyへ接続した。**2026-09-19（`flpdf-3yn9.48.166` 調査）**: QDF/normalize Generate と source ObjStm-bearing Preserve が同じ `LiveQueue` に接続されていることを実測確認した（`tests/fixtures/compat/*.pdf` 175件 × `{--qdf, --normalize-content=y}` × `{generate, preserve}` の700実行で698件がqpdf 11.9.0とbyte-identical、残り2件は無関係な QDF trailer 辞書インデント差 `flpdf-6isbu`）——D2/D3/D5/D9/D11/D29 はこれを根拠に canonical へ再分類した。本行の分類を mixed のまま維持するのは、この live-queue接続とは別の未検証事項（`D-U1`、`docs/qpdf-route-matrix/README.md:991`）のためである: linearized Preserve consumer（D31、`linearization/plan.rs`）もこの同じ関数を呼ぶが、qpdf の `writeLinearized` は Preserve メンバーを source-index 順で消費するのに対し、この共有関数が返す membership の member 順序が同じ順序で足りているかを比較する actual probe がまだ無い。この順序差の検証は D6/D31 の共通スコープであり、本 issue（QDF/normalize の非 linearized live-queue接続）の対象外。 |
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 22 | D2, D3, D4, D5, D7, D9, D10, D11, D12, D13, D15, D17, D18, D19, D20, D21, D22, D24, D25, D27, D29, D30 |
| mixed | 9 | D1, D6, D8, D14, D16, D23, D26, D28, D31 |
| bridge | 0 | — |
| unknown | 0 | — |
