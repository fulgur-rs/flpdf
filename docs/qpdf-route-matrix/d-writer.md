...
| D11 | `QPDFWriter::writeObject` / `unparseObject`（body 1 オブジェクトの emission） | `libqpdf/QPDFWriter.cc:1761-1809,1318-1603` | `crates/flpdf/src/writer/plain/body.rs::emit_bodies`（`pub(crate)`、`crates/flpdf/src/writer/plain/body.rs:18`） | prod: 1 (`crates/flpdf/src/writer/plain/mod.rs:35`) / test: 0 | mixed | 未完成（`writer/object.rs` に共有 emission 部品あり） | plain の `emit_bodies`、specialized の `writer.rs:4604`、PCLm の `writer.rs:3451`、linearized の `do_write_pass`（`linearization/writer.rs:2079`）に処理が分かれる。qpdf も `writeStandard` と `writeLinearized` の制御ループは別なので、4 ループを機械的に 1 本にすることは目標ではない。復元する境界は、standard/PCLm が共有する enqueue 中の body loop（`QPDFWriter.cc:2991-3044`）と、両出力方式が呼ぶ `writeObject` / `unparseObject` の単一 owner。plain Disable など 1 consumer ずつ移行する |
| D12 | `QPDFWriter::writeXRefTable`（classic xref table） | `libqpdf/QPDFWriter.cc:2343-2379,2335-2340` | `crates/flpdf/src/writer/plain/xref.rs::append_xref_and_trailer`（`pub(crate)`、`crates/flpdf/src/writer/plain/xref.rs:75`）の `XrefForm::Table` アーム | prod: 2 (`crates/flpdf/src/writer/plain/mod.rs:36`, `crates/flpdf/src/writer.rs:5433`) / test: 2 (writer/plain/plan.rs 内 test) | mixed | `crates/flpdf/src/writer/plain/xref.rs::append_xref_and_trailer` | classic xref は plain（`plain/xref.rs:254`）、specialized（`writer.rs:5321`）、PCLm（`writer.rs:3521`）、linearized first/main（`linearization/writer.rs:795,1006`）の 4 系統。qpdf の `writeXRefTable` は object 0 だけを `0000000000 65535 f ` とし、他は `m->xref[i].getOffset()` を使う（`QPDFWriter.cc:2363-2373`）。欠番は既定 type 0 となり、`QPDFXRefEntry.cc:27-32` が `std::logic_error` を投げる。Rust の plain は欠番を `0000000000 00000 f `（`:260`）、他は `65535 f`（`writer.rs:3539,5327`、`linearization/writer.rs:1029`）として出力するが、どちらも維持すべき oracle 契約ではない。忠実な共有 primitive は通常の欠番/type≠1 を `Error::Internal` とし、`suppress_offsets` の pass-1 専用動作を別分岐に保つ。正常入力から欠番が生じるかの producer 調査（U3）を、この primitive 移植の前提にはしない |
...
| D25 | `prepareFileForWrite` の graph direct 化と `unparseObject` の出力用 ADBE 処理 | `libqpdf/QPDFWriter.cc:2036-2056,1350-1435`、`libqpdf/QPDF.cc:1259-1269` | `writer.rs::inject_adbe_extension`（`:1921`）/ `strip_adbe_extension`（`:2003`）、`snapshot_catalog_extensions`（`:2125`）/ `restore_catalog_extensions`（`:2148`） | production consumer: non-linearized `emit_canonical_pdf_inner` と linearized writer | mixed | 未分離（graph preparation と root emission の 2 owner が必要） | qpdf の `prepareFileForWrite` は fixDanglingReferences と `/Extensions` / `/ADBE` の direct 化を一度行う。ADBE の追加・削除は別責務で、`unparseObject` が unsafe shallow copy した出力辞書を扱う。flpdf は両者を事前 mutation に畳み、non-linearized の `emit_canonical_pdf`（`:3389`）と linearized の snapshot/restore で補正する。移行は graph preparation と root emission を別 slice にし、各 consumer の移行後に復元機構を cleanup する。closed `flpdf-25kg.6.15` の旧復元契約を恒久的な設計制約にはしない |
...
| U2（source で解決）: decode-only と normalized set | `normalized_streams` を読むのは `QPDFWriter.cc:1279` の `normalize_content && normalized_streams.count(old_og)`。decode-only ではこの集合を参照しない。page/content seq map は QDF コメントで使う（`:1774-1781`） | decode-only に normalized set が無いことだけを出力バグとして扱わない。ただし `initializeSpecialStreams` の page 修復トリガ（`:2113-2115`）と順序は維持し、3map の owner 分散は D26 で扱う |
