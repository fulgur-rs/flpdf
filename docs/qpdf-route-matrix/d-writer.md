...
| D24 | `QPDFWriter::enqueueObjectsPCLm` + `writeStandard`（pclm は xref/trailer を standard と共有） | `libqpdf/QPDFWriter.cc:2928-2954,2991-3044` | `crates/flpdf/src/writer/pclm.rs::Plan` と `crates/flpdf/src/writer.rs::write_pclm` | prod: 1 / test: 0 | mixed | 未完了（plain Disable の `LiveQueue` / `WriteObject` は PCLmへ未接続） | qpdf の pclm は enqueue seed だけが差分で body loop・xref・trailerを standard と共有する。flpdf の PCLm独立body/xref/trailerは `.48.65` などの後続 consumerで移行する。 |
