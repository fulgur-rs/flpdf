...
| B7 | `QPDF::readToken`（`allow_bad = true` 固定） | `libqpdf/QPDF.cc:1535-1539,1801-1814,846-946` | `crates/flpdf/src/tokenizer.rs::Tokenizer::read_qpdf_token`（`pub(crate)`） | `read_qpdf_token` prod: 5（xref.rs 3、reader/resolver.rs 1、tokenizer.rs 1）・test: 0。5 経路すべてがこの 1 entrypoint 経由 | canonical | `crates/flpdf/src/tokenizer.rs::Tokenizer::read_qpdf_token` | **2026-09-08（`flpdf-3yn9.48.14`）**: ObjStm headerの両consumerを専用 `next_object_stream_integer`（内部 `read_token(true, 0)` → integer検査）へ移行した。汎用 `next_integer` と classic xref の `ByteCursor::read_token(false)` は変更していない。**2026-09-18（`flpdf-3yn9.48.156`、probe → parity 修正）**: `ByteCursor::read_token` を `allow_bad = true` へ揃え、その旧値の前提だった classic xref subsection loop の独自形（`skip_ws` + `peek_word(b"trailer")` による loop 先頭での先読み・break）を qpdf の `while (!done)` ループ（header 読み → entry 読み → 末尾で `readToken(...).isWord("trailer")`、不一致なら rewind、`libqpdf/QPDF.cc:851-890`）へ置き換えた。qpdf 11.9.0 と `--check` を直接比較する probe で **観測可能な乖離 4 件を実測**: (1) 最終 entry と `trailer` の間の `%comment` — qpdf は tokenizer が comment を ignorable として読み飛ばし正常 parse（warning 0 / exit 0）、flpdf は `skip_ws` が comment を飛ばさず `xref syntax invalid` → reconstruction 3 連 warning + exit 3。(2) subsection を 1 つも持たない `xref\ntrailer` — qpdf は loop が必ず subsection header 読みで始まるため `xref syntax invalid` → reconstruction、flpdf は空テーブルとして受理し `unable to find /Root dictionary` で hard error（回復せず）。(3) `startxref` の直後が bad token（`)`）— qpdf は `findStartxref` が `isInteger()` false で marker を拒否し `can't find startxref`（`libqpdf/QPDF.cc:412-421`）、flpdf は `allow_bad = false` の `?` が tokenizer 文言 `(offset 319): unexpected )` を warning に出していた。(4) lookahead 前の `skip_ws` が rewind 位置をずらし、`xref syntax invalid` の offset が qpdf 276 / flpdf 279 とずれていた。issue が literal に問うた「subsection loop 位置の bad token（`)`）」自体は、旧実装では `peek_word` gate により `read_token` へ到達しないため乖離無し（実測一致）で、同じ契約違反が別の到達経路に出ていた。6 fixture（clean / bad lookahead token / `%comment` / 空白後 bad token / subsection 無し / bad `startxref` token）を qpdf 11.9.0 の warning 列と直接照合する回帰ゲートを `crates/flpdf/tests/canonical_xref_owner_route_tests.rs` の `classic_xref_read_token_lookahead_matches_qpdf` に追加した。3 entrypoint を 1 本へ畳む cutover 自体は未着手のため分類は `mixed` のまま。 **2026-09-19（`flpdf-3yn9.48.175`）**: 3 entrypoint を `Tokenizer::read_qpdf_token`（`QPDF::readToken(input, max_len = 0)` の直訳。`allow_bad = true` を固定し `max_len` を透過する）へ統合した。issue が挙げた 3 経路（`read_trailer` のトレーラ `stream` lookahead・`next_object_stream_integer`・`ByteCursor::read_token`）に加え、監査で見つかった 同一契約の独立実装 2 本——`read_scan_token`（`reconstruct_xref` の行スキャン、`MAX_LEN = 100`、`libqpdf/QPDF.cc:548,553,558,559`）と `reader/resolver.rs::read_token_from_input`（`readObject`/`readStream` の `endobj`/`endstream` framing check、`libqpdf/QPDF.cc:1347-1354,1386-1389`）——も 同じ `QPDF::readToken` 実現だったため、5 経路すべてを 1 本化した（`content_stream.rs`/`pipeline/qpdf_tokenizer.rs` の `EI` inline-image lookahead は `QPDFTokenizer` 自身の lookahead で別 qpdf owner のため対象外のまま）。`allow_bad`/`allow_eof`/`max_len` の 3 スロットは個別に mutation で検証した：`allow_bad = false` は `classic_xref_read_token_lookahead_matches_qpdf` を、`allow_eof` 除去は `offset_read_preserves_qpdf_caller_description_in_stream_warnings` を、`max_len` 無視は新設の `reconstruction_skips_a_candidate_whose_generation_token_exceeds_max_len_like_qpdf`（`crates/flpdf/tests/canonical_xref_owner_route_tests.rs`、qpdf 11.9.0 と `--show-xref` を直接照合）をそれぞれ reddens させることを確認済み。単一 entrypoint 化は `crates/flpdf/tests/qpdf_route_hygiene_tests.rs::qpdf_read_token_calls_route_through_one_allow_bad_entrypoint` が構造的に固定する。3 entrypoint を 1 本へ畳む cutover が完了したため分類を `canonical` へ更新した。 |
| B8 | `QPDF::readObjectAtOffset`（`N G obj` 検査・offset 0・回復 1 回・cache 登録） | `libqpdf/QPDF.cc:1541-1697` | `crates/flpdf/src/reader/resolver.rs::read_object_at_offset_with_description`（private） | prod: 5 (resolver.rs) / test: 8 | canonical | `crates/flpdf/src/reader/resolver.rs::read_object_at_offset_with_description` | 2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: xref bootstrap 側の第 2 実装（`xref.rs::read_uncompressed_object` と `reader/file_object.rs`）を撤去した。qpdf は `read_xrefStream` からも `resolve` からも同じ `readObjectAtOffset` を呼ぶ（`libqpdf/QPDF.cc:956`, `:1725`）ので、xref 読み出しと resolve が 1 本の実装を共有するこの形が qpdf と一致する。 |
| B9 | `QPDF::readObject`（`endobj` framing・`empty object treated as null`・`expected endobj` warn） | `libqpdf/QPDF.cc:1330-1357` | `crates/flpdf/src/reader/resolver.rs::read_object_at_offset_with_description`（private。framing と 2 warning をこの中に持つ） | prod: 5 (resolver.rs) / test: 8 | canonical | `crates/flpdf/src/reader/resolver.rs::read_object_at_offset_with_description` | qpdf 側も `readObjectAtOffset` が `readObject` を呼ぶ 1 本道（`libqpdf/QPDF.cc:1614`）で、warn の文言生成は `crates/flpdf/src/reader/resolver.rs::push_expected_endobj_warning_qpdf_obj_gen` が `m->last_object_description` の描画（`:1298-1310`）まで写している。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: `reader/file_object.rs`（`parse_file_object_handle_syntax` / `finish_file_object_handle`）は bootstrap 専用の第 2 実装だったため、そのモジュールごと撤去した。 |
| B10 | `QPDF::readStream` + `QPDF::validateStreamLineEnd`（`/Length` 検査・`endstream` 検査・行末 3 種の warn） | `libqpdf/QPDF.cc:1360-1399,1401-1449` | `crates/flpdf/src/reader/resolver.rs::validate_stream_line_end`（private） | `validate_stream_line_end(` prod: 1 (resolver.rs) / test: 0 | canonical | `crates/flpdf/src/reader/resolver.rs::validate_stream_line_end` | qpdf の 3 warning 文言（`stream keyword followed by carriage return only` / `not followed by proper line terminator` / `followed by extraneous whitespace`）は 2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去） より前は resolver 側と `reader/file_object.rs` 側の 2 実装にまたがっていた。bootstrap 側の撤去で、qpdf と同じく 1 関数が持つ。 |
| B11 | `QPDF::recoverStreamLength`（`attempting to recover stream length` → `endstream`/`endobj` 探索 → 2 種の結果 warn） | `libqpdf/QPDF.cc:1469-1479,1481-1533` | `crates/flpdf/src/reader/resolver.rs::recover_stream_length`（private。実呼び出しは `recover_stream_length_qpdf_obj_gen` 経由） | `recover_stream_length_qpdf_obj_gen(` prod: 2 (resolver.rs) / test: 1 | canonical | `crates/flpdf/src/reader/resolver.rs::recover_stream_length` | 2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: 第 2 実装 `reader/file_object.rs::recover_stream_boundary` と、qpdf の「`attempt_recovery` の有無」1 bit（`libqpdf/QPDF.cc:1391`）を `RequireEndstream` / `Bounded` の 2 値に置き換えていた named deviation `RecoveryPolicy` をモジュールごと撤去した。分岐は qpdf と同じ 1 bit に戻っている。 |
| B12 | `QPDF::resolveObjectsInStream`（ObjStm 展開・`/N` `/First` 検査・`has wrong type` warn・xref 一致メンバーのみ cache） | `libqpdf/QPDF.cc:1755-1833`, `libqpdf/QPDF.cc:1451-1467` | `crates/flpdf/src/reader/resolver.rs::resolve_object_stream_with_failure_kind`（private） | prod: 1 (resolver.rs) / test: 1 | canonical | `crates/flpdf/src/reader/resolver.rs::resolve_object_stream_with_failure_kind` | qpdf は `m->resolved_object_streams` を 1 つ持つ（`include/qpdf/QPDF.hh:1485`）。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: 並行する第 2 の document 状態（`BootstrapHandleState` の `resolving` / `resolved_object_streams` / `diagnostics` / `handles`）と、その上の `resolve_objects_in_stream` を撤去した。**bootstrap 側にあった 4 件の未マーク逸脱（`as_stream_dict()` 失敗・`/N` `/First` 非整数・メンバ parse error で `Err` を伝播、既定 xref 行の warning 欠落。qpdf はいずれも warn 側、`libqpdf/QPDF.cc:1760-1795,2617-2644`）は、この撤去でカバレッジを失うのではなく挙動ごと解消した**。撤去前に `flpdf-3yn9.48.151.2` で 1:1 等価監査を行い、canonical 側に無かった 3 挙動（重複キー warning の raw bytes・回復可能な decode warning 後のメンバ保持・メンバ内 direct 値への description 継承）は先に canonical 側の test として追加してある。 |
| B13 | `QPDF::readTrailer`（trailer 位置の復元・`empty object` warn・`stream keyword found in trailer` warn） | `libqpdf/QPDF.cc:1312-1328` | `crates/flpdf/src/xref.rs::read_trailer`（classic と reconstruct candidate の共通 private helper） | prod: 3 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::read_trailer` | 2026-09-08: classic owner有り／無しの両経路と `parse_trailer_candidate` が同じ handle-producing parser、parser diagnostics、empty warning、dictionary後の `read_token(true, 0)` lookaheadを通る。`trailer_warning` が qpdf と同じ object attributionを付け、`trailer_start` が復元された logical last-offsetとして `/Size`・`/Prev` validationへ渡る。live probeで `stream keyword found in trailer` と empty candidateの warning detail/offsetをqpdf 11.9.0と照合した。2026-09-17（`flpdf-7yb9k`）: canonical ownerの`CanonicalTrailerParser`も`QPDFParser`と同じ共有`filename, trailer at offset $PO` templateをparserへ渡し、nested direct valueのtoken offsetを保持する。`/ID 7`（offset 393）のtype/accessor warning、exit 3、output bytesをqpdf 11.9.0と比較した。owner-less bootstrapの既存templateとtop-level setterは別consumerとして維持する。2026-09-17（`flpdf-o5pr0`）: readObject後の成功streamはendobj後のsource last-offsetを保持し、payload開始へ戻さない。self-referential `/Length` recoveryとvalid indirect-length regressionで、warning/statusとsource offsetの責務を固定する。 |
...
| B15 | `QPDF::read_xrefTable` の subsection / entry 解析（`parse_xrefFirst` / `parse_xrefEntry`、`'f'` の後回し） | `libqpdf/QPDF.cc:722,770,846-893,929-932` | `crates/flpdf/src/xref.rs::parse_xref_table`（private） | prod: 1 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::parse_xref_table` | 呼び出し元は `crates/flpdf/src/xref.rs:1574` の 1 箇所のみ。`'f'` の後回し（`deferred_free`, `crates/flpdf/src/xref.rs:1580-1588,1692-1694`）も qpdf の `deleted_items` 順序（`libqpdf/QPDF.cc:849,880-885,929-932`）と一致。2026-09-18（`flpdf-3yn9.48.156`）: この関数の subsection ループ形（`while (!done)` と末尾の `trailer` lookahead）は B7 の `readToken` 契約側で qpdf へ合わせ直した——B7 行参照 |
...
| B18 | hybrid: classic trailer の `/XRefStm` を読み、**戻り値を捨てて** trailer の `/Prev` を使う | `libqpdf/QPDF.cc:915-927` | `crates/flpdf/src/xref.rs::merge_xref_stream_from_classic_trailer_with_build_diagnostics`（private） | prod: 1 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::merge_xref_stream_from_classic_trailer_with_build_diagnostics` | 呼び出し元は `parse_xref_from_start_with_owner_and_build_diagnostics` の 1 箇所のみ。`invalid /XRefStm` の文言も実在。`ignore_xref_streams` 相当は `crates/flpdf/src/xref.rs::XrefLoadOptions` が持つ。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: `#[cfg(test)]` の薄い wrapper `merge_xref_stream_from_classic_trailer` を撤去し、entrypoint 名を実体に合わせた。 |
| B19 | `QPDF::insertXrefEntry` / `QPDF::insertFreeXrefEntry`（first-seen wins と object-number 単位の tombstone） | `libqpdf/QPDF.cc:1149-1184,1187-1192` | `crates/flpdf/src/xref.rs::XrefRegistration`（private struct。`insert_xref_entry` / `insert_free_xref_entry`） | `insert_xref_entry(` prod: 2 (xref.rs) / test: 19。`insert_free_xref_entry(` prod: 2 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::XrefRegistration` | `crates/flpdf/src/reader/resolver.rs:3316` にも同名 `insert_xref_entry` があるが、`:3315` の `#[cfg(test)]` 配下の fixture 専用で production 経路ではない — 上の規約どおり test 側に数えている（test 19 のうち 1 件）。`crates/flpdf/src/xref.rs:188-196` の doc が `deleted_objects` の lifetime（`libqpdf/QPDF.cc:686-708` vs `:575`）まで書き分けている |
| B20 | `QPDF::insertReconstructedXrefEntry`（reconstruct 専用の後勝ち上書き + `obj > 0 && 0 <= gen < 65535` guard + `deleted_objects` 抑止） | `libqpdf/QPDF.cc:1197-1210` | `crates/flpdf/src/xref.rs::insert_reconstructed_xref_entry`（private。後勝ち上書き + `deleted_objects` 抑止） + `crates/flpdf/src/xref.rs::scan_object_header_after_first_token`（private。guard） | `insert_reconstructed_xref_entry(` prod: 2 (xref.rs) / test: 3 (xref.rs)。`scan_object_header_after_first_token(` prod: 2 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::insert_reconstructed_xref_entry` | 2026-09-19（`flpdf-3yn9.48.176`）: mixed だった理由（qpdf の 1 関数が guard / 後勝ち / 抑止の 3 箇所に分裂）を解消した。新設した `insert_reconstructed_xref_entry` が後勝ち上書き + `deleted_objects` 抑止を 1 箇所に束ね、resolve 側 `recover_xref_entries` と open 側 `recover_xref_entries_from_source` の両方がこれを呼ぶ。open 側は呼び出し元 `recover_xref_from_linear_scan` が渡す実 `deleted_objects` をスキャン中に直接 consult する形に変えた——旧実装は scan 完了後の `.retain()` で事後フィルタしていた（値は同じ。`deleted_objects` はスキャン中に変化しないため事前/事後フィルタは等価。2026-09-18 `flpdf-3yn9.48.157` の P2 fixture で実測した byte 一致は維持——回帰テスト `crates/flpdf/tests/canonical_xref_owner_route_tests.rs::reconstruction_after_a_committed_free_row_suppresses_it_like_qpdf` で確認済み）が、呼び出し箇所を qpdf の `insertReconstructedXrefEntry` 呼び出し自体（`QPDF.cc:557-562`）に揃えた。resolve 側 `recover_xref_entries` は明示的な空集合を渡す（省略ではなく不変条件として明示）——qpdf の `m->deleted_objects` は `readObjectAtOffset` の retry（`QPDF.cc:1614-1637`）到達時点で必ず空（通常登録終了時に `QPDF.cc:686-708` で clear 済みで、この retry 自身は新たな xref 登録を行わない）ため。guard（`crates/flpdf/src/xref.rs:3046-3049`）は `scan_object_header_after_first_token` に残した——qpdf はトークン分解後に `insertReconstructedXrefEntry` を無条件で呼び、guard 自体は callee 側（`QPDF.cc:1199-1202`）にあるのに対し、flpdf は `ObjectRef`（`number: u32`, `generation: u16`）が guard に落ちた値を表現できないため呼び出し側で先に弾く——挙動は同一だが guard の置き場所は qpdf と逆転している唯一の残差で、これは記録に留め、この構成のまま canonical とする |
| B21 | `read_xref` 後段: `/Size` 不一致 warn と「同一 object 番号は最高 generation のみ残す」 | `libqpdf/QPDF.cc:689-704,708,710-718` | `crates/flpdf/src/xref.rs::append_xref_size_warning_for`（private） + `crates/flpdf/src/xref.rs::discard_lower_generations`（private） | `append_xref_size_warning_for(` prod: 3 (xref.rs) / test: 0。`discard_lower_generations(` prod: 2 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::append_xref_size_warning_for` | どちらも実装は 1 本。`append_xref_size_warning_for` は 3 つの完了地点（`crates/flpdf/src/xref.rs:2214,2229,3736`）から呼ぶ。`discard_lower_generations` は qpdf の `read_xref` 後段専用（`libqpdf/QPDF.cc:710-718`）なので、通常の `read_xref` チェーン完了（`crates/flpdf/src/xref.rs:2263`）と、`reconstruct_xref` が内部で `read_xref(max_offset)` を呼び直す候補 xref stream 再入（`:3288`、`libqpdf/QPDF.cc:576-607`）の 2 箇所のみ。reconstruction の line scan がそのまま return する経路では呼ばない（`flpdf-k4tn`）。`is not one plus the highest object number` の文言も一致。`crates/flpdf/src/xref.rs:223-233` の doc が `QPDF::removeObject`（`libqpdf/QPDF.cc:710-718`）との対応を記録 |
| B22 | `QPDF::reconstruct_xref`（再入禁止 → 3 連 warn → type 1 削除 → 行スキャン → 候補 xref stream からの trailer 復元 → 失敗時 throw） | `libqpdf/QPDF.cc:516-623` | `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan`（private、open 時） / `crates/flpdf/src/reader/resolver.rs::reconstruct_xref_and_retry`（private、resolve 時） | `recover_xref_from_linear_scan(` prod: 5 (xref.rs) / test: 0。`reconstruct_xref_and_retry(` prod: 1 (resolver.rs) / test: 2 | mixed | `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan` | qpdf は 1 実装を 2 箇所（`libqpdf/QPDF.cc:463` parse 直下、`:1617` `readObjectAtOffset` の再試行）から呼ぶ。flpdf は呼び出し箇所ごとに別実装で、resolve 側は qpdf の後半（候補 `/XRef` stream からの trailer 復元 `libqpdf/QPDF.cc:577-608` と `unable to find trailer dictionary while recovering damaged file` の throw `:610-616`）を実装しない。canonical open 側の warning channel は `.48.73` で `ResolverHandle::push_qpdf_warning` へ統合した。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: open 側の owner-less 第 2 実装と bounded read window を撤去し、行スキャンは live source 上の `recover_xref_entries_from_source` 1 本になった。**mixed が残る理由は open 側と resolve 側で実装が 2 本ある点**——このスキャン本体の共有自体は B23 が別途扱う。B20（guard/後勝ち/抑止の分裂）は 2026-09-19（`flpdf-3yn9.48.176`）に別途解消し canonical へ移した（B20 行参照）が、B22 自身の mixed 判定はこの 2 本立てそのものに対するもので、B20 の解消では変わらない。resolve 側が実装しない後半（候補 `/XRef` stream からの trailer 復元・throw）についての 2026-09-08（`flpdf-3yn9.48.19` close note）時点の暫定判断: resolve 側の呼び出し元（`readObjectAtOffset` の retry、`QPDF.cc:1614-1637`）に到達する時点で qpdf の文書モデル上 `m->trailer` は必ず初期化済み（trailer 未確定のまま QPDF が使える状態にはならない）ため、qpdf 自身がこの呼び出し経路では 577-616 を実行しない、という**ソース読解のみに基づく**判断で、実 fixture による到達不能性の実証はまだ行っていない（過去に同種の「到達経路なし」という暫定判断が P2 で覆った前例があるため、確定扱いにしない——この判断が誤りなら、resolve 側の実装差はこの後半部分についても真の gap になる）。`flpdf-3yn9.48.19` は 2026-09-08 に CLOSED 済みで、当時 P3（compressed retry の分岐統一）は解消したが、この後半差と P2 は別途 `docs/qpdf-route-matrix` 上の記述としてのみ残った——次に着手するセッションは、まずこの到達不能性を実 fixture で確認してから統合可否を判断すること |
...
| B25 | `m->reconstructed_xref`（回復予算 = `bool` 1 個。2 回目は例外を re-throw） | `include/qpdf/QPDF.hh:1480`, `libqpdf/QPDF.cc:518-522,524-526` | `crates/flpdf/src/reader/resolver.rs::reconstructed_xref`（`pub(crate)` getter。実体は `ResolverCore` の core フィールド） / `crates/flpdf/src/xref.rs::CanonicalTrailerOwner::set_reconstructed_xref`（`ResolverHandle::set_reconstructed_xref` へ委譲する write-through seam） | getter `reconstructed_xref(` prod: 4 (reader.rs 1, resolver.rs 3) / test: 25 (resolver.rs 22, xref.rs 3)。setter `set_reconstructed_xref(` prod: 1 (xref.rs、`recover_xref_from_linear_scan` の成功地点) / test: 0 | mixed | `crates/flpdf/src/reader/resolver.rs::reconstructed_xref` | 2026-09-19（`flpdf-3yn9.48.176`）: open 時の回復は従来 `Pdf` 構築前に起きるため `LoadedXrefState::already_reconstructed` という戻り値フィールドへ一旦貯め、`engine.rs::open_with_repair_mode_as` がロード完了後にまとめて `resolver.set_reconstructed_xref(...)` へ転記していた。qpdf の `reconstruct_xref`（`QPDF.cc:518-522`）は既存の `QPDF` インスタンスへ成功した瞬間に直接書くだけで、この「バッチ適用」ステップを持たない。`CanonicalTrailerOwner` は `set_header_offset` 等で既に `&self` + 内部可変性の write-through seam を確立していたため（Rust の所有権制約ではなく、単に `set_reconstructed_xref` だけがこのパターンに揃っていなかった）、同じ seam でトレイトへ `set_reconstructed_xref` を追加し、xref reconstruction が成功する `recover_xref_from_linear_scan` の地点で owner へ直接書き込むよう揃えた。`LoadedXrefState::already_reconstructed` フィールドと `engine.rs` 側の転記コード、および冗長だった `header_offset` の二重書き込み（`set_header_offset` は既に `xref.rs:850` で write-through 済みだった）を削除。write-through 化が `resolver.rs::reconstruct_xref_and_retry` の二重再構築ガードへ新たな到達経路（candidate-xref re-entry、`xref.rs:3288`）を作らないことは一時的な panic probe で検証済み（全 unit test + 181 件の compat fixture 統合テストでガード自体を直接検証する既存テスト 1 件以外に到達なし）。B22 とは別軸——B22 の open 側/resolve 側 2 実装分割はこの write-through では解消しない。B26（`attempt_recovery`、同じ document-lifecycle 由来のミラー）は `XrefLoadOptions::allow_repair` の参照箇所が 15 箇所超で規模が大きく、別途対応する **2026-09-19 再訂正（`canonical` 化を撤回）**: 書き込み地点は qpdf と同じではない。qpdf は再入ガードを**入口で**立てる——`reconstruct_xref` が `m->reconstructed_xref` を検査し、立っていなければ即座に `true` を代入してから warn と scan に入る（`QPDF.cc:518-524`）——のに対し、この write-through は scan・trailer 復元・candidate-xref re-entry が全て終わった成功地点にある。結果として (a) scan 中に回復へ再入するとフラグは `false` に見え、(b) scan が途中で失敗した場合はフラグが立たないまま残る。**実測**: 呼び出しを関数入口（診断 push の前）へ移すと `cargo test -p flpdf --lib` が 9 件失敗する（`candidate_xref_stream_wrong_size_warning_survives_later_decode_failure` ほか）。つまり flpdf の candidate re-entry は **scan 中に回復へ再入しており**、qpdf の予算セマンティクス（scan 自体も予算に含む）と実際に異なる。1 責務に対する回復予算の適用範囲が qpdf と揃うまで `mixed` を維持し、`flpdf-4wenf` で追跡する。 |
| B26 | `m->attempt_recovery`（回復分岐の on/off。分岐点は全ソースで 3 箇所） | `include/qpdf/QPDF.hh:1461`, `libqpdf/QPDF.cc:336,463,1391,1563` | `crates/flpdf/src/reader/resolver.rs::attempt_recovery`（`pub(crate)`、`ResolverCore` の core フィールド） / `crates/flpdf/src/xref.rs::CanonicalTrailerOwner::attempt_recovery`（`ResolverHandle::attempt_recovery` へ委譲する write-through seam） | getter `.attempt_recovery(` prod: 6 (resolver.rs 2, xref.rs 3, engine.rs 1) / test: 2 (xref.rs 1, engine.rs 1) | canonical | `crates/flpdf/src/reader/resolver.rs::attempt_recovery` | 2026-09-20（`flpdf-3yn9.48.176`）: B25 と同じ write-through seam（`CanonicalTrailerOwner` の `&self` + 内部可変性）でトレイトへ `attempt_recovery` を追加し、`xref.rs` の xref 読み込みロジック（`load_xref_state_from_source`/`load_xref_state_from_window` 内の `allow_repair` 分岐、計4箇所）が owner から直接読むよう揃えた。`XrefLoadOptions::allow_repair` フィールドと `engine.rs` 側の二重構築（resolver 構築時の `options.repair` とは別に `XrefLoadOptions { allow_repair: options.repair, ... }` を組んでいた箇所）を削除。qpdf の `m->attempt_recovery` は `parse()` エントリ（`QPDF.cc:463`）と resolve 時 retry（`QPDF.cc:1614-1637`）の両方が同じ `QPDF` インスタンス上の 1 bit を読むだけで、flpdf も同じ `ResolverCore::attempt_recovery` を owner 経由で読むだけになった。B22 の open 側/resolve 側 2 実装分割はこの統合では解消しない（別軸） |
| B27 | `m->warnings`（**唯一の warning sink**。順序 = `warn` 呼び出し順、`push_back` のみ） | `include/qpdf/QPDF.hh:1475`, `libqpdf/QPDF.cc:487-494` | `crates/flpdf/src/reader/resolver.rs::ResolverCore` の `repair_diagnostics` フィールド。canonical `Pdf::open` は `.48.73` で xref/recovery diagnostics を `ResolverHandle::push_qpdf_warning` へ呼出順に配送し、engine の replay/install bridge を撤去した | 公開ドア `.repair_diagnostics()` — prod: 20 (flpdf-cli/main.rs, flpdf-qtest-tools 5 ファイル, engine.rs, job/check.rs, job/inspection.rs, job/lifecycle.rs, json/document.rs, reader.rs) / test: 162 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverCore` | `.48.12` の最初の consumer は classic trailer の document/cache identity。`.48.15.1` で canonical `Pdf::open` の bootstrap cache 保持、trailer/xref-stream rebind、第二 teardown を撤去し、`.48.73` で warning replay/deferred channel を撤去した。`.48.72` で owner-less public route を、2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去） で残っていた test-only staging（`BootstrapHandleState` / `XrefReadContext`）を機構ごと撤去したため、**diagnostics を保持する document state は `ResolverCore` 1 つになった**。mixed が残るのは xref loader が終端エラーに添える `LoadedXref::repair_diagnostics` の一時バッファを数えるため。この `LoadedXref::repair_diagnostics` は、Rust の関数境界を跨いで分割生成される診断群を `warn()` 呼び出し順のまま `deliver_canonical_diagnostics` で owner へ drain するための容れ物で、`merge_recovered_qpdf_state`（`crates/flpdf/src/xref.rs:2157`）と `mem::take`（`crates/flpdf/src/xref.rs:1049,1138`）は 1 まとまりを次段へ渡すだけで並べ替えない（CLAUDE.md 分類 (B) — 詳細は `docs/qpdf-correspondence.md` の `flpdf-77kv` 表）。**2026-09-19 訂正**: 旧記載は根拠に `prepend_repair_diagnostics` と `XrefStreamFailure::diagnostics` も挙げ「破棄は無い」と書いていたが、実測すると前者は production で常に空入力の early return（`:1038` が渡す値は `:837` で drain 済み）、後者は 常に空（`:3313` が `Diagnostics::default()` を入れるだけで push 箇所なし）で、`deliver_canonical_diagnostics`（`:181`）が先に全体を drain していたため sink がバッチ途中で失敗すると以降が失われていた点は、qpdf の 1 件ずつ append と異なる挙動差なのでこの PR で未配送分を戻すよう修正した。**2026-09-20 解消（`flpdf-po4te`）**: 冗長だった 2 機構を probe（`debug_assert!`/`assert!` を一時的に挿入し、workspace test 全体 + qpdf qtest fixture 589 件 + `tests/fixtures/compat/*.pdf` 178 件を `--check` と `rewrite --qdf --object-streams=generate` の両方で掃引し、一度も発火しないことを実測）で確認したうえで削除した——`prepend_repair_diagnostics` 関数とその呼び出し、`XrefStreamFailure::diagnostics` フィールドと 2 箇所の forwarding ループを撤去。残るのは `LoadedXref::repair_diagnostics` 1 つ |
...
| B32 | 例外分類: `QPDFExc`（`std::runtime_error` 派生 + `qpdf_error_code_e`）と `std::logic_error` / `std::runtime_error` | `include/qpdf/QPDFExc.hh:29-77`, `libqpdf/QPDF.cc:481,1231`, `libqpdf/QPDFParser.cc:163`, `libqpdf/QPDFTokenizer.cc:241,248,770`, `libqpdf/QPDFLogger.cc:200,252`, `libqpdf/QPDF.cc:1101`（`std::range_error`） | `crates/flpdf/src/error.rs::Error`（`pub` enum。`Io` / `FileIo` / `Parse` / `QpdfExc` / `Unsupported` / `Usage` / `Missing` / `Pages` / `Encrypted` / `Internal` / `System` / `SystemBytes` / `OpenFailure`。旧記載は `QpdfExc` / `SystemBytes` の 2 variant が抜けていた stale な列挙） | `Error::Internal(` prod: 166 / test: 91（計 257 = 全出現）、`Error::System(` prod: 103 / test: 138（計 241。全出現は 242 で、差の 1 件は `crates/flpdf-qtest-tools/src/driver/test_18_25.rs:171` のコメント行のため上の規約で除外）、`Error::parse(` prod: 161 / test: 10（計 171 = 全出現） | mixed | `crates/flpdf/src/error.rs::Error` | 型は 1 つに集約されているが、**qpdf の例外クラス軸（`QPDFExc` / `std::logic_error` / `std::runtime_error` / `std::range_error`）を Rust の variant へ置き換えている**（**2026-09-19 訂正**: 旧記述の「2 軸→1 軸の畳み込み」は誤り。`qpdf_error_code_e` 軸は 畳まれておらず、`Error::QpdfExc` が包む `QpdfExc` が `error_code: QpdfErrorCode`（`crates/flpdf/src/error.rs:116`）を保持し `get_error_code`（`:148`）で公開する）。`Internal` ↔ `std::logic_error` / `System`（および byte 保存版の `SystemBytes`、`crates/flpdf/src/error.rs:246-247,321`）↔ `std::runtime_error` は `crates/flpdf/src/error.rs:223-224`（旧記載の `:42-43` は関数移動により stale。現在その行は `usage_what` ヘルパー）の doc が明記する 1:1 対応。一方 `Parse` は qpdf の `QPDFExc(qpdf_e_damaged_pdf, ...)` に相当するが `QPDFExc` の他の error code（`qpdf_e_object` / `qpdf_e_pages` 等）は `Pages` など別 variant に分かれ、`QPDFExc` という 1 つのクラスが flpdf では複数 variant に散る。`Missing` / `Unsupported` / `OpenFailure` は qpdf に対応する例外クラス・error code が無い。`crates/flpdf/src/reader/resolver.rs:2062-2079`（`reconstruct_xref_and_retry`。旧記載の `:1615` は関数移動により stale）は `Error::Parse` / `Error::QpdfExc` を reconstruct の trigger として許可し、qpdf の `catch (QPDFExc&)`（`libqpdf/QPDF.cc:1614`）と対応づけているので、この振り分けは回復分岐の正しさに直接効く。**この例外クラス軸の置換自体が CLAUDE.md 逸脱分類 (B) に該当することの記録**（条件 3 の 2 箇所必須）は、module doc 側を `crates/flpdf/src/error.rs` の `//!` ヘッダ（**2026-09-19 訂正**: 当初 `pub enum Error` 直前の item doc に置いたが、CLAUDE.md 条件 3 が要求するのは module doc なので移動した）に、`docs/qpdf-correspondence.md` 側を `docs/qpdf-correspondence.md:680-712`（見出し「⚪ **(B) `Error` carries qpdf's exception-class axis as variants**」。行番号がずれた場合はこの見出し文字列で検索する）にそれぞれ記載した。この記録は上記の per-code variant 振り分けが qpdf と厳密に 1:1 かという `mixed` の残課題そのものを解消するものではない。なお `Error::Internal(`/`Error::System(`/`Error::parse(` の呼び出し件数（prod/test 内訳）は今回未再集計で、生 grep 総数（それぞれ 363/269/133 件）は表内の数値と大きく異なる——旧集計が使った「コメント除外・test 判定」規約を今回は再適用していないため、この 3 つの数値の staleness 有無は未確認のまま残る |
| B33 | `reconstruct_xref` の 3 連 warn（`file is damaged` → 引数 `e` → `Attempting to reconstruct cross-reference table`、順序固定） | `libqpdf/QPDF.cc:528-530` | `crates/flpdf/src/xref.rs::push_repair_diagnostics`（private、open 時） / `crates/flpdf/src/reader/resolver.rs:2176-2197`（resolve 時のインライン 3 連 `push_warning`） | `push_repair_diagnostics(` prod: 2 (xref.rs) / test: 2。resolver 側インラインは 1 箇所 | mixed | `crates/flpdf/src/xref.rs::push_repair_diagnostics` | B22 の帰結。2 実装があり、中央の「引数 `e` をそのまま warn する」部分の文言生成が別。`push_repair_diagnostics`（`crates/flpdf/src/xref.rs:2652-2747`）は trigger error のメッセージごとに 5 通りの分岐で `QPDFExc` 相当の文言を再構成するのに対し、resolver 側は `"(object N G, offset X): message"` を固定書式で組む。qpdf はどちらも `warn(e)` の 1 行で、文言は `e` が作られた時点の `createWhat` が決めている（B30）。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去） は open 側の 2 実装化とは無関係で、この行の状態を変えない |
...
| B35 | `QPDF::isLinearized`（先頭1024バイトのcandidateスキャン → 候補オブジェクトの `/Linearized` 検証 → 整数/実数の許容と `floor()==1` 判定 → `/L` が整数ならファイルサイズと厳密一致を要求） | `libqpdf/QPDF_linearization.cc:83-155` | `crates/flpdf/src/linearization/check.rs::Pdf::is_linearized`（`pub`） | `is_linearized(` prod: 5 (4 ファイル: `linearization/check.rs` 1、`linearization/show.rs` 1、`job/check.rs` 2、`flpdf-qtest-tools/src/bin/qpdf_ctest.rs` 1) / test: 8。数値は `python3 scripts/qpdf-route-callers.py --root . --symbol is_linearized` の出力 | canonical | `crates/flpdf/src/linearization/check.rs::Pdf::is_linearized` | 候補スキャンの primitive は `crates/flpdf/src/reader/resolver.rs::linearization_candidate`（`crates/flpdf/src/reader.rs::linearization_candidate_ref` 経由）へ委譲し、`/L` のファイルサイズ照合は `resolver::source_length` に委譲する。qpdf の `isLinearized` 全体（候補 dict の resolve 失敗を `false` に丸める扱いを含む）を 1:1 で写した唯一の production 実装で、`crates/flpdf/src/linearization/show.rs:591` の同名関数は `#[cfg(test)]` 限定の孤立した検証用 helper（production では未使用）のため canonical 判定に影響しない。2026-09-20（`flpdf-ycb6l`）: `scripts/check-qpdf-route-matrix.py` は「qpdf 側の責務に対応する行の欠落」を検出しない（記載済み citation の実在と集計表の整合性しか見ない）ため、この行自体がこれまで未計上だった。 **B7 とは別経路**: この候補スキャンは `LiveTokenSource::next_token`（`crates/flpdf/src/parser.rs:295`）を回し、その中で `Tokenizer::present_character` と `Tokenizer::get_token` を直接駆動する——B7 が canonical owner とする `Tokenizer::read_qpdf_token`（`crates/flpdf/src/tokenizer.rs:1035`）は通らない。B7 の「`QPDF::readToken` の実体は 1 entrypoint」という主張はこの push-mode 経路を含まないので、B35 をその契約下と書くのは誤り。**qpdf 側は `readToken` を使う**: 先頭 1024 バイトを生バッファで読んで数字位置を探した後、その位置から `readToken(m->file)` を 4 回呼んで `N G obj <<` を確認する（`QPDF_linearization.cc:120-123`）。つまり qpdf ではこの走査も `readToken` 契約の下にあり、flpdf 側だけが push-mode の別経路を使っている。B7 を「1 entrypoint」と保つならこの経路を `read_qpdf_token` へ寄せる必要があり、その判断は `flpdf-z3imy` で追跡する。 |
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 29 | B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15, B16, B17, B18, B19, B20, B21, B23, B24, B26, B28, B30, B31, B34, B35 |
| mixed | 6 | B22, B25, B27, B29, B32, B33 |
| bridge | 0 | — |
| unknown | 0 | — |

合計 35 行。2026-09-20（`flpdf-ycb6l`）: `QPDF::isLinearized`
（`libqpdf/QPDF_linearization.cc:83-155`）に対応する行が存在しなかったため
B35 として追加した。owner は `crates/flpdf/src/linearization/check.rs::Pdf::is_linearized`
で canonical。合計を 34→35、canonical を 28→29 に更新。
2026-09-19（`flpdf-3yn9.48.176`）: B20 の guard / 後勝ち / 抑止が
`insert_reconstructed_xref_entry` 1 関数へ統合され、mixed から canonical へ移った
（B20 行参照）。mixed を 8→7、canonical を 26→27 に更新。
2026-09-06の再確認でB29はdocumentのwarning collectionをownerと確定し、
...
2026-09-19（`flpdf-3yn9.48.175`）: B7 の 3 entrypoint（`read_trailer`
の `stream` lookahead・`next_object_stream_integer`・`ByteCursor::read_token`）と、
監査で見つかった同一契約の独立実装 2 本（`read_scan_token`・
`reader/resolver.rs::read_token_from_input`）を `Tokenizer::read_qpdf_token` へ統合し、
mixed から canonical へ更新した（B7 行参照）。mixed を 9→8、canonical を 25→26 に更新。

## unknown / probe

### 2026-09-08 canonical warning-sink cutover
...
| P2 | B20 | **解消（2026-09-18、`flpdf-3yn9.48.157`）**: 2026-09-08 の「到達経路が見当たらない」という暫定判断は誤りで、実際に乖離する fixture を作れた。まず issue 本文が求めた「classic xref テーブルが free entry を登録した後にそのセクションの parse が失敗する」形は **qpdf でも flpdf でも構成できない**: `QPDF::read_xrefTable` は `'f'` 行を `deleted_items` に溜めるだけで `/XRefStm` 読取後にしか `insertFreeXrefEntry` へ渡さず（`libqpdf/QPDF.cc:880-885,928-931`）、flpdf の `deferred_free`（`crates/flpdf/src/xref.rs:1336-1343,1450-1452`）も同じ順序なので、セクション内で失敗すれば free 行は両実装とも破棄される。tombstone が reconstruction まで生き残るのは **xref stream の inline な free 行**（`QPDF::processXRefStream` は payload 走査中にその場で `insertFreeXrefEntry` を呼ぶ、`libqpdf/QPDF.cc:1122-1127`）だけ。そこで classic table + `/XRefStm` の hybrid fixture（`/XRefStm` の payload が object 4 の free 行の直後に unknown type 3 行を持つ）を作り、qpdf 11.9.0 `qpdf --warning-exit-0 --show-xref` と突き合わせた。**実測乖離**: qpdf は `1/0 2/0 3/0 5/0 6/0` を出し object 4 を抑止するのに対し、修正前の flpdf は `4/0: uncompressed; offset = 186` を含めていた（warning 3 行と他の offset は完全一致）。原因は初段 parse 失敗 handoff が `deleted_objects` を渡さないことで、抑止を `recover_xref_from_linear_scan` の行スキャン直後へ移し 5 handoff すべてに適用して解消した。回帰は `crates/flpdf/tests/canonical_xref_owner_route_tests.rs::reconstruction_after_a_committed_free_row_suppresses_it_like_qpdf` が fixture ごと固定し、flpdf の xref table を qpdf の `--show-xref` 出力と直接比較する（抑止行を削ると FAILED になることを確認済み）。2026-09-19（`flpdf-3yn9.48.176`）: この抑止はさらに `crates/flpdf/src/xref.rs::insert_reconstructed_xref_entry`（B20 参照）へ移り、行スキャン完了後の一括 `.retain()` ではなくスキャン中の逐次 consult になったが、`deleted_objects` はスキャン中に変化しないため結果は同一——上記回帰テストは変更なしで green のまま |
| P3 | B22 | **2026-09-08 部分解消・probe は継続（`flpdf-3yn9.48.19`）**: `reconstruct_xref_and_retry` の compressed 分岐を `Ok(None)`（`Free`/`None` と同じ warn+null）へ統一し、`qpdf --check` 相当の `object N G not found in file after regenerating cross reference table` 警告と null 解決を再構築後の compressed entry でも再現するようにした。新規ユニットテスト `reconstruct_xref_and_retry_treats_a_post_reconstruction_compressed_entry_as_not_found`（`crates/flpdf/src/reader/resolver.rs`）で `Ok(None)` を直接検証。呼び出し元にあった `Error::Unsupported` 捕捉からの `resolve_object_stream_or_null` 再ルーティング（未カバーで `cov:ignore` 済みだった箇所）は削除した  **未確認として残す部分**: 本番経路ではこの分岐に到達できない。`resolve_indirect` が `reconstruct_xref_and_retry` を呼ぶのは `Uncompressed` アームだけで、再構築は `install_source_xref_entries`（`crates/flpdf/src/reader/resolver.rs:3508`）がテーブルを丸ごと置換する形で行われ、`recover_xref_entries` は `crates/flpdf/src/xref.rs::insert_reconstructed_xref_entry`（2026-09-19、`flpdf-3yn9.48.176` で新設。B20 参照）経由でのみ書き込み、同関数は常に `XrefEntry::Uncompressed` しか構築しない。qpdf 側も同じで、`reconstruct_xref` が削除するのはtype 1 のみ（`libqpdf/QPDF.cc:531-540`）だが、要求 objgen は 1 エントリしか持てないためtype 1 を消した後にその objgen が type 2 として残ることはなく、`getType() == 1`（`:1618`）の compressed 側は qpdf でも防御的な分岐である。したがって新規ユニットテストは`source_xref_entries` に `Compressed` 行を直接注入して分岐だけを固定しており、警告文言と null 解決を end-to-end で通したわけではない。**この probe は「再構築後に compressed entry が残る実 fixture を構成できるか」の確認として開いたままにする**。 |
...
| P7 | B7 | **解消（2026-09-08、`flpdf-3yn9.48.14`）**: canonical/bootstrapのObjStm headerを専用 `Tokenizer::next_object_stream_integer`（`read_token(true, 0)` を2回呼ぶconsumer）へ移行し、qpdf `libqpdf/QPDF.cc:1801-1814` の token読取→integer検査順を固定した。汎用 `next_integer` と xref ByteCursorの `read_token(false)` はclassic readLine/parse_xrefEntry責務として変更していない。**2026-09-18（`flpdf-3yn9.48.156`）で残りも解消**: `ByteCursor::read_token` も `allow_bad = true` へ揃えた。そのとき classic subsection loop の独自 lookahead（`skip_ws` + `peek_word`）を qpdf の `while (!done)` 形へ戻す必要があり、`%comment`・subsection 無し・bad `startxref` token・rewind offset の 4 件で qpdf 11.9.0 との観測差を実測して修正した（B7 行参照）。**解消（2026-09-19、`flpdf-3yn9.48.175`）**: 3 entrypoint を 1 本へ畳む統合を完了した（B7 行参照）。 |
