...
| B29 | `QPDF::getWarnings`（**返して clear**） / `anyWarnings` / `numWarnings`（clear しない） | `libqpdf/QPDF.cc:345-352,353-363` | absent（`crates/flpdf/src/reader.rs::repair_diagnostics` は clear しない snapshot） | prod: 20 / test: 162（B27 と同じ） | unknown | absent | drain 版が無く、consumer は `entries().len()` を bookmark にして差分を読む（`crates/flpdf/src/reader/resolver.rs:1997-2008` の doc が `flpdf-cli` の `finish_lazy_warnings` / `emit_warnings_since` をその前提として明記）。qpdf の consumer は `getWarnings()` を 1 回呼んで空にする設計なので、二重出力/取りこぼしの条件が両者で違う。qpdf の CLI 側がどの粒度で drain するかを確認しないと canonical owner を決められない。必要な source: `qpdf/qpdf.cc` と `libqpdf/QPDFJob.cc` の `getWarnings` 呼び出し（`rg -n 'getWarnings' $Q/qpdf $Q/libqpdf`） |
...
合計 34 行。canonical owner が `absent` の行は B5 / B7 / B24 / B29 / B34 の 5 行で、
中身は 3 種類に分かれる: B5 / B7 / B29 は **qpdf にある集約点が flpdf に無い**
（`ParseGuard` の再入検出、`QPDF::readToken` の `allow_bad = true` 固定、`getWarnings` の
drain が、それぞれ消滅したか呼び出し元へ分散した。B5 / B29 は落としてよいかが未判断なので
unknown）。B34 は逆に **qpdf に無い状態を flpdf が持っている**。B24 は
