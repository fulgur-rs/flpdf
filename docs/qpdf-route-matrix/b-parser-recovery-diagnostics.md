...
caller の数え方: `rg -n '<pattern>' crates --glob '*.rs'` の出力を、`tests/` 配下・
`#[cfg(test)] mod ... { }` ブロック内・doc comment 行を test / 除外、それ以外の `src/` を
production として数える。宣言行自身も production に含む（`prod` の括弧内はファイル名）。
flpdf の宣言可視性は entrypoint 列に併記する。
...
| B2 | 同上を `content_stream = true` で呼ぶ経路（content stream の parse） | `libqpdf/QPDFParser.cc:27,44-47,99-100,193-196,313-320,338-348` — qpdf は **同じ `QPDFParser`** を bool 1 個で切り替える | `crates/flpdf/src/parser.rs::ContentHandleParser`（`pub(crate)`。`crates/flpdf/src/content_stream.rs::parse_content_stream_handles` から使う） | prod: 2 (content_stream.rs) / test: 0 | mixed | `crates/flpdf/src/parser.rs::parse_live_file_object_with_context` | flpdf は content stream 用に **構造的に別の再帰下降 parser** を持つ（`crates/flpdf/src/parser.rs:1809-2131`。独自の `content_good_count` / `content_give_up` / `recover_content_null` / `finish_content_dictionary`）。B1 と B4 の bad-token 予算・診断文言が経路ごとに独立しているため、qpdf の 1 実装に対する 2 経路。残 caller: `content_stream.rs:89`（`ContentHandleParser::with_tokenizer`）と `content_stream.rs:13`（import） |
| B3 | `QPDFParser::warn`（`context == nullptr` のとき warning を `QPDFExc` として throw に昇格） | `libqpdf/QPDFParser.cc:487-498`（`:496` が throw）。`libqpdf/QPDFObjectHandle.cc:1672-1698` から context 無しで呼ばれる | `crates/flpdf/src/parser.rs::parse_explicit_object_handle`（`pub(crate)`。`has_context = false` で `parse_live_file_object_with_context` を呼ぶ、`crates/flpdf/src/parser.rs:287-311`） | prod: 2 (parser.rs, object_handle.rs) / test: 0 | canonical | `crates/flpdf/src/parser.rs::parse_live_file_object_with_context` | B1 と同じ 1 実装の分岐なので経路は 1 本。qpdf の `R` に対する `std::logic_error`（`libqpdf/QPDFParser.cc:161-165`）と、warning 昇格（`:496`）の 2 種の throw を両方この分岐で扱えているかは B32 の error 分類と合わせて確認が要る |
| B4 | `QPDFParser::tooManyBadTokens`（`good_count <= 4` かつ `bad_count > 5` で打ち切り） | `libqpdf/QPDFParser.cc:472-485` | `crates/flpdf/src/parser.rs::too_many_bad_tokens`（private、B1 側）/ `ContentHandleParser` 側は `content_good_count` + `content_give_up`（`crates/flpdf/src/parser.rs:1968-2101`） | prod: 7 (parser.rs) / test: 0 | mixed | `crates/flpdf/src/parser.rs::too_many_bad_tokens` | B2 の帰結。qpdf は 1 つの `bad_count` / `good_count` メンバーを content / file 双方で共有する（`QPDFParser` のメンバー、`libqpdf/QPDFParser.cc:137,144`）。flpdf は 2 経路それぞれが自前のカウンタを持つため、同じ入力でも打ち切り位置が経路間で一致する保証がない |
| B5 | `QPDF::inParse` / `QPDF::ParseGuard`（parse 中の再入解決を `std::logic_error` で禁止） | `libqpdf/QPDF.cc:476-485`, `include/qpdf/QPDF.hh:798-817`, `libqpdf/QPDFParser.cc:34` | absent | prod: 0 / test: 0 | unknown | absent | `rg -n 'in_parse\|ParseGuard' crates/flpdf/src` は 0 件。flpdf の parser は handle-producing で parse 中に解決しない設計（`crates/flpdf/src/parser.rs:256-258` の doc）なので guard 不要と読めるが、qpdf の契約（この状態を logic_error として観測可能にする）を落としてよいかは未判断。必要な確認: `crates/flpdf/src/parser.rs::LiveFileParser` の全 `resolver` 呼び出しが `HandleResolver::indirect_handle`（解決しない）だけかを `rg -n 'self\.resolver\.' crates/flpdf/src/parser.rs` で列挙する |
...
| B7 | `QPDF::readToken`（`m->tokenizer` を **常に `allow_bad = true`** で呼ぶ薄いラッパー） | `libqpdf/QPDF.cc:1535-1539` | absent（各呼び出し元が `read_token(true/false, ...)` を直接指定する） | entrypoint が absent のため単一の caller 数は無い。`allow_bad` を明示する `.read_token(` は prod: 16 / test: 0、うち `false` を渡すのは 4 箇所 (tokenizer.rs:873,885,896 と xref.rs:3755) | mixed | absent | qpdf 側は「`QPDF` 経由の token 読みは bad token で throw しない」という不変条件をこの 1 関数で保証している。flpdf は同じ判断を呼び出し元 16 箇所に分散させており、`crates/flpdf/src/tokenizer.rs:868-911`（`skip_ignorable` / `next_integer` / `expect_word`）と `crates/flpdf/src/xref.rs:3755` は `allow_bad = false` を渡す。qpdf の対応箇所がどれも `allow_bad = true` である以上、この 4 箇所は経路ごとに throw/warn 境界が違う。確認が要る: `crates/flpdf/src/xref.rs:3755` の呼び出しが qpdf のどの読みに対応するか（`libqpdf/QPDF.cc:846-946` の classic 経路は `readLine` + `parse_xrefEntry` でトークナイザを使わない） |
| B8 | `QPDF::readObjectAtOffset`（`N G obj` 検査・offset 0・回復 1 回・cache 登録） | `libqpdf/QPDF.cc:1541-1697` | `crates/flpdf/src/reader/resolver.rs::read_object_at_offset_with_description`（private） / bootstrap 側は `crates/flpdf/src/xref.rs::read_uncompressed_object`（private） | prod: 5 (resolver.rs) + 2 (xref.rs) / test: 0 + 4 | mixed | `crates/flpdf/src/reader/resolver.rs::read_object_at_offset_with_description` | xref bootstrap（`Pdf` 構築前）と canonical resolver（構築後）で別実装。qpdf は `read_xrefStream` からも `resolve` からも同じ `readObjectAtOffset` を呼ぶ（`libqpdf/QPDF.cc:956`, `:1725`）。残 caller（bridge 側）: `crates/flpdf/src/xref.rs:915`（`read_uncompressed_object` 呼び出し）。`crates/flpdf/src/xref.rs:576-721` は `qpdf-deviation-start/end` でマーク済み（reconstruction-only の read window bound） |
| B9 | `QPDF::readObject`（`endobj` framing・`empty object treated as null`・`expected endobj` warn） | `libqpdf/QPDF.cc:1330-1357` | `crates/flpdf/src/reader/file_object.rs::parse_file_object_handle_syntax` + `crates/flpdf/src/reader/file_object.rs::finish_file_object_handle`（どちらも `pub(crate)`） | prod: 3 (file_object.rs, reader.rs, xref.rs) / 2 (file_object.rs, xref.rs) / test: 2 / 2 | mixed | `crates/flpdf/src/reader/file_object.rs::parse_file_object_handle_syntax` | 2 段（syntax → finish）に割った framing 自体は 1 実装だが、`crates/flpdf/src/reader.rs:2168` が legacy 経路から直接呼び、`crates/flpdf/src/xref.rs:537-545` が bootstrap から呼ぶ。canonical resolver は `read_object_at_offset_with_description`（B8）内で別に framing する。残 bridge caller: `crates/flpdf/src/reader.rs:2168`, `crates/flpdf/src/xref.rs:537`, `crates/flpdf/src/xref.rs:545` |
| B10 | `QPDF::readStream` + `QPDF::validateStreamLineEnd`（`/Length` 検査・`endstream` 検査・行末 3 種の warn） | `libqpdf/QPDF.cc:1360-1399,1401-1449` | `crates/flpdf/src/reader/resolver.rs::validate_stream_line_end`（private） / bootstrap・legacy 側は `crates/flpdf/src/reader/file_object.rs::finish_file_object_handle` | prod: 2 (resolver.rs) / 2 (file_object.rs) / test: 0 / 2 | mixed | `crates/flpdf/src/reader/resolver.rs::validate_stream_line_end` | qpdf の 3 warning 文言（`stream keyword followed by carriage return only` / `not followed by proper line terminator` / `followed by extraneous whitespace`）は `rg -l` で 2 / 2 / 1 ファイルにまたがって現れ、resolver 側と file_object 側の両方が持つ。qpdf は 1 関数 |
| B11 | `QPDF::recoverStreamLength`（`attempting to recover stream length` → `endstream`/`endobj` 探索 → 2 種の結果 warn） | `libqpdf/QPDF.cc:1469-1479,1481-1533` | `crates/flpdf/src/reader/resolver.rs::recover_stream_length`（private） / `crates/flpdf/src/reader/file_object.rs::recover_stream_boundary`（private） | prod: 3 (resolver.rs) / 2 (file_object.rs) / test: 2 / 0 | mixed | `crates/flpdf/src/reader/resolver.rs::recover_stream_length` | 2 実装。`file_object.rs` 側は `RecoveryPolicy`（`crates/flpdf/src/reader/file_object.rs:11-16`）という qpdf に対応物のない enum で `RequireEndstream` / `Bounded` を切り替え、qpdf の「`attempt_recovery` の有無」1 bit（`libqpdf/QPDF.cc:1391`）を 2 値の別概念に置き換えている。残 bridge caller: `crates/flpdf/src/reader/file_object.rs:377` |
| B12 | `QPDF::resolveObjectsInStream`（ObjStm 展開・`/N` `/First` 検査・`has wrong type` warn・xref 一致メンバーのみ cache） | `libqpdf/QPDF.cc:1755-1833`, `libqpdf/QPDF.cc:1451-1467` | `crates/flpdf/src/reader/resolver.rs::resolve_object_stream_with_failure_kind`（private） / `crates/flpdf/src/xref.rs::resolve_objects_in_stream`（`BootstrapHandleDocument` の private メソッド） | prod: 2 (resolver.rs) / 2 (xref.rs) / test: 1 / 0 | mixed | `crates/flpdf/src/reader/resolver.rs::resolve_object_stream_with_failure_kind` | qpdf は `m->resolved_object_streams` を 1 つ持つ（`include/qpdf/QPDF.hh:1485`）。flpdf は bootstrap 用に `crates/flpdf/src/xref.rs::BootstrapHandleState`（`resolving` / `resolved_object_streams` / `diagnostics` / `handles` を持つ、`crates/flpdf/src/xref.rs:67-73`）という **並行する第 2 の document 状態** を持つ。残 bridge caller: `crates/flpdf/src/xref.rs:931` |
| B13 | `QPDF::readTrailer`（trailer 位置の復元・`empty object` warn・`stream keyword found in trailer` warn） | `libqpdf/QPDF.cc:1312-1328` | `crates/flpdf/src/xref.rs::parse_trailer_candidate`（private、reconstruct 用） / classic 経路は `crates/flpdf/src/xref.rs:1590-1616` にインライン | prod: 2 (xref.rs) / test: 1 | mixed | `crates/flpdf/src/xref.rs::parse_trailer_candidate` | qpdf は 1 つの `readTrailer` を `read_xrefTable`（`libqpdf/QPDF.cc:894`）と `reconstruct_xref`（`:565`）の両方から呼ぶ。flpdf は classic 経路が `parse_qpdf_direct_object_handle_with_diagnostics` を直接呼ぶインライン、reconstruct 経路が `parse_trailer_candidate` と別。qpdf の `stream keyword found in trailer` warning は flpdf src に文字列として存在しない（`rg -l --fixed-strings` 0 件） |
| B14 | `QPDF::read_xref` の `while (xref_offset)` ループ（`/Prev` 追跡と `visited` による loop 検出） | `libqpdf/QPDF.cc:626-719`（`visited.insert(xref_offset)` が `:631`、検出が `:679-683`） | 初段は `crates/flpdf/src/xref.rs::parse_xref_from_start` 直呼び（`crates/flpdf/src/xref.rs:1340`）、2 段目以降は `crates/flpdf/src/xref.rs::merge_previous_xref_sections_with_observer`（private） | prod: 4 (xref.rs) / 3 (xref.rs) / test: 6 / 0 | mixed | `crates/flpdf/src/xref.rs::merge_previous_xref_sections_with_observer` | qpdf は初段も後続も同じループ 1 本。flpdf は初段を外に出しており、その結果 **`visited` に初段の startxref が seed されない**（`crates/flpdf/src/xref.rs:2008` で空集合から開始、初段 offset は挿入されない）。`/Prev` が初段 startxref を指す入力では qpdf が 1 周目で `loop detected following xref tables` を投げるのに対し、flpdf はその section をもう一度 parse してから 2 周目で検出し、重複 parse 分の診断を `crates/flpdf/src/xref.rs:2074-2076` で二重に push する。観測確認の probe は「unknown / probe」の P1 |
| B15 | `QPDF::read_xrefTable` の subsection / entry 解析（`parse_xrefFirst` / `parse_xrefEntry`、`'f'` の後回し） | `libqpdf/QPDF.cc:722,770,846-893,929-932` | `crates/flpdf/src/xref.rs::parse_xref_table`（private） | prod: 2 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::parse_xref_table` | 呼び出し元は `crates/flpdf/src/xref.rs:1574` の 1 箇所のみ。`'f'` の後回し（`deferred_free`, `crates/flpdf/src/xref.rs:1580-1588,1692-1694`）も qpdf の `deleted_items` 順序（`libqpdf/QPDF.cc:849,880-885,929-932`）と一致 |
| B16 | classic trailer の `/Size` 検証（最初の trailer のみ、`hasKey` → `isInteger` の順） | `libqpdf/QPDF.cc:902-913` | `crates/flpdf/src/xref.rs::validate_classic_trailer`（private） | prod: 2 (xref.rs) / test: 0 | canonical | `crates/flpdf/src/xref.rs::validate_classic_trailer` | 呼び出し元は `crates/flpdf/src/xref.rs:1658` の 1 箇所。`docs/qpdf-correspondence.md:187-197`（Classic xref trailer validation, 2026-09-02）が qpdf の `readTrailer` 位置復元（`libqpdf/QPDF.cc:1312-1328`）まで含めて対応を記録済み。`trailer dictionary lacks /Size key` の文言も flpdf src に実在 |
| B17 | `QPDF::read_xrefStream` + `QPDF::processXRefStream`（`/W` `/Index` `/Size` 検証、entry decode、type 0/1/2 振り分け） | `libqpdf/QPDF.cc:949-969,972-1146` | `crates/flpdf/src/xref.rs::parse_xref_stream`（private） | prod: 3 (xref.rs) / test: 1 | canonical | `crates/flpdf/src/xref.rs::parse_xref_stream` | 呼び出し元 2 箇所（`crates/flpdf/src/xref.rs:1699` 主経路、`:1923` hybrid `/XRefStm`）は qpdf の 2 呼び出し元（`libqpdf/QPDF.cc:676`, `:919`）と 1:1。ただし qpdf の検証文言（`Cross-reference stream does not have proper /W and /Index keys` / `unknown xref stream entry type`）は flpdf src に文字列として存在しない（`rg -l --fixed-strings` 0 件）— 文言の対応は本表の対象外だが、B22 の error 分類と合わせて別途確認する |
| B18 | hybrid: classic trailer の `/XRefStm` を読み、**戻り値を捨てて** trailer の `/Prev` を使う | `libqpdf/QPDF.cc:915-927` | `crates/flpdf/src/xref.rs::merge_xref_stream_from_classic_trailer`（private） | prod: 2 (xref.rs) / test: 2 | canonical | `crates/flpdf/src/xref.rs::merge_xref_stream_from_classic_trailer` | 呼び出し元は `crates/flpdf/src/xref.rs:1683` の 1 箇所。`invalid /XRefStm` の文言も実在。`ignore_xref_streams` 相当は `crates/flpdf/src/xref.rs::XrefLoadOptions` が持つ |
| B19 | `QPDF::insertXrefEntry` / `QPDF::insertFreeXrefEntry`（first-seen wins と object-number 単位の tombstone） | `libqpdf/QPDF.cc:1149-1184,1187-1192` | `crates/flpdf/src/xref.rs::XrefRegistration`（private struct。`insert_xref_entry` / `insert_free_xref_entry`） | `insert_xref_entry(` prod: 4 (xref.rs 3 + resolver.rs 1) / `insert_free_xref_entry(` prod: 3 (xref.rs) / test: 18 / 0 | canonical | `crates/flpdf/src/xref.rs::XrefRegistration` | `crates/flpdf/src/reader/resolver.rs:2593-2599` にも同名 `insert_xref_entry` があるが `#[cfg(test)]` 付きの fixture 専用で production 経路ではない。`crates/flpdf/src/xref.rs:188-196` の doc が `deleted_objects` の lifetime（`libqpdf/QPDF.cc:686-708` vs `:575`）まで書き分けている |
| B20 | `QPDF::insertReconstructedXrefEntry`（reconstruct 専用の後勝ち上書き + `obj > 0 && 0 <= gen < 65535` guard + `deleted_objects` 抑止） | `libqpdf/QPDF.cc:1197-1210` | `crates/flpdf/src/xref.rs::scan_object_header_after_first_token`（private。guard を実装） + `crates/flpdf/src/xref.rs:2428` の `entries.insert`（後勝ち） | prod: 2 (xref.rs) / test: 0 | mixed | `crates/flpdf/src/xref.rs::recover_xref_entries` | guard（`crates/flpdf/src/xref.rs:3056-3059`）と後勝ち（`:2428`）は qpdf と一致。3 つ目の条件 `m->deleted_objects.count(obj)` による抑止（`libqpdf/QPDF.cc:1204-1209`）は `recover_xref_entries` 自身ではなく **呼び出し後の `crates/flpdf/src/xref.rs::merge_recovered_qpdf_state` が `retain` で事後適用** する（`crates/flpdf/src/xref.rs:2344-2354`。コメントが `libqpdf/QPDF.cc:516-575,1194-1210` と scan filter の `:575` clear まで対応づけている）。mixed の理由は **この事後適用が経路ごとに有無が違う** こと: bootstrap の 4 つの handoff のうち `crates/flpdf/src/xref.rs:1403`（pending trigger）/ `:1432`（`/Prev` chain 失敗）/ `:1486`（`/Size` 由来）は merge を通すが、初段 parse 失敗の handoff（`crates/flpdf/src/xref.rs:1368-1383`）は merge を経ず `discard_lower_generations` だけで return する。resolver 経路は `crates/flpdf/src/reader/resolver.rs:1648-1654` のコメントで「qpdf の filter は適用できない」と別途判断済み。残る未決は P2 |
| B21 | `read_xref` 後段: `/Size` 不一致 warn と「同一 object 番号は最高 generation のみ残す」 | `libqpdf/QPDF.cc:689-704,708,710-718` | `crates/flpdf/src/xref.rs::append_xref_size_warning_for`（private） + `crates/flpdf/src/xref.rs::discard_lower_generations`（private） | prod: 4 / 5 (どちらも xref.rs) / test: 0 / 0 | canonical | `crates/flpdf/src/xref.rs::append_xref_size_warning_for` | どちらも実装は 1 本で、複数の完了地点（通常 / reconstruct / 候補 xref stream 再入）から同じ関数を呼んでいる。`is not one plus the highest object number` の文言も一致。`crates/flpdf/src/xref.rs:223-233` の doc が `QPDF::removeObject`（`libqpdf/QPDF.cc:710-718`）との対応を記録 |
| B22 | `QPDF::reconstruct_xref`（再入禁止 → 3 連 warn → type 1 削除 → 行スキャン → 候補 xref stream からの trailer 復元 → 失敗時 throw） | `libqpdf/QPDF.cc:516-623` | `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan`（private、open 時） / `crates/flpdf/src/reader/resolver.rs::reconstruct_xref_and_retry`（private、resolve 時） | prod: 5 (xref.rs) / 2 (resolver.rs) / test: 0 / 2 | mixed | `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan` | qpdf は 1 実装を 2 箇所（`libqpdf/QPDF.cc:463` parse 直下、`:1617` `readObjectAtOffset` の再試行）から呼ぶ。flpdf は呼び出し箇所ごとに別実装で、resolve 側は qpdf の後半（候補 `/XRef` stream からの trailer 復元 `libqpdf/QPDF.cc:577-608` と `unable to find trailer dictionary while recovering damaged file` の throw `:610-616`）を実装しない。さらに resolve 側は再構築後の entry が compressed だと `Error::Unsupported` を返す（`crates/flpdf/src/reader/resolver.rs:1674-1677`）が、qpdf は type 1 以外を一律 `not found in file after regenerating cross reference table` の warn + null にする（`libqpdf/QPDF.cc:1618-1633`）。probe は P3 |
| B23 | `reconstruct_xref` の行スキャン本体（行頭から `MAX_LEN = 100` で token を読み `int int obj` を拾う） | `libqpdf/QPDF.cc:543-575` | `crates/flpdf/src/xref.rs::recover_xref_entries`（`pub(crate)`） | prod: 3 (xref.rs, resolver.rs) / test: 1 | canonical | `crates/flpdf/src/xref.rs::recover_xref_entries` | B22 の 2 経路が **スキャン本体だけは共有** している（`crates/flpdf/src/reader/resolver.rs:1655` と `crates/flpdf/src/xref.rs:2228`）。B22 が mixed なのはスキャンの前後（warn 順序・trailer 復元・retry）であって、この primitive ではない |
| B24 | reconstruct 後の ObjStm 内部走査（qpdf は **意図的に行わない**） | `libqpdf/QPDF.cc:618-622`（コメント）+ `:532-541`（削除対象は type 1 のみ） | absent | prod: 0 / test: 0 | bridge | absent | 「qpdf に対応物が無い経路」で残 caller ゼロ = §3 の bridge 定義が言う削除候補が、**既に削除された後** の状態。canonical にはしない（§3 の canonical は当該 entrypoint の性質で定義されるので、entrypoint が無い行には述語が適用できない）。かつて存在した `recover_objstm_compressed_entries` はコミット `6ddb9661`（"fix: align damaged xref recovery with qpdf ObjStm behavior", 2026-08-09）で削除済み。現在 `rg -n 'recover_objstm_compressed_entries' crates` は 0 件で、`crates/flpdf/src/reader/resolver.rs:1648-1654` のコメントが「type 1 行のみ削除」の qpdf 挙動を明示している。回復後に type 2 entry が残るのは削除対象が type 1 だけだから、という理解が両経路のコードに反映されている |
| B25 | `m->reconstructed_xref`（回復予算 = `bool` 1 個。2 回目は例外を re-throw） | `include/qpdf/QPDF.hh:1480`, `libqpdf/QPDF.cc:518-522,524-526` | `crates/flpdf/src/reader/resolver.rs::reconstructed_xref`（`pub(crate)` getter。実体は `crates/flpdf/src/reader/resolver.rs:350-356` の core フィールド） / open 時は `crates/flpdf/src/xref.rs:1646` の `already_reconstructed` | prod: 9 (resolver.rs, reader.rs) / test: 20 | mixed | `crates/flpdf/src/reader/resolver.rs::reconstructed_xref` | 状態そのものは 1 つに集約されているが、open 時の回復は `Pdf` 構築前に起きるため `LoadedXrefState::already_reconstructed` を経由して `ResolverCore` に転記される（`crates/flpdf/src/engine.rs:215-224`）。qpdf は `QPDF` が最初から存在するので転記が無い。B22 と同根 |
| B26 | `m->attempt_recovery`（回復分岐の on/off。分岐点は全ソースで 3 箇所） | `include/qpdf/QPDF.hh:1461`, `libqpdf/QPDF.cc:336,463,1391,1563` | `crates/flpdf/src/reader/resolver.rs::attempt_recovery`（`pub(crate)`） / open 時は `crates/flpdf/src/xref.rs::XrefLoadOptions` の `allow_repair` | `attempt_recovery(` prod: 2 (resolver.rs) / test: 0。`XrefLoadOptions` prod: 18 (xref.rs, engine.rs) / test: 42。`RecoveryPolicy` prod: 20 (file_object.rs, xref.rs) / test: 6 | mixed | `crates/flpdf/src/reader/resolver.rs::attempt_recovery` | 同じ 1 bit が 2 つの型に分かれて存在する。さらに `crates/flpdf/src/reader/file_object.rs::RecoveryPolicy`（B11）が同じ設定を 3 つ目の表現（`RequireEndstream` / `Bounded`）に翻訳しており、`crates/flpdf/src/xref.rs:517-523` が `allow_repair` からそれを導出する。qpdf の分岐点 3 箇所に対して flpdf の分岐点は数え直しが要る |
| B27 | `m->warnings`（**唯一の warning sink**。順序 = `warn` 呼び出し順、`push_back` のみ） | `include/qpdf/QPDF.hh:1475`, `libqpdf/QPDF.cc:487-494` | `crates/flpdf/src/reader/resolver.rs::ResolverCore` の `repair_diagnostics` フィールド（`crates/flpdf/src/reader/resolver.rs:357-366`）。open 前は `crates/flpdf/src/xref.rs::LoadedXref` の `repair_diagnostics`（`crates/flpdf/src/xref.rs:57-64`）と `crates/flpdf/src/xref.rs::BootstrapHandleState` の `diagnostics`（`crates/flpdf/src/xref.rs:67-73`） | 公開ドア `.repair_diagnostics()` — prod: 21 (flpdf-cli/main.rs, flpdf-qtest-tools 5 ファイル, engine.rs, job/check.rs, job/inspection.rs, job/lifecycle.rs, json/document.rs, reader.rs) / test: 162 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverCore` | **`Pdf` 構築後は 1 本**（`crates/flpdf/src/reader/resolver.rs:357-366` の doc が「Every warning flpdf raises ... lands in this one collection, matching qpdf's single `m->warnings`」と明記し、`Pdf::push_warning` と `Pdf::repair_diagnostics` の 2 ドアだけを許す）。構築前が 3 段の staging になっているのが qpdf との差: `BootstrapHandleState::diagnostics` → `XrefReadContext::diagnostics`（`crates/flpdf/src/xref.rs:1142-1161` の `sync_handle_diagnostics` / `append_diagnostics_to`）→ `LoadedXref::repair_diagnostics` → `ResolverCore`（`crates/flpdf/src/engine.rs:214-231` で move + replay）。最終的な連結順は `append_diagnostics_to` を **呼んだ時点** で決まり、warn の発生順ではない。qpdf は `QPDF` が parse 前から存在するので staging 自体が無く、順序は常に `warn` 呼び出し順。probe は P4 |
...
| B29 | `QPDF::getWarnings`（**返して clear**） / `anyWarnings` / `numWarnings`（clear しない） | `libqpdf/QPDF.cc:345-352,353-363` | absent（`crates/flpdf/src/reader.rs::repair_diagnostics` は clear しない snapshot） | prod: 21 / test: 162（B27 と同じ） | unknown | absent | drain 版が無く、consumer は `entries().len()` を bookmark にして差分を読む（`crates/flpdf/src/reader/resolver.rs:1997-2008` の doc が `flpdf-cli` の `finish_lazy_warnings` / `emit_warnings_since` をその前提として明記）。qpdf の consumer は `getWarnings()` を 1 回呼んで空にする設計なので、二重出力/取りこぼしの条件が両者で違う。qpdf の CLI 側がどの粒度で drain するかを確認しないと canonical owner を決められない。必要な source: `qpdf/qpdf.cc` と `libqpdf/QPDFJob.cc` の `getWarnings` 呼び出し（`rg -n 'getWarnings' $Q/qpdf $Q/libqpdf`） |
| B30 | `QPDFExc::createWhat`（`filename (object, offset N): message` の文言組み立て。`offset == 0` は「offset 無し」） | `libqpdf/QPDFExc.cc:18-51`, `include/qpdf/QPDFExc.hh:29-77` | `crates/flpdf/src/object_handle.rs::format_qpdf_exception_what`（`pub(crate)`, `offset: i64`） / `crates/flpdf/src/reader/resolver.rs::qpdf_exception_what`（private, `offset: usize`） / `crates/flpdf/src/reader/resolver.rs::route_warning`（private、logger 行の prefix を独自に組む） | prod: 3 / 2 / 7（順に object_handle.rs+page_document_helper.rs, resolver.rs, resolver.rs） / test: 4 / 0 / 3 | mixed | `crates/flpdf/src/object_handle.rs::format_qpdf_exception_what` | **1 つの qpdf 関数に 3 実装**。(1) `format_qpdf_exception_what` は `i64` を取り、qpdf の「負 offset でも空の `()` を出す」挙動まで写している（`crates/flpdf/src/object_handle.rs:944-982` の doc が明記）。(2) `qpdf_exception_what` は `usize` なので負 offset の分岐が構造的に到達不能。(3) `route_warning`（`crates/flpdf/src/reader/resolver.rs:475-517`）は `createWhat` を呼ばず、`message` が `"(object "` / `"(trailer"` で始まるかで分岐する独自 prefix 規則を持つ。残 caller: `crates/flpdf/src/page_document_helper.rs:278`, `crates/flpdf/src/object_handle.rs:6239`, `crates/flpdf/src/reader/resolver.rs:1217`, および `route_warning` の 6 呼び出し（`crates/flpdf/src/reader/resolver.rs:463,1287,1887,1928,1962,1979`。宣言は `:475`） |
| B31 | `QPDFLogger`（`p_warn` 既定 null → `getError` へフォールバック、`setOutputStreams` で `p_warn` を null に戻す） | `libqpdf/QPDFLogger.cc:109-116,218-246,248-255` | `crates/flpdf/src/logger.rs::QPDFLogger`（`pub`） | prod: get_warn 呼び出しは `route_warning` 経由に集約 / test: — | canonical | `crates/flpdf/src/logger.rs::QPDFLogger` | 逐語移植。`get_warn` のフォールバック（`crates/flpdf/src/logger.rs:181-184`）と `set_output_streams` の `warn = None`（`crates/flpdf/src/logger.rs:282`）が qpdf と 1:1。qpdf 同様 **sink ではない**（`m->warnings` への push には関与しない）ことも B27/B28 の構造で保たれている |
| B32 | 例外分類: `QPDFExc`（`std::runtime_error` 派生 + `qpdf_error_code_e`）と `std::logic_error` / `std::runtime_error` | `include/qpdf/QPDFExc.hh:29-77`, `libqpdf/QPDF.cc:481,1231`, `libqpdf/QPDFParser.cc:163`, `libqpdf/QPDFTokenizer.cc:241,248,770`, `libqpdf/QPDFLogger.cc:200,252`, `libqpdf/QPDF.cc:1101`（`std::range_error`） | `crates/flpdf/src/error.rs::Error`（`pub` enum。`Internal` / `System` / `Parse` / `Pages` / `Encrypted` / `Missing` / `Unsupported` / `Usage` / `Io` / `FileIo` / `OpenFailure`） | `Error::Internal(` prod: 166 / test: 91、`Error::System(` prod: 103 / test: 138、`Error::parse(` prod: 161 / test: 10 | mixed | `crates/flpdf/src/error.rs::Error` | 型は 1 つに集約されているが、**qpdf の 2 軸（例外クラス × `qpdf_error_code_e`）を Rust の 1 軸に畳んでいる**。`Internal` ↔ `std::logic_error` / `System` ↔ `std::runtime_error` は `crates/flpdf/src/error.rs:42-43` の doc が明記する 1:1 対応。一方 `Parse` は qpdf の `QPDFExc(qpdf_e_damaged_pdf, ...)` に相当するが `QPDFExc` の他の error code（`qpdf_e_object` / `qpdf_e_pages` 等）は `Pages` など別 variant に分かれ、`QPDFExc` という 1 つのクラスが flpdf では複数 variant に散る。`Missing` / `Unsupported` / `OpenFailure` は qpdf に対応する例外クラス・error code が無い。`crates/flpdf/src/reader/resolver.rs:1615` は「`Error::Parse` だけが reconstruct の trigger になる」ことを qpdf の `catch (QPDFExc&)`（`libqpdf/QPDF.cc:1614`）と対応づけているので、この振り分けは回復分岐の正しさに直接効く |
| B33 | `reconstruct_xref` の 3 連 warn（`file is damaged` → 引数 `e` → `Attempting to reconstruct cross-reference table`、順序固定） | `libqpdf/QPDF.cc:528-530` | `crates/flpdf/src/xref.rs::push_repair_diagnostics`（private、open 時） / `crates/flpdf/src/reader/resolver.rs:1626-1636`（resolve 時のインライン 3 連 `push_warning`） | `push_repair_diagnostics(` prod: 3 (xref.rs) / test: 0。resolver 側インラインは 1 箇所 (resolver.rs:1626-1636) | mixed | `crates/flpdf/src/xref.rs::push_repair_diagnostics` | B22 の帰結。2 実装があり、中央の「引数 `e` をそのまま warn する」部分の文言生成が別。`push_repair_diagnostics`（`crates/flpdf/src/xref.rs:2892-2921`）は trigger error のメッセージごとに 5 通りの分岐で `QPDFExc` 相当の文言を再構成するのに対し、resolver 側（`crates/flpdf/src/reader/resolver.rs:1631-1635`）は `"(object N G, offset X): message"` を固定書式で組む。qpdf はどちらも `warn(e)` の 1 行で、文言は `e` が作られた時点の `createWhat` が決めている（B30） |
| B34 | （qpdf に対応物なし）bounded object window の read-to-end fallback 予算 | absent — qpdf の `readObjectAtOffset` は「次の object offset」で読みを打ち切らない（`libqpdf/QPDF.cc:1577-1697` は seek して parse するだけ） | `crates/flpdf/src/pdf.rs:148-154`（`resolution_fallbacks_remaining` フィールド） + `crates/flpdf/src/engine.rs::MAX_RESOLUTION_FALLBACKS`（`= 64`） | 消費点 prod: 2 (reader.rs — `crates/flpdf/src/reader.rs::parse_source_file_object_at` と `#[cfg(feature = "qtest-driver")]` の `qtest_read_source_object_with_retry`) / test: 0 | bridge | absent | CLAUDE.md 分類 (C)（qpdf に対応処理が存在しない flpdf 固有の挙動）。legacy の window 読み経路だけが使う予算で、canonical resolver 経路は使わない。`crates/flpdf/src/reader.rs:987` の `parse_source_file_object_at` は `#[allow(deprecated)]` 付き。**未マーク**: `// qpdf-deviation` も `#[deprecated]` も付いていない（`rg -n 'qpdf-deviation' crates/flpdf/src/reader.rs` は `:628-634` の別件のみ）。残 caller: `crates/flpdf/src/reader.rs:799`（`parse_source_file_object_at` の唯一の呼び出し元） |

### 分類集計

| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 12 | B1, B3, B6, B15, B16, B17, B18, B19, B21, B23, B28, B31 |
| mixed | 18 | B2, B4, B7, B8, B9, B10, B11, B12, B13, B14, B20, B22, B25, B26, B27, B30, B32, B33 |
| bridge | 2 | B24, B34 |
| unknown | 2 | B5, B29 |

合計 34 行。canonical owner が `absent` の行は B7 / B24 / B29 / B34 の 4 行で、
中身は 3 種類に分かれる: B7 / B29 は **qpdf にある集約点が flpdf に無い**
（`QPDF::readToken` の `allow_bad = true` 固定、`getWarnings` の drain が、それぞれ
呼び出し元へ分散した）。B34 は逆に **qpdf に無い状態を flpdf が持っている**。
B24 は **両者とも持たないのが正しい**（qpdf 自身が `libqpdf/QPDF.cc:618-622` で
やらないと明言している処理）。
...
| P5 | B5 | `crates/flpdf/src/parser.rs::LiveFileParser` が parse 中に一切解決しないことを機械的に確認する。コマンド: `rg -n 'self\.resolver\.' /home/ubuntu/flpdf/.worktrees/flpdf-3yn9.41/crates/flpdf/src/parser.rs` で呼び出しを全列挙し、`HandleResolver` の 5 メソッド（`crates/flpdf/src/parser.rs:19-67`）がいずれも解決を伴わないことを確認する。伴わないなら qpdf の `ParseGuard` は「不要になった guard」として (C) 逸脱、伴うなら真の欠落 |
| P6 | B29 | qpdf の consumer がどの粒度で `getWarnings()` を drain するかを確定する。コマンド: `rg -n 'getWarnings\|anyWarnings\|numWarnings' /home/ubuntu/.cache/flpdf/qpdf-11.9.0/qpdf /home/ubuntu/.cache/flpdf/qpdf-11.9.0/libqpdf` |
| P7 | B7 | `crates/flpdf/src/xref.rs:3755` と `crates/flpdf/src/tokenizer.rs:868-911` の `allow_bad = false` 呼び出しが qpdf のどの読みに対応するかを特定する。qpdf 側で `readToken` に `allow_bad = false` を渡す箇所は `rg -n 'readToken' /home/ubuntu/.cache/flpdf/qpdf-11.9.0/libqpdf` で全列挙して確認する（`QPDF::readToken` は常に `true`、`libqpdf/QPDF.cc:1535-1539`） |
| P8 | B13, B17 | qpdf の warning / error 文言のうち flpdf src に文字列として存在しないもの（`stream keyword found in trailer` / `Cross-reference stream does not have proper /W and /Index keys` / `unknown xref stream entry type` / `xref syntax invalid` / `expected trailer dictionary` / `extraneous whitespace seen before xref` / `unable to find trailer while reading xref` / `expected integer in object stream header`）が、別の文言で出ているのか出ていないのかを確定する。コマンド: 各条件を踏む fixture で `qpdf --check` と flpdf の diagnostics を比較。文言そのものは本表の対象外だが、B13 / B17 の canonical 判定の裏取りになる |
