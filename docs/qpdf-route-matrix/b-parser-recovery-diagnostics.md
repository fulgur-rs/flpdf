...
| B20 | `QPDF::insertReconstructedXrefEntry`（reconstruct 専用の後勝ち上書き + `obj > 0 && 0 <= gen < 65535` guard + `deleted_objects` 抑止） | `libqpdf/QPDF.cc:1197-1210` | `crates/flpdf/src/xref.rs::scan_object_header_after_first_token`（private。guard を実装） + `crates/flpdf/src/xref.rs:2221` の `entries.insert`（後勝ち） | prod: 1 (xref.rs) / test: 0 | mixed | `crates/flpdf/src/xref.rs::recover_xref_entries` | guard（`crates/flpdf/src/xref.rs:2993-2996`）と後勝ち（`:2235`）は qpdf と一致。3 つ目の条件 `m->deleted_objects.count(obj)` による抑止（`libqpdf/QPDF.cc:1204-1209`）は 2026-09-18（`flpdf-3yn9.48.157`）に `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan` の行スキャン直後（`crates/flpdf/src/xref.rs:2004`）へ移した。それ以前は `merge_recovered_qpdf_state` の事後 `retain` で、しかも 5 つの reconstruction handoff のうち `startxref == 0` と初段 parse 失敗の 2 つが merge を経ずに return するため適用されておらず、実測乖離になっていた（P2 参照）。現在は 5 handoff すべてが `deleted_objects` を渡し、抑止対象は行スキャンが作った行——qpdf の `insertReconstructedXrefEntry` 呼び出しと 1:1——に限られるので、reconstruction 開始前から `m->xref_table` にあった非 type-1 行は qpdf 同様に残る（`merge_recovered_qpdf_state` から filter 引数自体を外したので構造的に保証される。ただしこの差が観測されるには tombstone された object 番号に非 type-1 行が既にある状態でさらに別の reconstruction trigger が要り、P2 の fixture では到達しない——**qpdf ソース（`libqpdf/QPDF.cc:1204-1209` は挿入を抑止するだけで既存行を消さない）根拠であって実測ではない**）。mixed が残る理由は **qpdf の 1 関数 `insertReconstructedXrefEntry` が flpdf では 3 箇所に分かれている** こと: guard は `scan_object_header_after_first_token`、後勝ちは scan 本体の `entries.insert`、抑止は `recover_xref_from_linear_scan` の `retain`。resolve 側の scan（`crates/flpdf/src/xref.rs::recover_xref_entries`）は抑止を持たない——qpdf でも `read_xref` 完了時に `m->deleted_objects` が clear 済み（`libqpdf/QPDF.cc:706-708`）なので resolve 時の filter は常に空で、挙動としては一致する |
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 26 | B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15, B16, B17, B18, B19, B21, B23, B24, B28, B30, B31, B34 |
| mixed | 8 | B20, B22, B25, B26, B27, B29, B32, B33 |
| bridge | 0 | — |
| unknown | 0 | — |
