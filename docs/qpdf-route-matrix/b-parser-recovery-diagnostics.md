...
| B20 | `QPDF::insertReconstructedXrefEntry`（reconstruct 専用の後勝ち上書き + `obj > 0 && 0 <= gen < 65535` guard + `deleted_objects` 抑止） | `libqpdf/QPDF.cc:1197-1210` | `crates/flpdf/src/xref.rs::insert_reconstructed_xref_entry`（private。後勝ち上書き + `deleted_objects` 抑止） + `crates/flpdf/src/xref.rs::scan_object_header_after_first_token`（private。guard） | `insert_reconstructed_xref_entry(` prod: 2 (xref.rs) / test: 5 (xref.rs)。`scan_object_header_after_first_token(` prod: 1 (xref.rs) / test: 1 | canonical | `crates/flpdf/src/xref.rs::insert_reconstructed_xref_entry` | 2026-09-19（`flpdf-3yn9.48.176`）に後勝ち上書きと `deleted_objects` 抑止を 1 primitive へ統合。2026-09-23 の B22 cutover では両 recovery entrypoint が `reconstruct_xref_on_owner` → `recover_xref_entries_from_source` を通り、同じ owner registration と deleted-object filter を使う。resolve retry 時の `deleted_objects` は qpdf の `/Size` 後 clear により空。guard は `ObjectRef` を構築する前の `scan_object_header_after_first_token` にある（範囲外値を `ObjectRef` が表現できないため qpdf と配置は異なる）。P2 fixture の `qpdf --show-xref` 比較で deleted row 抑止と xref output の一致を確認済み |
...
| B22 | `QPDF::reconstruct_xref`（再入禁止 → 3 連 warn → type 1 削除 → 行スキャン → trailer 未確定時のみ候補 xref stream から復元） | `libqpdf/QPDF.cc:516-623` | `crates/flpdf/src/xref.rs::reconstruct_xref_on_owner`（`pub(crate)`、open/resolve 共通） / `crates/flpdf/src/reader/resolver.rs::reconstruct_xref_and_retry`（type-1 の retry continuation） | `reconstruct_xref_on_owner(` prod: 5 (xref.rs 4, resolver.rs 1) / test: 3。`reconstruct_xref_and_retry(` prod: 2 / test: 3 | canonical | `crates/flpdf/src/xref.rs::reconstruct_xref_on_owner` | 2026-09-23（`flpdf-3yn9.48.176`）: open と resolve の両方が同じ owner 操作を呼び、guard、順序付き warning、type-1 削除、owner の live-source scan、`deleted_objects` の解除、trailer 未確定時の candidate `/XRef` stream 再読込を一か所に集めた。open caller は `LoadedXrefState` を受けて `/Size` validation を続け、resolve caller は raw `QpdfObjGen` を再読して type-1 のみ retry する。resolve-time の parse trigger は qpdf 相当の object/offset を持つ `QpdfExc` に caller 境界で変換し、元の trigger は guard の再試行時に返す。qpdf 11.9.0 と flpdf CLI へ stale object header fixture を `--show-object=1` で与え、両者 exit 3、同じ 4 warning 順、`null` を確認。`canonical_xref_owner_route_tests::open_and_resolve_recovery_share_one_owner_operation` が共有 call site と helper の責務を固定する |
| B23 | `reconstruct_xref` の行スキャン本体（行頭から `MAX_LEN = 100` で token を読み `int int obj` を拾う） | `libqpdf/QPDF.cc:543-575` | `crates/flpdf/src/xref.rs::recover_xref_entries_from_source`（private、共有 owner scan） | prod: 1 (xref.rs) / test: 2 | canonical | `crates/flpdf/src/xref.rs::recover_xref_entries_from_source` | B22 の `reconstruct_xref_on_owner` が open/resolve 共通で 1 回だけ呼ぶ。旧 `recover_xref_entries` の byte-buffer adapter は parser unit fixture 専用 `#[cfg(test)]` として残し、production route から外した |
...
| B25 | `m->reconstructed_xref`（回復予算 = `bool` 1 個。2 回目は受け取った例外を再 throw） | `include/qpdf/QPDF.hh:1480`, `libqpdf/QPDF.cc:518-522,524-526` | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::begin_xref_reconstruction`（owner guard） / `crates/flpdf/src/xref.rs::reconstruct_xref_on_owner`（open/resolve 共通 entry） | getter `reconstructed_xref` prod: 11 (engine.rs 1, resolver.rs 10) / test: 25 (reader.rs 1, resolver.rs 21, xref.rs 3)。`begin_xref_reconstruction(` prod: 2 (xref.rs) / test: 1 | canonical | `crates/flpdf/src/reader/resolver.rs::begin_xref_reconstruction` | 2026-09-23（`flpdf-3yn9.48.176`）: qpdf は `reconstruct_xref` entry で guard を arm してから warning / scan に入る。flpdf の open-time と resolve-time は共通 helper に入り、再試行では warning を出さず元の trigger を返す。open-time は reconstruction scan / candidate re-entry 中の table 更新だけを scoped に許し、確定した raw table を owner に同期してから stale な外側 snapshot を拒否する。B22 の共有 owner operation と、B34 の qpdf に無い read-to-end 回数制限 bridge は独立の責務として記録する |
| B26 | `m->attempt_recovery`（回復分岐の on/off。分岐点は全ソースで 3 箇所） | `include/qpdf/QPDF.hh:1461`, `libqpdf/QPDF.cc:336,463,1391,1563` | `crates/flpdf/src/reader/resolver.rs::attempt_recovery`（`pub(crate)`、`ResolverCore` の core フィールド） / `crates/flpdf/src/xref.rs::CanonicalTrailerOwner::attempt_recovery`（`ResolverHandle::attempt_recovery` へ委譲する write-through seam） | getter `.attempt_recovery(` prod: 6 (resolver.rs 2, xref.rs 3, engine.rs 1) / test: 2 (xref.rs 1, engine.rs 1) | canonical | `crates/flpdf/src/reader/resolver.rs::attempt_recovery` | 2026-09-20（`flpdf-3yn9.48.176`）: B25 と同じ write-through seam（`CanonicalTrailerOwner` の `&self` + 内部可変性）でトレイトへ `attempt_recovery` を追加し、`xref.rs` の xref 読み込みロジック（`load_xref_state_from_source`/`load_xref_state_from_window` 内の `allow_repair` 分岐、計4箇所）が owner から直接読むよう揃えた。`XrefLoadOptions::allow_repair` フィールドと `engine.rs` 側の二重構築（resolver 構築時の `options.repair` とは別に `XrefLoadOptions { allow_repair: options.repair, ... }` を組んでいた箇所）を削除。qpdf の `m->attempt_recovery` は `parse()` エントリ（`QPDF.cc:463`）と resolve 時 retry（`QPDF.cc:1614-1637`）の両方が同じ `QPDF` インスタンス上の 1 bit を読む。flpdf も同じ `ResolverCore::attempt_recovery` を owner 経由で読み、B22 共通 owner operation を recovery policy が許可した両 entrypoint から呼ぶ |
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 32 | B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15, B16, B17, B18, B19, B20, B21, B22, B23, B24, B25, B26, B28, B30, B31, B33, B34, B35 |
| mixed | 3 | B27, B29, B32 |
| bridge | 0 | — |
| unknown | 0 | — |
