...
| B26 | `m->attempt_recovery`（回復分岐の on/off。分岐点は全ソースで 3 箇所） | `include/qpdf/QPDF.hh:1461`, `libqpdf/QPDF.cc:336,463,1391,1563` | `crates/flpdf/src/reader/resolver.rs::attempt_recovery`（`pub(crate)`） / open 時は `crates/flpdf/src/xref.rs::XrefLoadOptions` の `allow_repair` | `attempt_recovery(` prod: 1 (resolver.rs) / test: 0。`XrefLoadOptions` prod: 14 (xref.rs, engine.rs) / test: 60 | mixed | `crates/flpdf/src/reader/resolver.rs::attempt_recovery` | 同じ 1 bit が 2 つの型に分かれて存在する。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）: 3 つ目の表現だった `reader/file_object.rs::RecoveryPolicy`（`RequireEndstream` / `Bounded`、B11）は module ごと撤去し、`XrefLoadOptions::allow_repair` からの導出も消えた。残るのは resolver の `attempt_recovery` と open 時の `allow_repair` の 2 表現で、qpdf の分岐点 3 箇所に対する flpdf の分岐点は数え直しが要る。2026-09-19（`flpdf-3yn9.48.176`）: この 2 表現は B25 と同じ document-lifecycle 由来（`Pdf`/`ResolverHandle` 構築前後で state の持ち場が変わる）で、B20 の関数統合では解消しない——再確認のみ、コード変更なし |
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 27 | B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15, B16, B17, B18, B19, B20, B21, B23, B24, B28, B30, B31, B34 |
| mixed | 7 | B22, B25, B26, B27, B29, B32, B33 |
| bridge | 0 | — |
| unknown | 0 | — |
