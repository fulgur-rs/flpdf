...
| B9 | `QPDF::readObject`（`endobj` framing・`empty object treated as null`・`expected endobj` warn） | `libqpdf/QPDF.cc:1330-1357` | `crates/flpdf/src/reader/file_object.rs::parse_file_object_handle_syntax` + `crates/flpdf/src/reader/file_object.rs::finish_file_object_handle`（どちらも `pub(crate)`） | `parse_file_object_handle_syntax(` prod: 1 (xref.rs) / test: 2。`finish_file_object_handle(` prod: 1 (xref.rs) / test: 2 | mixed | `crates/flpdf/src/reader/file_object.rs::parse_file_object_handle_syntax` | `flpdf-3yn9.48.22`でreader.rsのlegacy直接callerは消滅。残るbootstrap callerは`xref.rs:735-742`で、canonical resolver側のframingはB8の`read_object_at_offset_with_description`内にある。 |
...
| B22 | `QPDF::reconstruct_xref`（再入禁止 → 3 連 warn → type 1 削除 → 行スキャン → 候補 xref stream からの trailer 復元 → 失敗時 throw） | `libqpdf/QPDF.cc:516-623` | `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan`（private、open 時） / `crates/flpdf/src/reader/resolver.rs::reconstruct_xref_and_retry`（private、resolve 時） | `recover_xref_from_linear_scan(` prod: 4 (xref.rs) / test: 0。`reconstruct_xref_and_retry(` prod: 1 (resolver.rs) / test: 2 | mixed | `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan` | qpdf は 1 実装を 2 箇所（`libqpdf/QPDF.cc:463` parse 直下、`:1617` `readObjectAtOffset` の再試行）から呼ぶ。flpdf は呼び出し箇所ごとに別実装で、resolve 側は qpdf の後半（候補 `/XRef` stream からの trailer 復元 `libqpdf/QPDF.cc:577-608` と `unable to find trailer dictionary while recovering damaged file` の throw `:610-616`）を実装しない。canonical open側のwarning channelは`.48.73`で`ResolverHandle::push_qpdf_warning`へ統合し、旧`DeferredDiagnosticsGuard`による二チャネル補正を撤去した。resolve側のcandidate trailer復元差とcompressed-entry probeは`flpdf-3yn9.48.19`の残範囲であり、owner-less loaderの第2 stateは`.48.72`で追跡する。 |
...
| B27 | `m->warnings`（**唯一の warning sink**。順序 = `warn` 呼び出し順、`push_back` のみ） | `include/qpdf/QPDF.hh:1475`, `libqpdf/QPDF.cc:487-494` | `crates/flpdf/src/reader/resolver.rs::ResolverCore` の `repair_diagnostics` フィールド（`crates/flpdf/src/reader/resolver.rs:357-366`）。canonical `Pdf::open` は`.48.73`で xref/recovery diagnosticsを`ResolverHandle::push_qpdf_warning`へ呼出順に配送し、engineのreplay/install bridgeを撤去。`.48.72`でowner-less public loader/exportとproduction callerも撤去し、`BootstrapHandleState` / `XrefReadContext` stagingはtest-only bounded reconstruction scaffoldingとして残る | 公開ドア `.repair_diagnostics()` — prod: 20 (flpdf-cli/main.rs, flpdf-qtest-tools 5 ファイル, engine.rs, job/check.rs, job/inspection.rs, job/lifecycle.rs, json/document.rs, reader.rs) / test: 162 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverCore` | `.48.12`の最初のconsumerはclassic trailerのdocument/cache identity。`.48.15.1`でcanonical `Pdf::open`のbootstrap cache保持、trailer/xref-stream rebind、第二teardownを撤去し、`.48.73`でwarning replay/deferred channelを撤去した。`.48.72`のpublic route撤去後に残るtest-only stagingはproduction document ownerの経路ではない。probeはP4を継続 |
