...
| B7 | `QPDF::readToken`（`allow_bad = true` 固定） | `libqpdf/QPDF.cc:1535-1539,1801-1814,846-946` | `crates/flpdf/src/xref.rs::read_trailer`（trailer dictionary後のlookahead） / `Tokenizer::next_integer`（ObjStm） / `ByteCursor::read_token`（classic xref） | `read_trailer` の `read_token(true, 0)` は qpdf の `readTrailer` 後段 lookahead を所有する。ObjStm と classic の別consumerは残る | mixed | `crates/flpdf/src/xref.rs::read_trailer`（trailer lookahead） | 2026-09-08: `read_trailer` が `stream keyword found in trailer` 判定だけ `allow_bad = true` で行う。`Tokenizer::next_integer` の header検査と `ByteCursor::read_token` の classic `readLine` 相当は別責務なので、一律に `true` へ変えない。B8-B11 の file-object/header移行と、残るObjStm consumerは後続slice。 |
