...
| B25 | `m->reconstructed_xref`（回復予算 = `bool` 1 個。2 回目は受け取った例外を再 throw） | `include/qpdf/QPDF.hh:1480`, `libqpdf/QPDF.cc:518-522,524-526` | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::begin_xref_reconstruction`（open / resolve 共通の owner guard） / `crates/flpdf/src/xref.rs::recover_xref_from_linear_scan`（open entry） | getter `reconstructed_xref` prod: 11 (engine.rs 1, resolver.rs 10) / test: 25 (reader.rs 1, resolver.rs 21, xref.rs 3)。`begin_xref_reconstruction` prod: 3 (resolver.rs 1, xref.rs 2) / test: 1 (xref.rs) | canonical | `crates/flpdf/src/reader/resolver.rs::begin_xref_reconstruction` | 2026-09-23（`flpdf-4wenf`）: qpdf は `reconstruct_xref` entry で guard を arm してから warning / scan に入る。flpdf の open-time と resolve-time も共有 helper を entry で呼び、再試行では warning を出さず元の trigger を返す。open-time は reconstruction scan / candidate re-entry 中の table 更新だけを scoped に許し、確定した raw table を owner に同期してから stale な外側 snapshot を拒否する。初回の9 test failureは nested reconstruction ではなく、この table update と stale snapshot の切り分け不足が原因だった。indirect `/Type` candidate probe では nested reconstruction は再現しなかった。B22 の mixed は reconstruct 処理自体が open / resolve の2実装に分かれている別軸で残る。B34 の qpdf に無い read-to-end 回数制限 bridge は独立の追跡対象。 |
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 30 | B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15, B16, B17, B18, B19, B20, B21, B23, B24, B25, B26, B28, B30, B31, B34, B35 |
| mixed | 5 | B22, B27, B29, B32, B33 |
| bridge | 0 | — |
| unknown | 0 | — |
