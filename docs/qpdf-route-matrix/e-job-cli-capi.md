...
|---|---|---|---|---|
| 0/1 | `qpdf/test_driver.cc:201-286` | `crates/flpdf-qtest-tools/src/driver/test_0_1.rs::run_test_0_1` | canonical | 2026-09-08（`.44`/`.25`）: E-27 の bridge（`filters::decode_stream_data_recovering_with_limits` 経由の DecodeParms warning/source-offset 再構成）を解消し、attribution を `ObjectHandle::try_get_parsed_offset` の canonical entrypoint 1 本へ寄せたうえで、旧 source-reread API 3 本と `source_stream_data_offset` を削除した。E-27 の canonical 化に合わせてこの行も canonical にする。 |
