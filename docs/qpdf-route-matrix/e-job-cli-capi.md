...
**caller 数え方の規約（領域 B / D と共通）**: 本ワークトリー（main `8fd1a2bf`）で
...
| E-9 | `QPDFJob::doListAttachments` / `doShowAttachment` / `addAttachments` / `copyAttachments` | `libqpdf/QPDFJob.cc:876-911`, `include/qpdf/QPDFJob.hh:531-532,540-541` | `crates/flpdf/src/job/attachments.rs::QPDFJob::list_attachments`（pub、`crates/flpdf/src/job/attachments.rs:289`）ほか同 impl の 5 メソッド | `list_attachments` prod: 1 (flpdf-cli/src/main.rs:7665) / test: 2 | mixed | `crates/flpdf/src/job/attachments.rs`（`QPDFJob` impl） | `QPDFJob` メソッド側は根拠 2 で legitimate。一方 free 関数 `format_attachment_list` / `format_attachment_list_with_sink` / `list_attachment_info` / `AttachmentInfo`（`crates/flpdf/src/job/attachment_list.rs`）のうち、`format_attachment_list_with_sink` だけは prod 2（`crates/flpdf/src/job/attachment_list.rs:239` の `format_attachment_list` 内、`crates/flpdf/src/job/attachments.rs:344` の `QPDFJob::list_attachments` 内 — いずれも `#[cfg(test)]`（`:380` / `:638`）より前）で caller ゼロの削除候補ではなく、残る 3 つは `src/` に prod caller が 1 つも無い — `format_attachment_list` は test 4、`list_attachment_info` は test 9（`crates/flpdf/src/job/attachment_list.rs` の `mod tests`（`crates/flpdf/src/job/attachment_list.rs:381` 以降）に 459,477,1032,1059,1080,1121 の 6 件、`crates/flpdf/src/job/attachments.rs` の `mod tests` に 908,1035,1119 の 3 件）に加え `crates/flpdf/examples/pull_attachments.rs:40` が 1 件。8 (B) の debt がそのまま残る → `flpdf-xsq1` |
...
| E-14 | `QPDFJob::parseRotationParameter` | `libqpdf/QPDFJob.cc:368-415`, `include/qpdf/QPDFJob.hh:482` | `crates/flpdf/src/job/rotate_spec.rs::RotateSpec::parse`（pub、`crates/flpdf/src/job/rotate_spec.rs:74`） | prod: 2 (flpdf/src/job/lifecycle.rs:1950, flpdf-cli/src/main.rs:5853) / test: 3 (flpdf/src/job/rotate_spec.rs:203,207,431) | mixed | `crates/flpdf/src/job/rotate_spec.rs::RotateSpec::parse` | 8 (D) の debt がそのまま残存 — qpdf 側は private メソッドのみで `QPDFJob` public の薄いラッパーも無いのに `main.rs` が直接呼ぶ。可視性は `flpdf-hxmj`、命名（`parse_rotation_parameter` になっていない）は `flpdf-ei0h` |
| E-15 | `QUtil::parse_numrange`（`QPDFJob::parseNumrange` は例外処理を足した薄いラッパー） | `include/qpdf/QUtil.hh:464`, `libqpdf/QPDFJob.cc:417-426` | `crates/flpdf/src/job/page_range.rs::PageRange::parse`（pub、`crates/flpdf/src/job/page_range.rs:94`）+ `crates/flpdf/src/job/page_range.rs::PageRange::resolve`（pub、`crates/flpdf/src/job/page_range.rs:131`） | `PageRange::parse` prod: 14 (flpdf-cli/src/main.rs 10 箇所: 4594,4625,4733,4742,4751,4886,4888,4895,4897,4906 + flpdf/src/job/lifecycle.rs:637 の 1 箇所 + flpdf/src/job/rotate_spec.rs:167,169,179 の 3 箇所。`crates/flpdf-cli/src/main.rs:4691` / `crates/flpdf/src/job/rotate_spec.rs:73,173` はコメント内の出現なので規約どおり除外) / test: 67 (flpdf/src/job/page_combine.rs 24, page_specs.rs 27, page_plan.rs 8, page_range.rs 6 の各 `mod tests` と flpdf/tests/job_lifecycle_tests.rs:210,238 の 2) |  mixed | `crates/flpdf/src/job/page_range.rs::PageRange` | `pub` 自体は 8 の根拠 1 で legitimate（qpdf 側 `QUtil::parse_numrange` が真に public）。ただし qpdf の 1 関数を `parse`（page count 不要）と `resolve`（page count 必須）へ 2 分割している点は qpdf の処理順序からの逸脱で、`flpdf-ei0h` に記録済み。route としては「構文検証だけ先に走らせる」CLI 経路と「解決まで一気に行う」job 経路の 2 本 |
...
| E-27 | `QPDFObjectHandle::getParsedOffset`（parsed offset の報告） | `include/qpdf/QPDFObjectHandle.hh:419`（outer class の `public:` は `include/qpdf/QPDFObjectHandle.hh:68` から）, `qpdf/test_parsedoffset.cc` | `crates/flpdf/src/reader.rs::Pdf::qtest_object_value_source_offsets`（pub、`crates/flpdf/src/reader.rs:861`）ほか同ファイルの `qtest_*` 5 メソッド | `qtest_object_value_source_offsets` prod: 2 (flpdf-qtest-tools/src/driver/test_0_1.rs:260, flpdf/src/reader.rs:844) / test: 0；`qtest_array_item_source_offsets` prod: 2 (同 :267, flpdf/src/reader.rs:908) / test: 0；`qtest_decode_parms_source_offset` prod: 1 (同 :282) / test: 0；`qtest_object_value_source_offset`（単数形）prod: 0 / test: 0；`qtest_array_item_source_offset`（単数形）prod: 0 / test: 0 | bridge | absent | qpdf は object handle 側に public な `getParsedOffset()` を置くが、flpdf は `Pdf` 側に `qtest_` 接頭辞つきの複数取り版を置いている（7 の「qpdf 識別子からの導出」に反する命名）。**別 crate の `flpdf-qtest-tools` からしか見えないため `pub(crate)` にできない**という構造的緊張がある（8 の「test-only な `pub` は `pub(crate)` へ」は同一クレート内テストを前提にしている）。単数形 2 本は caller ゼロ。複数形の `crates/flpdf/src/reader.rs:844` / `:908` という production caller は **その死んだ単数形からの内部委譲**なので、単数形を消せば複数形の production caller は qtest-tools からの 1 本だけになる。E-24 と同じ「閉じた枝」の形だが、こちらはテスト側 caller が無いため単数形 2 本は緊張と無関係に削除可能 |
| E-28 | `qpdf/test_driver.cc` consumer（`QPDF` / `QPDFObjectHandle` / helper の public を総当たりで叩く 99 ケース） | `qpdf/test_driver.cc:3540-3557`（`std::map<int, void (*)(QPDF&, char const*)> test_functions` に 0〜98 の 99 エントリ）, `qpdf/test_driver.cc:3559-3562` | `crates/flpdf-qtest-tools/src/driver/mod.rs::run`（`crates/flpdf-qtest-tools/src/driver/mod.rs:33`、`crates/flpdf-qtest-tools/src/driver/` は 15 ファイル 15,819 行） | prod: 1 bin（`crates/flpdf-qtest-tools/src/bin/driver.rs`）/ test: 3 (`crates/flpdf-qtest-tools/tests/driver_cli.rs`, `driver_goldens.rs`, `xref_parsedoffset_cli.rs`) | unknown | unknown | 本表（領域 E）は `QPDFJob` / CLI / C API の境界を対象としており、`test_driver.cc` が触る API 面は領域 A（ObjectHandle）・B（parser）・C（stream）に散る。**この consumer の route 分類は各領域の行が確定してからでないと決められない** — 必要な作業は「`driver/*.rs` が呼ぶ `flpdf::` 公開項目を全列挙し、A〜D の canonical owner 行と突き合わせる」こと（下記 probe P-2） |
| E-29 | `QPDFJob::createQPDF` の入力オープン側（`processFile` → `doProcess` → `doProcessOnce`。`QPDF` 構築直後に必ず `setQPDFOptions` を適用してから読む） | `libqpdf/QPDFJob.cc:428-481`, `libqpdf/QPDFJob.cc:1793-1804`, `libqpdf/QPDFJob.cc:1695-1716`, `libqpdf/QPDFJob.cc:650-666`（`noWarn` → `setSuppressWarnings` は `libqpdf/QPDFJob.cc:663-665`） | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::open_document`（pub、`crates/flpdf/src/job/lifecycle.rs:2303`）と `crates/flpdf/src/job/lifecycle.rs::QPDFJob::open`（pub、`crates/flpdf/src/job/lifecycle.rs:3438`）、および `crates/flpdf-cli/src/main.rs` 直書きの `Pdf::open_with_options` | `QPDFJob::open` prod: 5 (flpdf-cli/src/main.rs:3136,3575,5916,6637,7060) / test: 6 (flpdf/src/job/check.rs:2372,2412 は `:973` の `mod tests` 配下、flpdf/src/job/lifecycle.rs:3760,3774 は `:3739` の `mod tests` 配下、flpdf/tests/pdf_logger_tests.rs:232、flpdf/tests/fuzz_regression_tests.rs:25 — 後ろ 2 件は別 crate からの caller)；`QPDFJob::open_document` prod: 1 (flpdf/src/job/lifecycle.rs:2416) / test: 1 (flpdf/tests/job_lifecycle_tests.rs:321)；CLI 直書き `Pdf::open_with_options` prod: 4 (flpdf-cli/src/main.rs:3673,4872,6691,7736) / test: 0 | mixed | absent | **E-4 の書き出し側と対をなす読み込み側の二重正本で、3 経路ある。**(1) `QPDFJob::open_document`（erased `Box<dyn ReadSeek>`）— `options.suppress_warnings \|= self.suppress_warnings` を行い qpdf の `setQPDFOptions` の `noWarn` を再現する。(2) `QPDFJob::open`（generic `Pdf<R>`）— logger 設置・description 設定・`root_handle()`・`record_document_warnings` は同じだが **`suppress_warnings` の OR が無い**。qpdf は `doProcessOnce` から 1 本の `setQPDFOptions` を通すので、この差に対応物は無い。(3) CLI 直書き `Pdf::open_with_options` — job を通らないので logger も warning 記録も `suppress_warnings` も適用されない。用途は copy-encryption donor（`crates/flpdf-cli/src/main.rs:3673`）、overlay/underlay source（`:4872`）、encryption probe（`:6691`）、attachment copy source（`:7736`）。`crates/flpdf-cli/src/main.rs:6951` の `Pdf::create_from_json` も同型の非経由サイト（compliant な相方は `job.create_from_json_document`、`crates/flpdf-cli/src/main.rs:3040,3117`）。`crates/flpdf-cli/src/main.rs:7914` の `Pdf::open_mem_owned` は `mod tests`（`crates/flpdf-cli/src/main.rs:7778` 以降）内なので prod から除外、`:6844` は doc コメント |
...
| mixed | 20 | E-1, E-2, E-3, E-4, E-5, E-6, E-7, E-8, E-9, E-10, E-11, E-12, E-13, E-14, E-15, E-16, E-17, E-19, E-21, E-29 |
| unknown | 1 | E-28 |

（合計 29 行。canonical 5 + bridge 3 + mixed 20 + unknown 1 = 29。`E-18` は `mixed` に見えるが、CLI が使わないのは「別の正本がある」からではなく E-17 の帰結であるため `canonical`）
