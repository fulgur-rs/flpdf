...
| E-27 | `QPDFObjectHandle::getParsedOffset`（parsed offset の報告） | `include/qpdf/QPDFObjectHandle.hh:419`（outer class の `public:` は `include/qpdf/QPDFObjectHandle.hh:68` から）, `qpdf/test_parsedoffset.cc` | `crates/flpdf/src/reader.rs::Pdf::qtest_object_value_source_offsets`（pub、`crates/flpdf/src/reader.rs:840`）ほか同ファイルの `qtest_*` 3 メソッド | `qtest_object_value_source_offsets` prod: 1 (`flpdf-qtest-tools/src/driver/test_0_1.rs:260`) / test: 0；`qtest_array_item_source_offsets` prod: 1 (`同 :267`) / test: 0；`qtest_decode_parms_source_offset` prod: 1 (`同 :282`) / test: 0 | bridge | absent | qpdf は object handle 側に public な `getParsedOffset()` を置くが、flpdfはqtest helper用のplural batch APIを`Pdf`側に保持する。単数形2本は`.42`でcaller 0のdead wrapperとして削除し、plural APIとqtest-tools consumerは残す |
