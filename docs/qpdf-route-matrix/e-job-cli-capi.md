...
| E-27 | `QPDFObjectHandle::getParsedOffset` と stream DecodeParms type-warning の attribution（driver consumer） | `include/qpdf/QPDFObjectHandle.hh:419`, `libqpdf/QPDFObjectHandle.cc:1875-1882`, `qpdf/test_driver.cc` | `crates/flpdf/src/reader.rs::Pdf::qtest_object_value_source_offsets`（:903）、`qtest_array_item_source_offsets`（:947）、`qtest_decode_parms_source_offset`（:877）。いずれも feature-gated pub | 各 API prod: 0（`.44` で置換済み、以前は `crates/flpdf-qtest-tools/src/driver/test_0_1.rs:260,267,282`） / test: 0（直接 caller） | bridge | `crates/flpdf/src/object_handle.rs::ObjectHandle::try_get_parsed_offset`（`.28` で追加、dereference 契約は `get_parsed_offset` 直後に定義・実装済み）+ canonical stream warning owner（領域 C） | 単数形2本は `.42` で削除済み。2026-09-08（`.44`）: 残る3 API（と `Pdf::source_stream_data_offset`、A22 参照）を `driver/handle.rs::DecodeParamTypeWarning` の生成時点で `try_get_parsed_offset()` を直接キャプチャする形へ置換した——qpdf の `typeWarning`（`libqpdf/QPDFObjectHandle.cc:2168-2187`）自身が offending handle の `getParsedOffset()` を読むだけで、別経路での re-read を行わないのと同じ形。57件の golden fixture（`tests/driver_goldens.rs`）で byte-identical を確認。3 API・`source_stream_data_offset` とも prod caller 0 になったため、削除自体は `.25` に委ねる。CLI（`flpdf-cli/src/main.rs:7146`）・qtest metadata（`flpdf-qtest-tools/src/metadata.rs:261,286`）は `get_parsed_offset` の直接 caller として `.25`/`.37` で引き続き追跡する |
