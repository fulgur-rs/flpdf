...
| E-14 | `QPDFJob::parseRotationParameter` | `libqpdf/QPDFJob.cc:368-415`, `include/qpdf/QPDFJob.hh:482` | `crates/flpdf/src/job/rotate_spec.rs::RotateSpec::parse`（pub、`crates/flpdf/src/job/rotate_spec.rs:74`） | prod: 2 (flpdf/src/job/lifecycle.rs:1950, flpdf-cli/src/main.rs:5853) / test: 3 (flpdf/src/job/rotate_spec.rs:203,207,431) | mixed | `crates/flpdf/src/job/rotate_spec.rs::RotateSpec::parse` | 8 (D) の debt がそのまま残存 — qpdf 側は private メソッドのみで `QPDFJob` public の薄いラッパーも無いのに `main.rs` が直接呼ぶ。`flpdf-hxmj` は single-source 接続を完了して closed で、残る CLI caller の可視性整理は別 slice。命名（`parse_rotation_parameter` になっていない）は `flpdf-ei0h` |
| E-15 | `QUtil::parse_numrange`（`QPDFJob::parseNumrange` は例外処理を足した薄いラッパー） | `include/qpdf/QUtil.hh:464`, `libqpdf/QUtil.cc:1304-1438`, `libqpdf/QPDFJob.cc:399-425`, `libqpdf/QPDFJob_argv.cc:240-272`, `libqpdf/QPDFJob_config.cc:1055-1074` | `crates/flpdf/src/job/page_range.rs::PageRange::parse`（pub、:94）+ `PageRange::resolve`（pub、:131） | CLI、job JSON、RotateSpec、page plan/combine/specs と各テスト | mixed | absent（signed `max` を取る単一 `parse_numrange` 契約は未確立） | **2026-09-06 oracle 訂正**: qpdf 自身が pages の file/range 判別、rotation、underlay/overlay 設定で `parse_numrange(..., 0)` により page count 未確定の構文検証を行う。先行検証自体を flpdf 固有の処理順序とする旧記述は誤り。残る差は Rust AST の `parse`/`resolve` 分割、空文字の全ページ sentinel、zero-page guard、signed/max=0、エラー契約であり、canonical primitive の differential で個別に照合する。`pub` の根拠1は契約の等価性まで正当化しない。`flpdf-ei0h` の命名と semantic comment は分離して扱う |
...
| E-28 | `qpdf/test_driver.cc` consumer（`QPDF` / `QPDFObjectHandle` / helper の public API を呼ぶ 99 ケース） | `qpdf/test_driver.cc:3540-3562` | `crates/flpdf-qtest-tools/src/driver/mod.rs::run` と `driver/*.rs` | prod: 1 bin（`crates/flpdf-qtest-tools/src/bin/driver.rs`）/ test: `driver_cli.rs`, `driver_goldens.rs`, `xref_parsedoffset_cli.rs` | mixed | case/API ごとの領域 A〜D owner（未照合の個別経路は unknown） | **2026-09-06 再分類**: canonical handle caller に加え、test 0/1 に `filters::decode_stream_data_recovering_with_limits` と DecodeParms warning/source-offset 再構成（E-27）が残るため、consumer 全体は mixed と判定できる。未照合 case の所有者は引き続き unknown。imports、型経由メソッド、呼出し順序まで case/API ごとに A〜D owner と照合する（P-2）。A〜D の全行確定を待つ必要はない |
...
| unknown | 0 | —（E-28 の未照合個別経路は unknown のまま） |
...
| P-2 | E-28 の未照合 case/API の owner と最初の bounded consumer 移行 | `driver/*.rs` の `flpdf::` 完全修飾参照に imports・型経由メソッド・実際の呼出し順序も加えて列挙し、qpdf の同じ case と A〜D owner に対応付ける。test 0/1 の stream warning bridge は既知の mixed、未照合経路だけ unknown とする。caseごとに既存issue、canonical prerequisite、残callerを記録する |
