...
| 34 | `qpdf/test_driver.cc:1252-1265` | `crates/flpdf-qtest-tools/src/driver/test_34_41.rs::run_test_34` | mixed | `root_handle`/`get_key` は canonical だが、**未追跡ギャップ、`flpdf-wd2e` で追跡開始**: `version_prefix_major_minor`（`QPDF::getVersionAsPDFVersion` の regex 相当）に production 対応物が無い（`Pdf::version()` は生ヘッダ文字列のみ、`crates/flpdf/src/pdf.rs:305`）。`catalog_extension_level` の方は `Pdf::adobe_extension_level`（`pub`、`crates/flpdf/src/pdf.rs:409`）が direct `/Root` 受理と `/Extensions` → `/ADBE` → `/ExtensionLevel` の全段解決を既に持ち、qpdf の `getExtensionLevel`（`libqpdf/QPDF.cc:2329-2343`）との差は「欠損時 `None` vs `0`」と「`getIntValueAsInt` の int クランプ + warning が無い」の 2 点のみ。 |
...
| 86 | `qpdf/test_driver.cc:3065-3085` | `crates/flpdf-qtest-tools/src/driver/test_80_87.rs::run_test_86` | mixed | **未追跡ギャップ、`flpdf-jzj1` で追跡開始**: `QUtil::utf8_to_pdf_doc` に対応物が無く assertion 1 件が未移植。`utf8_to_ascii` は `flpdf::qutil::utf8_to_ascii`（`pub`、`crates/flpdf/src/qutil.rs:205`、`lib.rs:139` が `pub mod qutil`）として実在し値の assertion は移植可能で、qpdf の 2 引数版が返す representability の `bool` だけが signature 上落ちている（driver 自身の doc comment に明記）。残りは `filespec_helper::encode_utf16be`/`pdf_string::utf8_value`/`pdf_string::new_unicode_string`（全 `pub`）。 |
...
| 98 | `qpdf/test_driver.cc:3425-3450` | `crates/flpdf-qtest-tools/src/driver/test_88_98.rs::run_test_98` | bridge | **未追跡ギャップ、`flpdf-wkju` で追跡開始**: 関数本体が `Ok(())` のみの完全未実行 stub。必要な API（`ObjectHandle::write_json`/`get_json`/`write_stream_json` の per-object オーバーロード、`object_handle.rs:6173,6957,6981`）は全て `pub(crate)` 限定でこの crate から到達不能。qpdf 挙動を一切 exercise していない。 |
...
| P-2 | **解決済み（2026-09-07）**: E-28 の case/API owner 対応付け | `driver/*.rs` の 99 ケース全件を imports・型経由メソッド・実際の呼出し順序を含めて qpdf の同じ case と A〜D owner に対応付けた。結果は「E-28 detail」表（`canonical` 29 / `mixed` 65 / `bridge` 4 / `unknown` 0）。未追跡だった 6 件のギャップは新規 issue 化（`flpdf-83jc`/`flpdf-wd2e`/`flpdf-jzj1`/`flpdf-6f6h`/`flpdf-cm84`/`flpdf-wkju`）。実装cutover自体は各 issue の担当範囲で別途行う（本 issue の対象外） |
