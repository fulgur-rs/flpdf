...
| E-11 | `QPDFJob::handleUnderOverlay` / `doUnderOverlayForPage` | `libqpdf/QPDFJob.cc:1936-2043`, `libqpdf/QPDFJob.cc:1858-1911` | `crates/flpdf/src/job/overlay.rs::handle_under_overlay`（pub free、`crates/flpdf/src/job/overlay.rs:531`）と `crates/flpdf/src/job/overlay.rs::overlay_verbose_report`（pub free、`crates/flpdf/src/job/overlay.rs:633`） | `handle_under_overlay` prod: 2 (`crates/flpdf/src/job/lifecycle.rs:3726`, `crates/flpdf-cli/src/main.rs:8272`) / test: 9；`overlay_verbose_report` prod: 2 (`crates/flpdf/src/job/lifecycle.rs:3723`, `crates/flpdf-cli/src/main.rs:8268`) / test: 0 | mixed | `crates/flpdf/src/job/overlay.rs::handle_under_overlay` | 現行 main では Job 内の create-stage callerに加え、`--pages` post-plan consumerだけが CLI から free helper を直接呼ぶ。`flpdf-xsq1` は内部 helper の可視性整理に限定され、この direct caller は未移行。overlay source の open/保持も `lifecycle.rs` と CLI page-selection route に分かれており、qpdf の `createQPDF` → `handleUnderOverlay` owner への完全統合は `flpdf-3yn9.48.92` で追跡する。top-level inspection は `flpdf-clpz`/`flpdf-p4og4` で QPDFJob routeへ接続済み。 |
