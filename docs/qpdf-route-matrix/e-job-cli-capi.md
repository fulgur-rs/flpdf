...
| `E-28` | `flpdf-wd2e` | case 34: `getVersionAsPDFVersion`相当が未実装。`getExtensionLevel`は`Pdf::adobe_extension_level`（`pub`）が既に担い、差は「欠損時None vs 0」と「intクランプ+warning無し」の2点のみ |
