...
|---|---|---|---|---|---|---|---|
| E-1 | `QPDFJob::run`（`createQPDF` → `writeQPDF` の 2 段） | `libqpdf/QPDFJob.cc:513-520`, `include/qpdf/QPDFJob.hh:371-373` | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::run`（pub、`crates/flpdf/src/job/lifecycle.rs:2756`） | prod: 8 (flpdf-cli/src/main.rs, flpdf-qtest-tools/src/driver/test_80_87.rs, flpdf-qtest-tools/src/bin/qpdfjob_ctest.rs) / test: 53 | mixed | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::run` | flpdf の `run` は `create_qpdf` → `run_document_erased`（private）で、**`write_qpdf` を呼ばない**。qpdf の `run` は `createQPDF` → `writeQPDF` の 2 呼び出しだけ。CLI からの `run` 到達は `crates/flpdf-cli/src/main.rs:2883`（`run_job_json_file`、`--job-json-file` 経路）**1 箇所のみ** |
| E-2 | `QPDFJob::createQPDF`（`checkConfiguration` → `processFile` → `updateFromJSON` → `handlePageSpecs` → `handleRotations` → `handleUnderOverlay` → `handleTransformations`） | `libqpdf/QPDFJob.cc:428-481` | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::create_qpdf`（pub、`crates/flpdf/src/job/lifecycle.rs:2551`） | prod: 3 (flpdf/src/job/lifecycle.rs, flpdf-qtest-tools/src/bin/qpdfjob_ctest.rs) / test: 11 | mixed | absent | `.48.3` で `create_qpdf` は単一primaryの `updateFromJSON` と `handleRotations` を完了してから返す。page-spec source ownershipとunder/overlay・残transformのrun consumerは段階移行として `run_document_erased` / `run_document_stages` に残り、public create/writeを直接使うpage-spec consumerは後続sliceで移す。RED/GREENは `job_lifecycle_tests::create_qpdf_returns_the_primary_after_rotation_transformation` で固定した |
| E-3 | `QPDFJob::writeQPDF` の 3 分岐（`!createsOutput()`→`doInspection` / `split_pages`→`doSplitPages` / else→`writeOutfile`） | `libqpdf/QPDFJob.cc:483-511`, `libqpdf/QPDFJob.cc:528-532` | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::write_qpdf`（pub、`crates/flpdf/src/job/lifecycle.rs:2619`） | prod: 2 (flpdf/src/job/lifecycle.rs, flpdf-qtest-tools/src/bin/qpdfjob_ctest.rs) / test: 4 | mixed | absent | 3 分岐は `write_qpdf` の中ではなく `run_document_stages` の末尾（`crates/flpdf/src/job/lifecycle.rs:2902-2933`）にあり、しかも qpdf と条件が違う — flpdf は「inspection フラグ 10 種の OR」→「`json_version`」→「`check` または出力先なし」→「`write_qpdf`」の 4 段。qpdf は `createsOutput()` 1 個で分ける。flpdf の `write_qpdf` 自身は qpdf の `writeOutfile` + `doSplitPages` 分岐 + 完了処理を畳んだもの（E-4 / E-5） |
...
| E-19 | `QPDFJob::getExitCode` / `hasWarnings` / `createsOutput` | `libqpdf/QPDFJob.cc:522-564` | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::complete`（pub）+ `crates/flpdf/src/job/lifecycle.rs::QPDFJob::has_warnings`（pub）; document warning API は `crates/flpdf/src/reader.rs::Pdf::get_warnings` / `any_warnings` / `num_warnings` | `complete` prod: 13 (flpdf/src/job/check.rs 2, flpdf/src/job/lifecycle.rs 5, flpdf-cli/src/main.rs 6) / test: 5；`has_warnings` prod: 8 (flpdf-cli/src/main.rs 6, flpdf-qtest-tools/src/driver/test_80_87.rs 2) / test: 6 | mixed | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::complete` + `crates/flpdf/src/reader/resolver.rs::ResolverCore` | qpdf は document operation 後に `getWarnings()` を drain して job warning stateへ取り込み、`getExitCode()` は状態を 1 回だけ判定する。flpdf は `record_document_warnings` を intermediate の `any_warnings`、`drain_document_warnings`（`crates/flpdf/src/job/lifecycle.rs:3891-3905`）を completion の drain として分離した。`inspect` / JSON / write / check / linearization の Job completion はこの境界を通るが、CLI direct completion と複数 inspection の全体集約は後続 E-7/E-9/E-10 consumer cutoverに残る。`--is-encrypted` / `--requires-password` の 4 値 exit code は `run_encryption_status` が別に持つ |
...
| E-27 | `QPDFObjectHandle::getParsedOffset` と stream DecodeParms type-warning の attribution（driver consumer） | `include/qpdf/QPDFObjectHandle.hh:419`, `libqpdf/QPDFObjectHandle.cc:1875-1882`, `qpdf/test_driver.cc` | `crates/flpdf/src/reader.rs::Pdf::qtest_object_value_source_offsets`（:903）、`qtest_array_item_source_offsets`（:947）、`qtest_decode_parms_source_offset`（:877）。いずれも feature-gated pub | 各 API prod: 1 (`crates/flpdf-qtest-tools/src/driver/test_0_1.rs:260,267,282`) / test: 0（直接 caller） | bridge | `crates/flpdf/src/object_handle.rs::ObjectHandle::try_get_parsed_offset`（`.28` で追加、dereference 契約は `get_parsed_offset` 直後に定義・実装済み）+ canonical stream warning owner（領域 C） | 単数形2本は `.42` で削除済み。残る3 APIは driver test 0/1 が DecodeParms warning を再構成するため source を再読する経路で、`test_parsedoffset` consumer の APIではない。`.28` で qpdf の dereference-forcing契約を持つ `try_get_parsed_offset` が実装済みなので、単純置換の候補はここにある——ただし本エントリの3 APIは a source re-read 経路であり `ObjectHandle::get_parsed_offset` 系のconsumerではないため、置換の要否はA22の read-window bridge・領域Cのdecode/warning consumer移行と合わせて判断する。CLI（`flpdf-cli/src/main.rs:7146`）・qtest metadata（`flpdf-qtest-tools/src/metadata.rs:261,286`）は `get_parsed_offset` の直接 caller として `.25`/`.37`/`.44` で引き続き追跡する |
