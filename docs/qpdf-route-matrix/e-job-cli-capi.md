...
| E-7 | `QPDFJob::doInspection`（10 分岐を逐次実行し最後に 1 回だけ warning/完了） | `libqpdf/QPDFJob.cc:1645-1693` | `crates/flpdf/src/job/lifecycle.rs::run_configured_inspection`（private、`:4017`。**2026-09-19 再訂正**: `:3211` も `:4005` も別のコードを指す stale。`:4005` は `OverlayKind` の match 行） | prod: 2（`write_qpdf` `lifecycle.rs:3354` と `inspect_configured` `:3699`。**2026-09-19 再訂正**: `prod: 1` は既に一度直したものが merge で巻き戻っていた） / test: 0 | mixed | `crates/flpdf/src/job/lifecycle.rs::run_configured_inspection` | 分岐順序は qpdf と 1:1で、report helperは完了せず `write_qpdf` が全分岐後に warning drain・summary・memory reportを1回だけ行う。CLIの個別 inspection public APIは別の standalone consumerとして残るため、領域全体の classification は mixed。**2026-09-19（`flpdf-3yn9.48.186`）**: qpdf の argv 側は選択されたinspection flagの個数に関わらず常に同一の `initializeFromArgv`→`run()`→`getExitCode` dispatchを使う（フラグ数固有の分岐は無い）ため、flpdf-cli 側の top-level 単一フラグ standalone dispatch を `top_level_inspection_combination_requested`／`run_combined_top_level_inspection`（既存の canonical `job.config()`+`job.run()` 経路）へ統合した。対象10フラグ中9個（`--check`/`--show-npages`/`--show-pages`/`--show-xref`/`--check-linearization`/`--show-linearization`/`--show-encryption`/`--list-attachments`/`--show-attachment`）はこの統合で standalone 関数（`run_check`/`run_show_npages`/`run_show_pages`/`run_show_xref`/`run_check_linearization`/`run_show_linearization`/`run_show_encryption`/`run_list_attachments`/`run_show_attachment`）の top-level flag呼び出し経路を撤去した（**2026-09-19 訂正**: 9 関数すべてが残るわけではない。`run_show_xref`/`run_show_linearization`/`run_list_attachments`/`run_show_attachment`/`reserve_show_attachment_output` は本変更で削除され、`main.rs` 内の参照は 0 になった。残る 5 つ（`run_check`/`run_check_linearization`/`run_show_npages`/`run_show_pages`/`run_show_encryption`）だけが、qpdf に対応物の無い flpdf-native `Commands::` subcommand 面から呼ばれるため残る——qpdf自身は subcommand grammarを持たないため、この subcommand 面はそもそも E-7 の対象外）。**`--show-object` は対象外のまま**: 共有 `JobObjectSelector`/`parse_job_object_selector`（`crates/flpdf/src/job/lifecycle.rs`）が generationを `u16` にclampし raw-identity resolution（`Pdf::get_object_handle_by_raw_identity`）を経由しないため、CLI 側の `ShowObjectSelector`/`parse_show_object_selector` が受理する generation>65535 の raw identity selectorをcombined経路に流すと qpdf と乖離する（`cli_raw_xref_identity.rs::show_object_accepts_a_raw_generation_like_qpdf` で実証）。この乖離は `crates/flpdf` 側の修正を要するため `flpdf-t3as9` で別途追跡し、standalone `run_show_object` の top-level呼び出しはそれまで維持する。領域全体の classification は（`--show-object` と subcommand 面の standalone consumer が残るため）引き続き mixed。 |
