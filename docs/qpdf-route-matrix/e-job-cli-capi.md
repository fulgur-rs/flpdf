...
| E-4 | `QPDFJob::writeOutfile`（`replace_input` 前後処理、`json_version` 分岐、`QPDFWriter` ブロックスコープ、`setWriterOptions` → `write()`） | `libqpdf/QPDFJob.cc:3029-3091` | `crates/flpdf/src/job/lifecycle.rs::QPDFJob::write_qpdf`（pub）+ `crates/flpdf-cli/src/main.rs::write_with_pdf_writer`（private） | `write_with_pdf_writer` prod: 1 (`crates/flpdf-cli/src/main.rs:8780`) / test: 0；`write_qpdf_to_memory` prod: 2 (flpdf-cli/src/main.rs) / test: 0 | mixed | absent | **本領域で最大の二重正本**。CLI の通常出力は `QPDFJob` を通らず `main.rs` 自身の `PdfWriter` 経路を一部辿る。**2026-09-17（`flpdf-3yn9.48.143`）**: `qdf` サブコマンドの直接 `PdfWriter` consumerを撤去し、qpdf の qdf writer optionを `run_rewrite` → `QPDFJob::create_qpdf` → `write_qpdf` へ接続した。残る直接 writer は rewrite page-extraction の ordinary branchで、E-4全体は mixed のまま。`write_qpdf_to_memory` は `crates/flpdf/src/writer.rs` の同名 `pub(crate)` 関数と別物で、test-only scaffolding は production direct writer callerに混ぜない。既存の replace-input/write-stage cutover（`flpdf-3yn9.48.4`）と、旧 10 caller の履歴は保持する。|
