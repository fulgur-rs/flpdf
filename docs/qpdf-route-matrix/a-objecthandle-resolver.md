...
|---|---|---|---|---|---|---|---|
| A1 | `QPDF::Members::obj_cache`（object identity の唯一の正本） | `include/qpdf/QPDF.hh:1467`、`include/qpdf/QPDF.hh:868-889` | `crates/flpdf/src/reader/resolver.rs::ResolverCore::object_cache`（`BTreeMap<QpdfObjGen, ObjectHandle>`、private） | prod: canonical resolver only / test: 0 | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverCore::object_cache` | `flpdf-3yn9.48.22` で facade cache と bootstrap/provenanceの二重帳簿を撤去し、`flpdf-3yn9.48.78` で key/handle identity を raw `QpdfObjGen` へ揃えた。`ObjectRef` は valid parser/public projection に留まり、source-xref keys と canonical handles は同じ qpdf-shaped document state の view として扱う。 |
| A2 | `QPDF::ObjCache`（`{object, end_before_space, end_after_space}` の 3 フィールド、private nested class） | `include/qpdf/QPDF.hh:868-889` | `crates/flpdf/src/reader/resolver.rs::ResolverCore::object_cache` | prod: canonical resolver only / test: 0 | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverCore::object_cache` | `flpdf-3yn9.48.22` で qpdfにない6状態の `CacheEntry` facade、公開 `cache` module/export、compatibility cacheを削除。offset metadataはcanonical handle slot側に残る。`flpdf-rbja1` では `read_xref` 後の lower generation cleanup（`libqpdf/QPDF.cc:650-725`）も exact raw key で canonical cache から除去し、object 0 の raw row は projectionせず cache に保持する。 |
| A3 | `QPDF::getObject(QPDFObjGen)`（**resolve しない** handle 取得） | `libqpdf/QPDF.cc:1951-1959`、`include/qpdf/QPDF.hh:362-372` | `crates/flpdf/src/reader.rs::Pdf::get_object_handle`（`pub`）→ `crates/flpdf/src/reader/resolver.rs::ResolverHandle::get_object_handle` / `get_object_handle_qpdf_obj_gen`（`pub(crate)`） | `rg -n '\.get_object_handle\(' crates`（facade 経由と resolver 直呼びの合算）prod: 257 (66 files; うち `crates/flpdf-qtest-tools` の driver 群 `test_10_17.rs` 他) / test: 533 | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::get_object_handle_qpdf_obj_gen` | `Pdf::get_object_handle` は raw `QpdfObjGen` key への 1 行委譲で、`or_insert_with` が qpdf の `if (!isCached(og)) { obj_cache[og] = ObjCache(QPDF_Unresolved::create(...)) }` に 1:1 対応する。`ObjectHandle::object_ref()` は valid indirect-reference projection、`qpdf_obj_gen()` は raw header/cache identityであり、resolve を起こさない契約も守られている。 |
| A4 | `QPDF::resolve(og)`（loop warning → xref type dispatch → catch して warn → null fallback） | `libqpdf/QPDF.cc:1699-1753` | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::resolve_indirect` / `resolve_qpdf_obj_gen`（`DocumentResolver` の impl） | `rg -n --glob '*.rs' '[.:]resolve_indirect\(' crates` → prod: **1**、`crates/flpdf/src/object_handle.rs`（`try_dereference` 内）/ test: existing resolver tests | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::resolve_qpdf_obj_gen` | `ResolveMark` が qpdf の `ResolveRecorder` と同じ raw `QpdfObjGen` keyを保持する。valid `ObjectRef` 呼び出しは raw identityへ昇格し、generation 65536 の header/cacheも同じ dispatch・catch・null fallbackを通る。 |
...
| A6 | 型アクセサが暗黙に dereference する（`asInteger`/`asDictionary`/`isNull` 等が全て `dereference()` を通る） | `libqpdf/QPDFObjectHandle.cc:240-446` | 2 族が併存: **解決する** `try_*` 族（`crates/flpdf/src/object_handle.rs::ObjectHandle::try_as_integer` 等、`pub(crate)`/`pub`）と、**解決しない** `as_*`/`is_null` 族（`crates/flpdf/src/object_handle.rs::ObjectHandle::as_dictionary` 等、`pub`、`crates/flpdf/src/object_handle.rs:3821-3906`） | 非解決族 prod 合計 679: `as_dictionary` 189 / `is_null` 163 / `as_array` 126 / `as_integer` 74 / `as_name` 53 / `as_string` 53 / `as_real` 21。test 合計 1053。解決族 `try_as_integer` prod: 28 / test: 6 | mixed | `crates/flpdf/src/object_handle.rs::ObjectHandle::try_as_integer`（`try_*` 族） | **本領域最大の二重経路**。qpdf は `asInteger()` も `isNull()` も必ず `dereference()` を通すので、未解決の間接 handle でも正しい型/値を返す。flpdf の `as_*`/`is_null` は doc 自身が "never performs resolution itself" と明記し、未解決の間接 handle に `None`/`false` を返す（`crates/flpdf/src/object_handle.rs:3860-3878`）。同じ handle に対して 2 族が **異なる答え**を返しうるのが mixed の実体。この差を埋めるために呼び出し側が A7 を前置している。2026-09-08（`flpdf-3yn9.48.29`、reader/resolver cohort）: 本領域自身のファイル（`pdf.rs`/`reader.rs`/`reader/resolver.rs`/`reader/file_object.rs`/`cache.rs`）内の A7 前置 caller を全数点検し、`pdf.rs::extension_level_handle`（3 箇所）・`pdf.rs::root_handle`・`reader.rs::remove_security_restrictions`（2 箇所）を `try_as_dictionary`/`try_is_dictionary`/`try_as_integer` へ寄せ、前置していた `self.resolve(...)` を全廃した。あわせて `reader.rs::encrypt_dictionary_handle` の `encrypt.is_null()`（非解決族）を `try_is_null()` に修正 -- qpdf は `m->trailer.hasKey("/Encrypt")`（`libqpdf/QPDF_encryption.cc:729`、resolved-null を absent 扱いする `QPDF_Dictionary::hasKey`)で判定するため、間接参照が null に解決する `/Encrypt` では非解決 `is_null()` が誤答しうる。ただし `engine.rs`の`open_with_repair_mode_as`が`initialize_encryption_inspection`（既に`try_is_null()`を使用）を`authenticate_if_encrypted`より先に必ず呼ぶため、共有 object cache 経由でこの経路は現行の呼び出し順序では到達不能（`/tmp/qpdf-probes/encrypt-resolves-null-clean.pdf` で qpdf 実行環境・flpdf 双方とも修正前後で "File is not encrypted" のまま変化なしと確認済み）。`reader/resolver.rs`/`reader/file_object.rs`/`cache.rs` 自身の prod code には A7 前置 caller が無く（前者の非解決 `is_null()` 3 箇所は resolver 内部の再入回避チェックで、後者の `as_dictionary()` 2 箇所は direct-parse 結果への型観察でどちらも AC の「parserのdirect-only型観察は再入回避契約として保持」に該当）、本 cohort の bridge caller は 0 件になった。次 cohort（`.48.30`〜`.48.33`）の残 caller数は未再計測。2026-09-08（`flpdf-3yn9.48.30`、page/object helper cohort 第1スライス）: `crates/flpdf/src/page_object_helper.rs` の共有継承属性 walk 3 関数（`resolve_attribute_target`/`get_attribute_for_target`/`resolve_inherited_rotate_with_max_depth`、qpdf `QPDFPageObjectHelper::getAttribute` 相当、`libqpdf/QPDFPageObjectHelper.cc:236-247`）の A6/A7 bridge を `try_as_dictionary`/`try_as_name`/`try_is_null` へ寄せた。同ファイルの残りは前スライス後の再計測で `.resolve(` 19 / `resolve_handle`系 9 / `as_x`系 45 / `get_key`系 6（prod のみ、`#[cfg(test)] mod tests` 開始行より前）で、他の page/object helper ファイル（`annotation_object_helper.rs`/`page_splice.rs`/`page_annotation_flatten.rs`/`page_form_xobject.rs` 等）は未着手のまま `flpdf-3yn9.48.70` に切り出した。 2026-09-08（`flpdf-3yn9.48.70`、page/object helper cohort の `annotation_object_helper.rs` 全体）: 同ファイル全9箇所の A6/A7 bridge（`resolved_key`/`get_appearance_stream`/`array_as_rectangle`/`rectangle_from_handle`/`matrix_from_handle`）を閉じ、`rectangle_from_handle`/`matrix_from_handle`/`as_number` は resolve 不要になったため `&mut self` を落として自由関数化した。実装中に「`try_get_key` は受信側 (self) だけを resolve し、返す子 handle 自体は resolve しない」という誤解に基づく regression（`get_appearance_stream` で `ap_sub`/`ap_sub_val` の明示 resolve を誤って削除）を作り込み、`page_annotation_flatten` の既存テスト23件の失敗で検出・訂正した——`try_get_key`/`try_get_keys` 等「self を resolve してから self の値を見る」アクセサと、`try_get_key` のように「self を resolve するが返す子は resolve しない」アクセサを混同しないこと。 2026-09-19（`flpdf-3yn9.48.204`、`job/*.rs` cohort、stale count 訂正）: issue 記載の「`job/page_range.rs`(8)/`job/lifecycle.rs`(6)/`job/overlay.rs`(3)/`json/handler.rs`(1)/`job/page_plan.rs`(1)/`job/json_sections.rs`(1)、計20箇所の A6/A7 bridge」は `.resolve(` 部分文字列一致による誤集計で、実際の内訳は全て非 ObjectHandle: `page_range.rs`/`lifecycle.rs`/`overlay.rs`/`page_plan.rs` の `.resolve(` 計18箇所は数値範囲を解決する `PageRange::resolve(page_count: u32) -> Result<Vec<u32>>`（`job/page_range.rs:96`、`QUtil::parse_numrange` の解決モードに対応、A6/A7 とは無関係）、`json/handler.rs` の1箇所は `Rc`/`Weak` ディスパッチの `HandlerEdge::resolve`（`json/handler.rs:28`、`Weak::upgrade` 相当）、`json_sections.rs` の1箇所は production に bridge が無いことを検証する既存 regression guard `json_section_production_uses_resolving_handle_accessors`（`job/json_sections.rs:1174-1197`）内の文字列リテラル `"pdf.resolve("` 自身。`python3 scripts/qpdf-route-callers.py --symbol resolve --symbol is_null --symbol as_dictionary --symbol as_array --symbol as_integer --symbol as_name --symbol as_string --symbol as_real --symbol get_key --symbol has_key` で6ファイルを再計測しても同じ内訳（`resolve` の prod caller は `overlay.rs`3/`page_plan.rs`1/`json/handler.rs`4、いずれも上記と同一の非 ObjectHandle 呼び出し）で、非解決アクセサ（`as_*`/`is_null`/`get_key`/`has_key`）の ObjectHandle 呼び出しは `job/overlay.rs:725` の `value.is_null()` 1 箇所のみ実在した。これは `PageObjectHelper::get_media_box`/`get_trim_box`（`page_object_helper.rs:504-548` 経由 `get_attribute_for_target`）が返す `ObjectHandle` に対する呼び出しで、両経路とも `copy_if_shared=false`/`copy_if_fallback=false`（overlay.rs の呼び出し引数）では必ず解決済みで返ることを確認した——`get_attribute_for_target` 内の `result.try_is_null()?`（`page_object_helper.rs:2175`）は `&&` の左辺として常に評価されるため `get_attribute` の戻り値は常に解決済みで、`get_trim_box` が `/TrimBox`→`/CropBox`→`/MediaBox` とフォールバックする経路でも各段の `get_attribute` 呼び出しがそれぞれ解決を強制し、`apply_fallback`（`page_object_helper.rs:804-827`）は `copy_if_fallback=false` の場合、その論理和条件の短絡評価により解決済み handle をそのまま素通しする（新規 unresolved handle は生成しない）。観測可能な出力差分は無いが、`page_object_helper.rs` の姉妹アクセサ（`get_crop_box`/`get_bleed_box`/`get_trim_box` はいずれも同じ戻り値に対して `try_is_null()` を明示している）と同じ防御的規約に揃える構造的整合として `try_is_null()?` へ移行した（`job/overlay.rs:725`、テストは同ファイル `mod tests` の `page_box_or_err_rejects_a_page_with_no_media_box_in_its_ancestry`/`page_box_or_err_accepts_a_page_with_a_media_box`）。この6ファイルの production コードは以上で A6/A7 bridge callers が 0 件と確認できたが、A6 行の canonical 化には受け入れ基準が要求する crate 全体での bridge-caller-zero が必要で、まだ満たされない。**2026-09-19 訂正**: ここは当初 `as_dictionary` 189 / `as_array` 126 を挙げていたが、それは同じ行の「非解決族 prod 合計 679」列の値で、`is_null` の 74 だけ script 由来という混成だった。`python3 scripts/qpdf-route-callers.py --root . --symbol as_dictionary --symbol as_array --symbol is_null` を本コミットで実行した値は `as_dictionary` prod 39（17 ファイル）/ `as_array` prod 43（22 ファイル）/ `is_null` prod 74（33 ファイル）。以降この段落は script の集計方法に統一する（679 列は別方法の既存値なのでこの PR では触らない）。crate 全体の残 cohort 洗い出しは follow-up（`flpdf-3yn9.48.208`）に切り出した。 2026-09-23（`flpdf-3yn9.48.208.1`）: `writer/plain/body.rs` の production `as_dictionary` 4件と `as_array` 5件を resolving `try_as_*` へ移行。`qpdf-route-callers.py` の fresh workspace inventory（production 260件、`flpdf-qtest-tools` を含む）は `as_dictionary` 32/15 files, `as_array` 32/20, `as_integer` 20, `as_name` 25, `as_string` 46, `as_real` 16, `is_null` 75, `get_key` 11, `has_key` 3。A6 は mixed のままで、残callerは親 `flpdf-3yn9.48.208` の対象。 2026-09-24（`flpdf-3yn9.48.208.2`）: `flpdf-qtest-tools/src/clean.rs` の `as_array` 1 / `as_dictionary` 1 / `get_key` 2 / `has_key` 2 を resolving accessor へ移行し、このファイルの対象 production caller は 0。fresh workspace inventory（`flpdf-qtest-tools` 含む）は合計 254: `as_dictionary` 31 / `as_array` 31 / `as_integer` 20 / `as_name` 25 / `as_string` 46 / `as_real` 16 / `is_null` 75 / `get_key` 9 / `has_key` 1。A6 は mixed のまま。 |
| A7 | `QPDF::resolve` は `QPDFObject` 内部だけが呼ぶ private resolver（public な明示 handle resolver は無い） | `include/qpdf/QPDF.hh:770-781,1031`; `libqpdf/QPDF.cc:1699-1753` | 通常 route: `ObjectHandle::try_dereference` → `ResolverHandle::resolve_qpdf_obj_gen`; qtest-driver 例外だけ `Pdf::resolve` を hidden feature route として保持 | 非 qtest: prod 0 / test 0; qtest-driver の直接 caller は別 qtest session の例外として除外 | canonical | `crates/flpdf/src/object_handle.rs::ObjectHandle::try_dereference` | `Pdf::resolve` / `Pdf::resolve_handle` の通常 build facade は撤去済み。
| A8 | `QPDFObjectHandle::getKey` / `hasKey` は receiver を dereference し warning + null/false fallback または例外を返す | `libqpdf/QPDFObjectHandle.cc:965-1015,2168-2189` | 通常 route: `ObjectHandle::try_get_key` / `try_has_key`; `get_key` / `has_key` は qtest-driver hidden feature exception のみ | 非 qtest: prod 0 / test 0; qtest-driver の直接 caller は別 qtest session の例外として除外 | canonical | `crates/flpdf/src/object_handle.rs::ObjectHandle::try_get_key` / `try_has_key` | panic convenience は通常 build から撤去し、warning/error は fallible accessor から伝播する。
| A9 | `QPDF::getAllObjects`（`fixDanglingReferences` → `obj_cache` 全走査） | `libqpdf/QPDF.cc:1239-1269,1285-1295` | `crates/flpdf/src/reader.rs::Pdf::get_all_objects`（`pub`） | prod callers は writer/rewrite_renumber.rs、document_json.rs、reader.rs、qtest-tools の driver / renumber / metadata。全件数は今回未再集計 | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::get_all_objects` | `.48.20`で履歴trailer参照の未解決登録をopen境界へ移し、Pdfに保持していた参照集合とgetAllObjectsのlate mint/eager resolveを撤去した。`Pdf::get_all_objects` は raw `QpdfObjGen` key の canonical cache walk を直接委譲し、page-merge の preserve seed も `canonical_live_object_handles` で同じ raw identity を保持する。`ObjectRef` を要求する writer/public projection は別の明示的な境界で narrowed される。`flpdf-rbja1` では `read_xref` が除去した lower generation を cache からも消し、raw object-zero row を `obj:0 0 R` として列挙する。 |

| A10 | 同じ `obj_cache` の列挙（qpdf は `getAllObjects` 1 本のみ） | `libqpdf/QPDF.cc:1285-1295` | private `Pdf::canonical_object_refs` / `Pdf::canonical_live_object_refs`; public complete boundary `Pdf::get_all_objects` | prod: canonical views only / test: integration helpers use `get_all_objects` | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::all_object_handles`（A9） | `.48.22` で qpdfにない `object_refs` / `live_object_refs` / `resolved_count` のfacade APIとcache mergeを削除。complete viewはcanonical cache keys、live viewはeffective xrefまたはdocument-owned allocationを使う。`flpdf-rbja1` の JSON v2 回帰で、active generation だけ（`4 1`）を残し、raw object-zero identity（`0 0`）を落とさないことを qpdf 11.9.0 と確認する。 |
| A11 | `QPDF::getObjectCount` / `QPDF::nextObjGen` | `libqpdf/QPDF.cc:1271-1283,1872-1880` | `ResolverHandle::get_object_count` / `next_obj_gen` と `Pdf::get_object_count` / `Pdf::next_obj_gen` | `python3 scripts/qpdf-route-callers.py --symbol get_object_count --symbol next_obj_gen`: `get_object_count` prod 7 (6 files: writer.rs ×2, document_json.rs, job/page_merge.rs, reader.rs, reader/resolver.rs, writer/object_streams/eligibility.rs) / test 10; `next_obj_gen` prod 4 (3 files: reader/resolver.rs ×2, nntree.rs, reader.rs) / test 4 | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::next_obj_gen` | `.48.20`で`Pdf::get_object_count`はcanonical cache-key最大値に委譲し、列挙によるidentity更新を除いた。public makeIndirect factoryも`next_obj_gen`へ統一済み。`flpdf-3yn9.48.171` で判明: `.48.22`時点で「legacy混じり`object_refs()`を使う」としていたこの行の前提は、`27d0caf02`（2026-09-14、`fix(cli): apply transformations before JSON output`）の時点で既に stale だった——同コミットが `page_annotation_flatten.rs::add_content_stream` を最後の production consumer として `Pdf::new_stream_with_data`（qpdf の `QPDF::newStream(std::string)`）へ移し、`next_available_object_ref` 自体を `#[cfg(test)]` gate した。`object_refs()` はそれ以前の `.48.22`（facade cache cutover）で既に削除済みで、`next_available_object_ref` の実装はその時から `self.resolver.next_obj_gen()` への 1 行委譲のみだった。残っていたのは、production 非公開の `#[cfg(test)]` facade `next_available_object_ref` が、production にも公開されている同一委譲の `Pdf::next_obj_gen`（`nntree.rs:1422` の `QPDF::nextObjGen()` 呼び出しコメント参照）と完全に重複していた点のみ。本 issue でこの重複 `#[cfg(test)]` facade を削除し、唯一の test consumer（`embedded_files.rs`, `job/attachment_list.rs`）を `Pdf::next_obj_gen` へ直結した。`Pdf::get_object_count` / `Pdf::next_obj_gen` はいずれも `ResolverHandle` の同名メソッドへの 1 行委譲で、二重経路は残らない。 |
...
| A13 | `QPDF::removeObject`（private。xref erase → cache値をnullにassign → og解除 → cache erase） | `libqpdf/QPDF.cc:1995-2005`, `include/qpdf/QPDF.hh:1041` | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::remove_object` | facade `crates/flpdf/src/reader.rs::Pdf::remove_object_handle` は production eligibility walkから1件、resolver unit testsから到達 → resolver prod: 1 / test: 2、facade prod: 1 / test: 2 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::remove_object` | canonical primitive自体はqpdfのprivate責務を持つ。`compressible_objgens_qpdf_plan` がqpdfのstale-generation分岐でこのprimitiveを実行し、保持aliasをdirect null化する。残るmixed部分はtest facadeと、qpdfに無いflpdf側の検証用入口であり、primitiveとその意味を検証するtestは保持する。`.46` でhandle-retaining variantは撤去済みで、**現在のcanonical removalがog/identityを保持するという旧probe前提は誤り**。**2026-09-19 追記（`flpdf-3yn9.48.205`）**: 依頼元 issue は `default_xref_entries` を「未文書化の第2 xref map」として撤去候補に挙げていたが、この前提は stale と判明した。`default_xref_entries` は `resolver.rs:456-462` で qpdf の `m->xref_table[og]`（`operator[]`、`QPDF.cc:1823`。ObjStm ヘッダ検査中に type-0 の transient row を default-construct する副作用）を明示的に引用して既に文書化されており、`m->xref_table` という**単一の** qpdf map を flpdf が「live な type-1/2 row（`raw_source_xref_entries`）」と「transient type-0 row（`default_xref_entries`）」の互いに排他な 2 集合へ分割した結果である（og は常にどちらか一方にしか存在しない）。したがって `remove_object` が両方から erase するのは qpdf の `m->xref_table.erase(og)` 1 行への忠実な翻訳であり、第 3 の経路ではない — 撤去も marker も不要、**mixed の分類自体もこの理由では変わらない**（mixed は上記の test facade 二重 entry によるもので、default_xref_entries の扱いとは無関係）。一方、残り 2 つの除去は qpdf に対応物のない flpdf 固有 bookkeeping で、本 issue で `// qpdf-deviation` marker を追加した: (1) `default_xref_warnings`（default row の warning 配信済みフラグ。`discard_cached_generations` は既にこれを clear していたが `remove_object` は漏れていた——object 0 の default row が除去後に再生成されるとqpdfなら再度出す warning が抑制される regression で、`remove_object_clears_default_xref_warning_delivery_bookkeeping` テストで RED→GREEN 確認・mutation 検証済み。production caller（`page_merge.rs:195`/`eligibility.rs:216`）はいずれも object 0 を渡さないため両方とも現状 unreachable だが、primitive 自体の正しさとして修正した）、(2) `allocated_object_refs`（`flpdf-uwn0` で既に確立された allocation provenance。書込み側は A16/A17 で既に marker 済みで、この除去側は今回追加した対称な cleanup）。 |
...
| A16 | `QPDF::replaceObject(og, oh)`（indirect/未初期化を `std::logic_error` で拒否 → `updateCache(og, obj, -1, -1)`） | `libqpdf/QPDF.cc:1985-1993`、`libqpdf/QPDF.cc:1842-1858` | `crates/flpdf/src/reader.rs::Pdf::replace_object`（`pub`）→ `crates/flpdf/src/reader/resolver.rs::ResolverHandle::replace_object`（`pub(crate)`） | prod: 17 (json/input.rs ×4, reader.rs ×2, writer.rs ×2, page_annotation_flatten.rs ×2, `crates/flpdf-qtest-tools` driver/test_10_17.rs ×2, embedded_files.rs, page_extract.rs, object_copy.rs, job/outline_dest_remap.rs, job/page_merge.rs) / test: 113 | mixed |`crates/flpdf/src/reader/resolver.rs::ResolverHandle::replace_object` | **2026-09-19 訂正（`canonical` 化を撤回）**: `ResolverHandle::replace_object`（`reader/resolver.rs:1904`）は `belongs_exclusively_to_pdf` を呼ぶが、**qpdf の `QPDF::replaceObject`（`QPDF.cc:1986-1993`）は `isIndirect()` と `!isInitialized()` しか見ず `checkOwnership` を呼ばない**。`object_handle.rs` の doc 自身が「qpdf の shallow な `checkOwnership` と違い direct-value 子孫グラフ全体を歩く flpdf 固有の防御」と明記している。さらに末尾で `record_allocated_object` を呼ぶ（qpdf 側に対応なし）。§3 の「アルゴリズム・呼び出し順序が 1:1」を満たさないため `mixed`。 `flpdf-3yn9.48.172` の再監査で本行が stale と判明: 旧稿が挙げていた3操作（`synchronize_cache_with_resolver_xref`、`qpdf_parsed_xref_stream_refs`/`qpdf_dangling_refs` からの除去、facade側の追加ロジック）は `flpdf-3yn9.48.22`（`crates/flpdf/src/cache.rs` 347行を含む facade object cache 撤去）で既に削除済みだった。現在の `Pdf::replace_object`（`crates/flpdf/src/reader.rs:1211-1218`）は `self.resolver.replace_object(...)` への1行委譲のみで、`ResolverHandle::replace_object`（`crates/flpdf/src/reader/resolver.rs:1889-1945`）が `updateCache` の insert-or-assign 分岐に1:1対応する。`mark_object_handle_mutated` は `.48.24`、空集合からの無効なremoveは `.48.21` で撤去済み。legacy 2 集合が既に存在しないため A10 との食い違いも解消済み。xref 無し generation での `record_allocated_object` 呼び出しは CLAUDE.md 逸脱分類 (C) 対象（**2026-09-19 訂正**: `replace_object` 内の `belongs_exclusively_to_pdf` guard を同じ (C) に含めていたのは誤り。(C) は「出力バイトを変えない」ことが前提だが、この guard は変える——qpdf の `QPDF::replaceObject`（`QPDF.cc:1986-1993`）は `checkOwnership` を呼ばないので、foreign な indirect 子を持つ direct 値は qpdf では成功して書き出される一方、flpdf は `Unsupported` を返す。未解決の挙動乖離としてmarker を残し、本行で追跡する）（`make_indirect_from_object_handle` 側の同じ関数の呼び出しは引き続き `flpdf-uwn0` が防御する対象で別物）で、`flpdf-3yn9.48.172` で `ResolverHandle::replace_object` 本体に `// qpdf-deviation` marker を追加した。 |
| A17 | `QPDF::swapObjects(og1, og2)`（**先に両方 resolve** → `swapWith` で value と og を交換） | `libqpdf/QPDF.cc:2284-2291`、`libqpdf/qpdf/QPDFObject_private.hh:121-130` | `crates/flpdf/src/reader.rs::Pdf::swap_objects`（`pub`）→ `crates/flpdf/src/reader/resolver.rs::ResolverHandle::swap_objects`（`pub(crate)`） | prod: 3 (reader.rs `:1555`、`crates/flpdf-qtest-tools` driver/test_10_17.rs `:295`,`:323`) / test: 7 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::swap_objects` |**2026-09-19 訂正（`canonical` 化を撤回）**: `ResolverHandle::swap_objects`（`reader/resolver.rs:1974-1990`）は末尾で `record_allocated_object` を呼び `allocated_object_refs` を更新する（コメント自身が writer の live view 補償だと認めている）。**qpdf の `QPDF::swapObjects`（`QPDF.cc:2284-2291`）は `resolve` / `resolve` / `swapWith` の 3 行のみ**で、この副作用に対応物がない。§3 の 1:1 条項を満たさないため `mixed`。 A16 と同型で `flpdf-3yn9.48.172` の再監査対象。空のremoved-ref集合へのremoveは `.48.21` で撤去済み。旧稿が挙げていた `CacheEntry::Deleted`/`Missing`/`Reserved` の tombstone を手で消す分岐は、当時の `crates/flpdf/src/cache.rs`（`CacheEntry` enum の定義元）ごと `flpdf-3yn9.48.22` で完全に削除されている——現在の `crates/flpdf/src/reader.rs`/`reader/resolver.rs` に `CacheEntry` 型は存在せず（`ObjectCacheEntry` 構造体のみ）、旧稿が引用していた `reader.rs:1432-1450` は現在無関係なコードを指す。tombstone 分岐自体はこの経緯で消滅したが、上記の `record_allocated_object` 呼び出しは残っており、`flpdf-3yn9.48.172` で `ResolverHandle::swap_objects`（`replace_object` も同様）の本体に `// qpdf-deviation-start`/`-end` marker を追加し、CLAUDE.md 逸脱分類 (C) の記載義務（モジュール doc + `docs/qpdf-correspondence.md` の `flpdf-uwn0` 追記 + コード側 marker）を満たした。`replace_object_generation_replacement_matches_qpdf_tombstone_lifetime`（`crates/flpdf/src/reader/resolver.rs`）が、qpdf に永続 tombstone が無いのと同じ寿命で canonical cache が振る舞うことを検証する。 |
...
| A19 | `QPDF::closeInputSource`（`m->file` を `InvalidInputSource` に差し替えるだけ。cache は触らない） | `libqpdf/QPDF.cc:277-281`、`include/qpdf/QPDF.hh:162-166`、`libqpdf/QPDF.cc:99-106` | `crates/flpdf/src/pdf.rs::Pdf::close_input_source`（`pub`）→ `crates/flpdf/src/reader/resolver.rs::ResolverHandle::close_input_source`（`pub(crate)`、`crates/flpdf/src/reader/resolver.rs:2558`） | `rg -n 'close_input_source\(' crates` の宣言 2 行を除いた prod 2: `Pdf::close_input_source` の外部呼び出しは `crates/flpdf-qtest-tools/src/driver/test_72_79.rs:276` の 1 件のみ、もう 1 件は `crates/flpdf/src/pdf.rs:227` の canonical への委譲 / test: 2 (reader.rs `:2280`, resolver.rs `:7102`) | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::close_input_source` | `Pdf::close_input_source` は resolver の差し替えに加えて `set_input_source_stay_open(false)` を呼ぶ（`crates/flpdf/src/pdf.rs:226-229`）。これは qpdf の `ClosedFileInputSource::stayOpen` を持つ file source が `m->file` 置換で最後の owner を失う挙動の再現で、doc に理由が明記されている（同 `:218-225`）。逸脱ではなく `shared_ptr` reset 相当の補完。 |
| A20 | `QPDF::~QPDF`（`xref_table.clear()` → `obj_cache` 全件 `disconnect()` → `ot_null` 以外は `destroy()`） | `libqpdf/QPDF.cc:215-236`、`libqpdf/QPDFObject.cc:13-17`、`libqpdf/qpdf/QPDFObject_private.hh:19-180` | `crates/flpdf/src/pdf.rs::Pdf::drop`（`impl Drop for Pdf`）→ `crates/flpdf/src/reader/resolver.rs::ResolverHandle::disconnect_all` | `disconnect_all(` prod: 3（`crates/flpdf/src/pdf.rs`、`crates/flpdf/src/job/lifecycle.rs`、宣言元 `crates/flpdf/src/reader/resolver.rs`）/ test: 1。`.disconnect()` prod: 1（`disconnect_all` 内）/ test: 26（全て object_handle.rs） | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::disconnect_all` | `ObjectHandle::disconnect` 自体は qpdf の `disconnect()`+`destroy()` の組を `ot_null` を `destroy` しない条件込みで再現しており正確。`.48.15.1` で canonical `disconnect_all` は `source_xref_entries.clear()` を先に実行してから canonical object cache の handle を disconnect/destroy し、qpdf の clear-before-disconnect 順序（`QPDF.cc:215-236`）を揃えた。2026-09-18（`flpdf-3yn9.48.151`、owner-less bootstrap parser 撤去）**: owner-less bootstrap cache の第 2 の teardown walk（旧 `xref.rs::BootstrapCache` の `Drop`）がその機構ごと消え、qpdf と同じく teardown walk は 1 本になった**。`disconnect` は `slot.object_ref.is_none()` で **早期 return する**（`crates/flpdf/src/object_handle.rs:2540-2542`）ので `slot.resolver` を残す経路が形式上あるが、それが起きるのは (a) 直接 handle — `try_dereference` は `object_ref` が無い時点で `Ok(())` を返すので resolve 経路自体が無い、(b) 既に 1 回 disconnect 済みの handle — その 1 回目で `resolver` は `None` になっている、の 2 つだけで、いずれも「teardown 後に resolve が成功する」窓にはならない。 |
...
| A24 | `QPDF::resolveObjectsInStream`（`resolved_object_streams` で二重展開防止、xref 再チェックで上書き済みメンバーを cache しない） | `libqpdf/QPDF.cc:1756-1833`（早期 return + insert は `:1758-1761`） | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::resolve_object_stream_with_failure_kind`（本行の 2 つの不変条件の**両方**を実装する owner。二重展開防止は guard state `ResolverCore::resolved_object_streams`（`BTreeSet<u32>`、private）が担い、xref 再チェックによる上書き済みメンバーの除外は同メソッド `:2786-2801` が担う。2026-09-19 訂正: 旧記載は guard state のフィールドだけを owner にしており、行が謳うもう一方の不変条件を強制できない状態を指していた。唯一の書き込みは `resolve_object_stream_with_failure_kind` 内の `insert` で、戻り値 `false`（既に解決済み）なら即 `Ok(())` を返す） | `rg -n --glob '*.rs' 'resolved_object_streams' crates` → prod: 3 (1 file, `crates/flpdf/src/reader/resolver.rs`：struct 初期化 2 箇所＋`resolve_object_stream_with_failure_kind` 内の 1 箇所) / test: 0 | canonical | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::resolve_object_stream_with_failure_kind`（二重展開防止の guard state `ResolverCore::resolved_object_streams` と、xref 再チェック `:2786-2801` の両方をこのメソッドが実装する） | **再分類（本 issue、`flpdf-3yn9.48.173`）**: 旧 facade `compressed_member_parents` provenance map（旧 `crates/flpdf/src/pdf.rs:136`）は本ファイル 154-161 行の cutover note が記す通り `flpdf-3yn9.48.22` で既に削除済みで、`rg -n 'compressed_member_parents' crates` は 0 hits（`docs/qpdf-route-matrix/README.md:1017-1026` の記録とも一致）。この行が「facade 側に残る」と記していたのは、`.48.22` 後に本行だけ再分類が漏れていた stale な記述だった。ObjStm 二重展開防止は canonical `resolved_object_streams` 1 本に統合済みで、qpdf の `resolveObjectsInStream` に 1:1 対応するため mixed ではなく canonical に改める。 |
...
2026-09-14（`flpdf-fo71t`）: `Pdf::get_all_objects` は complete raw cache
enumerationへ直接委譲し、raw generationは返却された `ObjectHandle` に保持する。
`canonical_object_refs` / `canonical_live_object_refs` は従来どおり checked
`ObjectRef` projection view として残す。
...
The A1/A2/A9/A10/A11/A13/A15/A16/A17 rows above were authored before
...
superseded by the cutover note at lines 144-160. (A24 was removed from this list on
2026-09-19: `flpdf-3yn9.48.173` recounted that row against the post-cutover tree, so it
is no longer a pre-cutover snapshot and the warning must not send readers past it.)
The current production symbols
...
3. **A13 — canonical removalとtest facadeを区別する**。qpdfのcache erase / og解除
   ...
   **2026-09-19 追記（`flpdf-3yn9.48.205`）で解消**: `default_xref_entries`
   除去が「未文書化の第2 map」かという問いは、qpdfの単一`m->xref_table`を
   flpdfが2つの互いに排他な集合へ分割した結果であり忠実な翻訳と判明、撤去も
   markerも不要と確定した。`default_xref_warnings`/`allocated_object_refs`の
   除去（qpdf対応物なし）には`// qpdf-deviation` markerを追加し、
   `default_xref_warnings`の除去漏れ（`discard_cached_generations`との非対称）は
   RED→GREENで修正済み。本行に残るtest facade二重entry自体は未解消。
...
The final non-qtest cleanup removes the caller-side resolution and panic-key
facades from the normal flpdf build. All core source, unit-test, integration-
test, CLI-test, and example callers now use the existing fallible
`ObjectHandle::try_*` accessors. The only production changes outside tests are
`DictItemCursor::current`, which now fetches its live child through
`try_get_key` while retaining its value-returning cursor contract, and the
resource merge path, which propagates `try_has_key` errors.

The qpdf authority is unchanged: typed/null accessors dereference at entry
(`libqpdf/QPDFObjectHandle.cc:240-446`), dictionary key access uses the same
boundary (`libqpdf/QPDFObjectHandle.cc:965-1015`), and warning/error delivery
is owned by the handle/document boundary (`libqpdf/QPDFObjectHandle.cc:2168-2189`).
Explicit `QPDF::resolve` remains private to `QPDFObject` through
`QPDF::Resolver` (`include/qpdf/QPDF.hh:770-781,1031`; implementation
`libqpdf/QPDF.cc:1699-1753`), so `ObjectHandle::try_dereference` and the
fallible `try_*` accessors are the canonical flpdf routes.

The post-cleanup caller census on this worktree, based on `origin/main`
`2928b4ef2`, found zero non-qtest
production and test callers for `Pdf::resolve`, `Pdf::resolve_handle`,
`resolve_handle_ref`, `resolve_qpdf_json_handle`, `ObjectHandle::get_key`, and
`ObjectHandle::has_key`. `Pdf::resolve` and the two panic key methods remain
only behind the hidden `qtest-driver` feature so the separate qtest-exception
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 20 | A1, A2, A3, A4, A5, A7, A8, A9, A10, A11, A12, A14, A15, A18, A19, A20, A21, A22, A23, A24 |
| mixed | 4 | A6, A13, A16, A17 |
| bridge | 0 | — |
| unknown | 0 | — |
