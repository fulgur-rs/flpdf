...
| A9 | `QPDF::getAllObjects`（`fixDanglingReferences` → `obj_cache` 全走査） | `libqpdf/QPDF.cc:1239-1269,1285-1295` | `crates/flpdf/src/reader.rs::Pdf::get_all_objects`（`pub`） | prod callers は writer/rewrite_renumber.rs、document_json.rs、reader.rs、qtest-tools の driver / renumber / metadata。全件数は今回未再集計 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::all_object_handles` | `crates/flpdf/src/reader.rs:1756-1783` は trailer reference を後から seed/resolve し、number 0 / generation 65535 を除外する facade を持つ。`qpdf_removed_refs` filter も残るが、2026-09-06 の全 `crates` 調査ではこの集合への insert は 0 件で常に空。したがって **現在この集合による観測差があるとは主張しない**。空集合filterの削除と、bootstrap期から同一cacheにtrailer childを登録する責務移行は別slice。 |
...
| A11 | `QPDF::getObjectCount` / `QPDF::nextObjGen` | `libqpdf/QPDF.cc:1271-1283,1872-1880` | `ResolverHandle::get_object_count` / `next_obj_gen` と `Pdf::get_object_count` / `next_available_object_ref` | `rg -n -e 'get_object_count\(' -e 'next_obj_gen\(' -e 'next_available_object_ref\(' crates --glob '*.rs'` で facade と resolver を識別する。全件数は今回未再集計 | mixed | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::next_obj_gen` | 2026-09-06: facade の `get_object_count` は `crates/flpdf/src/reader.rs:1225-1237` で **`get_all_objects()` の最大object number**を取る。旧表のresolver count直接委譲という記述は古い。`Pdf::next_obj_gen`（同 `:1243-1245`）はcanonicalへ委譲する一方、`next_available_object_ref`（`:1566-1584`）はlegacy混じり `object_refs()` とresolver最大値を合わせる。A12のclone factoryが後者を使うため二重経路が残る。 |
...
| A23 | （qpdf に対応物なし。dictionary key は `/` 付き decoded name） | `libqpdf/QPDFParser.cc:464`, `libqpdf/QPDF_Name.cc:27-49` | `crates/flpdf/src/object_handle.rs::legacy_dictionary_key` と `canonical_dictionary_key` | 実呼出 prod: 6 — parser.rs:629,736、writer/object.rs:196、stream_filter.rs:586,621,623。別にimport 2行 / test: 0 | bridge | `crates/flpdf/src/object_handle.rs::canonical_dictionary_key` | 旧 `Object` / `Dictionary` は削除済み。残る責務はparser warning、writer name emission、stream-filter key判定の3群。qpdf parserはcanonical keyをそのままwarningへ連結し、`QPDF_Name::normalizeName` は先頭slashを保つ。各consumerをその責務へ移行し、最後にhelperを削除する。旧8件はimport 2行を含み、実呼出は6件。 |
