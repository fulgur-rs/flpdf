...
| A22 | （qpdf に対応物なし。qpdf は live input source を読む） | `libqpdf/QPDF.cc:1360-1398,1541-1697` | `crates/flpdf/src/reader/resolver.rs::ResolverHandle::read_window`（`pub(crate)`、deprecated） | prod: 5、全て `crates/flpdf/src/reader.rs:988,994,1017,1033,1039` / test: 0 | bridge | absent | `read_to_owned` も同じdeprecated bridge。残callerは `qtest_read_source_object_with_retry` と `parse_source_file_object_at`。2026-09-08（`.44`）: この2経路が `driver/test_0_1.rs` から辿り着いていた外部 caller（旧 `:260,267,282` の plural qtest metadata API 経由、旧 `:343,389` の `source_stream_data_offset` 経由）は、`try_get_parsed_offset()` の生成時キャプチャへの置換（E27 参照）で両方とも 0 になった。`read_window`/`read_to_owned` 自身は依然 `reader.rs` 内部から `qtest_read_source_object_with_retry`/`parse_source_file_object_at` 経由で呼ばれており prod: 5 は変わらないが、これら2関数自体の外部 caller が 0 になったため、B34 のfallback予算撤去（`.25`）で丸ごと caller-zero 削除できる状態になった。 |
