...
| C23 | `QPDFWriter::willFilterStream` 相当の公開 helper | `libqpdf/QPDFWriter.cc:1238-1315`（qpdf 側は private） | `crates/flpdf/src/writer.rs::apply_stream_compress_policy`（**`pub`**, `crates/flpdf/src/writer.rs:5489-5497`）→ `crates/flpdf/src/writer.rs::apply_stream_compress_policy_with_decode_level`（private, `crates/flpdf/src/writer.rs:5499-5599`） | prod: **0**（`crates/flpdf/src/lib.rs:236` は `pub use` の再輸出行のみ） / test: 2 (`crates/flpdf/src/writer.rs:5980,5996`) + 3 (private 版 `crates/flpdf/src/writer.rs:6026,6044,6059`) | bridge | `crates/flpdf/src/writer/plain/body.rs::canonical_stream_filter_plan` | production caller ゼロ。`pipe_stream_data` を通らず `get_raw_stream_data` → `filter_chain_is_decodable`（`crates/flpdf/src/writer.rs:5621`、qpdf に対応物なし）→ `filters::decode_stream_data` → `filters::encode_stream_data` の whole-buffer route。CLAUDE.md 分類 (B) 根拠 8 の 3 根拠すべてに該当しない: (1) qpdf 側 `willFilterStream` は `include/qpdf/QPDFWriter.hh:440` の `private:` 配下、(2) flpdf-cli からの呼び出し無し、(3) `crates/flpdf/src/lib.rs` 冒頭 `//!` doc に記載なし（`merge_documents`/`extract_pages` は記載あり）。cutover 最小単位: この `pub` を落とせば `apply_stream_compress_policy_with_decode_level` / `filter_chain_is_decodable` / `stream_dictionary_copy` 経由の subtree ごと外せる |
...
| C26 | （C25/C23 の下請け）filter chain の whole-buffer decode | `libqpdf/QPDF_Stream.cc:487-638`（qpdf に whole-buffer 専用 route は無い） | `crates/flpdf/src/filters.rs::decode_stream_data`（`pub`, `crates/flpdf/src/filters.rs:114-121`） | prod: 4 (`crates/flpdf/src/json_inspect.rs:274`, `crates/flpdf/src/writer.rs:5527`, `crates/flpdf-qtest-tools/src/compare.rs:85,89` + `crates/flpdf-qtest-tools/src/driver/test_34_41.rs:519` = parity harness) / test: 3 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | flpdf 本体の残 production caller は C25（json_inspect）と C23（caller ゼロの `pub` helper）の 2 つだけ。source dispatch も復号も token filter も持たず、呼び出し側が別途取得した `&[u8]` を受け取る |
...
| mixed | 11 | C4, C8, C9, C17, C18, C22, C25, C26, C27, C29, C44 |
| bridge | 4 | C19, C23, C28, C43 |
