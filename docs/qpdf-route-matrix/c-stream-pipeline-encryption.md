...
| C27 | 同上（`ObjectHandle`-native 版） | `libqpdf/QPDF_Stream.cc:487-638` | `crates/flpdf/src/filters.rs::decode_stream_data_from_handle`（`pub(crate)`, `crates/flpdf/src/filters.rs:366`） | prod: 1 (`crates/flpdf/src/xref.rs:1432`) / test: 0 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `docs/qpdf-correspondence.md:312` の production caller は、`.48.14` で bootstrap ObjStm（qpdf `libqpdf/QPDF.cc:1792`）を `ObjectHandle::get_stream_data(DecodeLevel::Specialized)` へ移行したため除去済み。残るのは `crates/flpdf/src/xref.rs:1432` の bootstrap-context xref stream consumer（qpdf `libqpdf/QPDF.cc:1051`、`.48.43` 後も `XrefObjectContext::decoded_xref_stream_data` の `XrefReadContext` 実装内）のみで、bootstrap xref streamのwhole-buffer撤去は `.48.49` の後続範囲とする。 |
