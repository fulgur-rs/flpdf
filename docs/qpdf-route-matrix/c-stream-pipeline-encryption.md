...
| C27 | 同上（`ObjectHandle`-native 版） | `libqpdf/QPDF_Stream.cc:487-638` | `crates/flpdf/src/filters.rs::decode_stream_data_from_handle`（`pub(crate)`, `crates/flpdf/src/filters.rs:405-409`） | prod: 3 (`crates/flpdf/src/resources.rs:277`, `crates/flpdf/src/xref.rs:751,3381`) / test: 0 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `docs/qpdf-correspondence.md:312` の「production 経路の `decode_stream_data`/`encode_stream_data` 呼び出しはテストコードのみに残る」に反する現存 production caller。現在の `crates/flpdf/src/xref.rs:751` は bootstrap ObjStm（qpdf `libqpdf/QPDF.cc:1792`）、`crates/flpdf/src/xref.rs:3381` は xref stream（qpdf `libqpdf/QPDF.cc:1051`）。ともに qpdf は `getStreamData(qpdf_dl_specialized)` を使う。`.48.42` で `filespec_helper/embedded_file_stream.rs::EmbeddedFileStream::payload` を `ObjectHandle::get_stream_data`（C5）へ移行しこの行の caller から除いた — 唯一の実挙動差は token filter の適用（旧経路は無視していた） |
