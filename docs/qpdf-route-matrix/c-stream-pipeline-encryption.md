...
| C42 | `QPDF::recoverStreamLength` | `libqpdf/QPDF.cc:1482-1530` | `crates/flpdf/src/reader/resolver.rs::recover_stream_length`（private） | prod: canonical resolver の stream recovery 呼び出し / test: recovery fixture と EOL metadata tests | mixed | `crates/flpdf/src/reader/resolver.rs::recover_stream_length` | warning 3 種、`endobj` 巻き戻し、全 recovered length の計算は 1:1。`RecoveredStreamEol` は inspection の表示 framing のための flpdf-only metadata として残るが、C12 の AES/RC4 pipe は qpdf と同じく `length` をそのまま渡す（`flpdf-zvjf`）。 |
...
| bridge | 2 | C28, C43 |
| unknown | 0 | なし（C42 の EOL subtraction は `flpdf-zvjf` で qpdf parity として解決） |
