...
| C26 | （C25 の下請け）filter chain の whole-buffer decode | `libqpdf/QPDF_Stream.cc:487-638`（qpdf に whole-buffer 専用 route は無い） | `crates/flpdf/src/filters.rs::decode_stream_data`（`pub`, `crates/flpdf/src/filters.rs:75-82`） | prod: 2 (`crates/flpdf-qtest-tools/src/compare.rs:85,89`) / test: 1 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `.45` が `driver/test_34_41.rs` の qpdf test 36 consumer を raw `PlFlate` pipeline に移行。`compare.rs:85,89` は別 bounded slice として decoded comparison に残る。C25の旧json_inspect consumerは`.40`でcanonical stream APIへ移行・撤去した。 |
| C27 | 同上（`ObjectHandle`-native 版） | `libqpdf/QPDF_Stream.cc:487-638` | `crates/flpdf/src/filters.rs::decode_stream_data_from_handle`（`pub(crate)`, `crates/flpdf/src/filters.rs:366`） | prod: 2 (`crates/flpdf/src/xref.rs:857,1395`) / test: 0 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `docs/qpdf-correspondence.md:312` の「production 経路の `decode_stream_data`/`encode_stream_data` 呼び出しはテストコードのみに残る」に反する現存 production caller。現在の `crates/flpdf/src/xref.rs:857` は bootstrap ObjStm（qpdf `libqpdf/QPDF.cc:1792`）、`crates/flpdf/src/xref.rs:1395` は xref stream の bootstrap-context consumer（qpdf `libqpdf/QPDF.cc:1051`、`.48.43` 後は `XrefObjectContext::decoded_xref_stream_data` の `XrefReadContext` 実装内）。ともに qpdf は `getStreamData(qpdf_dl_specialized)` を使う。`.48.42` で `filespec_helper/embedded_file_stream.rs::EmbeddedFileStream::payload` を `ObjectHandle::get_stream_data`（C5）へ移行しこの行の caller から除いた — 実挙動差は 2 点 — (1) token filter が適用される（旧 whole-buffer 経路は無視していた）、(2) `DecodeLimits` の `max_filter_chain`（`MAX_FILTER_CHAIN_LEN = 16`, `crates/flpdf/src/filters.rs:20,203`）が外れる。canonical `pipe_stream_data` は chain 本数上限を持たない（`pipe_stream_data_accepts_a_qpdf_unbounded_filter_chain` が 17 段で保証）ため、17 段以上の filter chain を持つ添付は旧経路の `Unsupported` からデコード成功へ変わる。qpdf に chain 本数上限は無く（C28 のとおり `DecodeLimits` は qpdf 対応物のない hardening budget）、方向としては parity 改善・hardening 後退である。(3) `/Filter /Crypt`（例 `/Name /Identity`）の扱いも変わる — 旧 whole-buffer 経路は常に `reject_crypt_stage`（`crates/flpdf/src/filters.rs:330,423`）で `Error::Unsupported` を返したが、canonical 経路の `CryptStreamFilter` は段を寄与しないためデコードが成立する。qpdf は `/Crypt` を decode 段として扱わない（`QPDFWriter.cc:1457-1481` は writer 側で `/Crypt` を除去する）ので、これも parity 方向の変化である。(4) 回復可能な codec warning の扱いも変わる — document-backed な `/FlateDecode` 添付が不完全な trailer を持つ場合、canonical 経路は `Z_BUF_ERROR` を warning として記録しつつ保持済みの decode 結果を返すが、旧 whole-buffer 経路は全 warning を `reject_decode_warning` に渡してエラーにしていた。qpdf も `pipeStreamData` の warning は診断として出しつつデータを返すため、これも parity 方向である。(5) 不正な filter metadata（`/Filter 1`、filter 配列中の非 name 要素、`/Filter` と長さの異なる `/DecodeParms` 配列など）の扱いも変わる — 旧 whole-buffer 経路は document warning を残さず `Error::Unsupported` を返すだけだったが、canonical 経路は qpdf 同様に warning を記録する。`.48.41` で `resources.rs::remove_unreferenced_resources_in_form_xobjects` の Form pre-pass 呼び出しを `PageObjectHelper::parse_contents`（C5 相当、`ResourceFinder` callback 経由）へ移行しこの行の caller から除いた — qpdf の `removeUnreferencedResourcesHelper`（`QPDFPageObjectHelper.cc:539-556`）は page と Form で同一関数のため、既存の page-level canonical block と同じ diagnostic-count ゲートを Form 側にも適用した。`.48.43` で xref stream の **canonical-owner** consumer（`parse_xref_stream_with_canonical_owner` → `build_xref_stream`）を `ObjectHandle::get_stream_data(DecodeLevel::Specialized)`（C5）へ移行しこの行の caller から除いた — bootstrap-context consumer（`XrefReadContext`）はこの行に残る。移行と同時に qpdf-parity の逸脱を1件解消した: 旧経路は `CanonicalXrefContext::raw_stream_data` が `recovered_stream_eol` で recovery が付加した末尾 EOL を decode 前に trim していたが、qpdf の `recoverStreamLength`（`QPDF.cc:1482-1497`）はその EOL を含めた長さをそのまま `getStreamData` に渡す。実機 qpdf 11.9.0 で `/Length` 欠落の非圧縮 xref stream を確認したところ `Cross-reference stream data has the wrong size; expected = N; actual = N+1` を warn しており、旧 flpdf はこの warning を出さずに trim していた（乖離）。`get_stream_data` への切り替えで trim を撤去し、`CanonicalTrailerOwner::recovered_stream_eol`（このためだけに存在した trait メソッド）も削除した — qpdf の unfilterable stream 診断文言（`getStreamData called on unfilterable stream`）への収斂も同時に得られる（旧 `decode_stream_data_from_handle` は独自文言 `unsupported stream filter: <name>` を返していた、D3 相当の乖離） |
...
- **C27**: `xref.rs:770` は **bootstrap ObjStm** の
  `BootstrapHandleDocument::resolve_objects_in_stream`（qpdf `QPDF.cc:1792`）、
  `xref.rs:3525` が **xref stream**（qpdf `QPDF.cc:1051`）である。
  bootstrap owner の `DocumentResolver` 実装は `xref.rs:885` にあり、通常 resolver の
  original pipe を備えていない。bootstrap owner の責務統合を先に確認する。
  残る whole-buffer consumer は `resources.rs:277` の Form pre-pass のみ。
  `filespec_helper/embedded_file_stream.rs` の `payload` は `.48.42` で
  `ObjectHandle::get_stream_data`（C5）へ移行済みで、この slice からは外れた。
...
- **C26 の harness consumer**: `.45` で `driver/test_34_41.rs:551` を
  ...
  JSON wrapper と残る比較 consumer が無くなった後。
