...
flpdf 側は `object_handle.rs` の stream 面、`stream_filter.rs`、`pipeline/*`、
...
| C3 | `QPDFObjectHandle::pipeStreamData`（6 引数 / 5 引数 overload） | `libqpdf/QPDFObjectHandle.cc:1300-1325`, `include/qpdf/QPDFObjectHandle.hh:1041-1059` | `crates/flpdf/src/object_handle.rs::pipe_stream_data`（`pub`, `crates/flpdf/src/object_handle.rs:6168-6186`） | prod: 24 (13 files; うち 5 は flpdf-qtest-tools = parity harness) / test: 67 (3 files) | canonical | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | flpdf は 6 引数版だけを持ち、5 引数版（success を捨てて `filtering_attempted` を返す）に対応する overload が無い。`pipe_content_streams` は `!succeeded or !filtering_attempted` を明示的に検査して qpdf の 5 引数 content-stream 境界を再現する（`flpdf-b8xcx`）。一方、`writer/plain/body.rs:838-841` は `filtering_attempted && success` を計算して 6 引数版の source-success 契約を使い、qpdf 互換の raw fallback を保持する。`pipe_stream_data_inner` も失敗時に `filtering_attempted` を落とす（`crates/flpdf/src/object_handle.rs:6577-6579`）。legacy 4 引数版（`libqpdf/QPDFObjectHandle.cc:1327-1342`）にも対応物なし |
| C4 | 同上（ObjStm resolve 経路） | `libqpdf/QPDF.cc:1792`（`getStreamData(qpdf_dl_specialized)` を catch せず呼ぶ） | `crates/flpdf/src/object_handle.rs::get_stream_data`（`pub`, `crates/flpdf/src/object_handle.rs:6220-6252`）→ `crates/flpdf/src/reader/resolver.rs::resolve_object_stream_with_failure_kind` | prod: 1 (`crates/flpdf/src/reader/resolver.rs`) / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::get_stream_data` | `.48.38` で残っていた専用 `recover_codec_errors`/error map を撤去。qpdf は ObjStm でも通常の `getStreamData` を呼び、replaced buffer・provider・original source の例外を `QPDF::resolve` の catch へ渡す。invalid Filter は filter warning と typed unfilterable `QpdfExc` を同じ resolver warning/null boundaryで処理する。whole-buffer/ObjStm compatibility の残差は `.49` scopeで追跡する |
| C5 | `QPDF_Stream::getStreamData` | `libqpdf/QPDF_Stream.cc:344-360` | `crates/flpdf/src/object_handle.rs::get_stream_data`（`pub`, `crates/flpdf/src/object_handle.rs:6223-6252`） | prod: 20 (12 files; うち 15 hits は flpdf-qtest-tools = parity harness、残り 5 は flpdf-cli `main.rs:6153`、`crates/flpdf/src/job/inspection.rs:102,210`、`crates/flpdf/src/linearization/check.rs:1612`、`crates/flpdf/src/filespec_helper/embedded_file_stream.rs`（`.48.42` で C27 から移行した `payload`）) / test: 29 (12 files) | canonical | `crates/flpdf/src/object_handle.rs::get_stream_data` | `!filtering_attempted` の `QPDFExc` 相当（filename + parsed offset 付き）まで写している。qpdf に無い `stream_data_succeeded` 判定を追加している（`crates/flpdf/src/object_handle.rs:6246-6250`）が、qpdf の 6 引数 `pipeStreamData` は同条件で `filtered=false` も返すため観測差は無い |
...
| C7 | `QPDF_Stream::filterable`（factory lookup + `/DecodeParms` 整合 + `setDecodeParms`） | `libqpdf/QPDF_Stream.cc:378-485` | `crates/flpdf/src/object_handle.rs::prepare_stream_filter_plan`（private） | prod: 1 / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::prepare_stream_filter_plan` | `/Filter` shape、factory lookup、空/不一致 `/DecodeParms`、`setDecodeParms` の順序と warning sink は qpdf と対応する。`.48.37` で warning attribution を修正し、`.48.96` で重複していた C9 legacy spec readerを削除したため、現在この責務に残る production routeは1本。 |
| C8 | 同上（decode level gate 用の capability 判定だけを切り出したもの） | `libqpdf/QPDF_Stream.cc:467-482,508-513` | `crates/flpdf/src/object_handle.rs::prepare_stream_filter_plan`（private, `crates/flpdf/src/object_handle.rs:6609-6719`） | prod: 1 (`crates/flpdf/src/object_handle.rs:6469`) / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::prepare_stream_filter_plan` | 旧 `stream_filter_capabilities` は `.40` で撤去。これは旧routeの完了記録であり、qpdf `filterable` 全責務の完了を意味しない。`parsed_offset` warning経路の分裂は `.48.37` で解消済み（C7行参照）。 |
| C9 | 同上（legacy spec 読み取り route） | `libqpdf/QPDF_Stream.cc:386-461` | 撤去済み（旧 `stream_filter.rs` の spec reader） | prod: 0 / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::prepare_stream_filter_plan` | `.48.96` で recovering compatibility route とともに削除。現在の filter shape、factory lookup、`/DecodeParms`、length、`setDecodeParms` は `prepare_stream_filter_plan` が qpdf順で所有し、qpdfに無い chain length capも残さない。 |
| C10 | `QPDF_Stream::filter_factories` の built-in factory lookup | `libqpdf/QPDF_Stream.cc:72-94,419-435` | `crates/flpdf/src/stream_filter.rs::stream_filter_for`（`pub(crate)`） | prod: 2 (`crates/flpdf/src/object_handle.rs`) / test: 1 (`crates/flpdf/src/stream_filter.rs`) | canonical | `crates/flpdf/src/stream_filter.rs::stream_filter_for` | built-in lookup は qpdf と同じ7名称（`Crypt`/`FlateDecode`/`LZWDecode`/`ASCII85Decode`/`ASCIIHexDecode`/`RunLengthDecode`/`DCTDecode`）。built-inとruntime登録は同じ `BTreeMap` を読むため、固定 `match` の第二registryは残らない。alias展開と未知名の扱いは C7/C11 のcanonical plannerで共有する。 |
| C11 | `QPDFStreamFilter` 抽象と `QPDF::registerStreamFilter` 公開拡張点 | `include/qpdf/QPDF.hh:186-194`, `libqpdf/QPDF.cc:295-300`, `libqpdf/QPDF_Stream.cc:72-94,147-152,419-485`, `include/qpdf/QPDFStreamFilter.hh:26-66`, `libqpdf/QPDFStreamFilter.cc:1-19`, `include/qpdf/Pipeline.hh:19-33,46-60,98-108` | `crates/flpdf/src/stream_filter.rs::StreamFilter`（trait, `pub`）/ `crates/flpdf/src/stream_filter.rs::register_stream_filter`（`pub`）/ `crates/flpdf/src/pipeline.rs::PipelineRef`（`pub`）/ `crates/flpdf/src/stream_filter.rs::OwnedDecodePipeline`（`pub`） | prod: built-in 7実装 + canonical planner / test: 9（`stream_filter_registry_tests`） | canonical | `crates/flpdf/src/stream_filter.rs::StreamFilter` | qpdfの4つのvirtual hook、process-globalな名前置換、alias展開後のfactory lookup、全factory構築後の未知名false、full `/DecodeParms` handle、decode-only登録を公開APIとcanonical ObjectHandle routeへ接続した。PDF名payloadにescaped slashが含まれる場合も区切りslashを常に1つ付け、通常の `/Foo` と `//Foo` の登録キーを分離する。Rustの `OnceLock`/`Mutex` は安全な共有実装の詳細であり、qpdfのlock semanticsは主張しない。qpdfがfilter instanceに保持するPipelineをRustでは `OwnedDecodePipeline`/`PipelineRef` で所有する。private warning adapterは公開traitへ漏らさない。 |
...
| C13 | `QPDF::pipeForeignStreamData` / `ForeignStreamData` | `libqpdf/QPDF.cc:110-124,2564-2585` | `crates/flpdf/src/reader/resolver.rs::original_stream_data_provider_for_destination`（`pub(crate)`, `crates/flpdf/src/reader/resolver.rs:1231`） | prod: 2 (`crates/flpdf/src/reader/resolver.rs:1208,1224`。`crates/flpdf/src/object_handle.rs:423` は `DocumentResolver` trait の既定メソッド宣言で caller ではない) / test: 3 (`crates/flpdf/src/reader/resolver.rs:6278,6355`, `crates/flpdf/src/object_handle.rs:8726`) | canonical | `crates/flpdf/src/reader/resolver.rs::original_stream_data_provider_for_destination` | source 側の `StreamInput`/encryption state/objgen/offset/length と destination dictionary を凍結し、destination resolver を warning sink にする点まで qpdf 通り（`description_override` が qpdf の `file` 引数、`crates/flpdf/src/reader/resolver.rs:4781-4788`） |
| C14 | `QPDF::decryptStream`（method 決定 + AES/RC4 前置） | `libqpdf/QPDF_encryption.cc:1044-1154`（唯一の呼び出し元 `libqpdf/QPDF.cc:2491`） | `crates/flpdf/src/reader/resolver.rs::inspect_stream_encryption`（private, `crates/flpdf/src/reader/resolver.rs:5129-5204`）+ `pipe_stream_data_from_input`（薄い wrapper、実体は `pipe_stream_data_from_qpdf_obj_gen` の復号分岐、`crates/flpdf/src/reader/resolver.rs:4823-4968`） | `inspect_stream_encryption` prod: 1 (`crates/flpdf/src/reader/resolver.rs:4851`) / test: 0 | canonical | `crates/flpdf/src/reader/resolver.rs::pipe_stream_data_from_input` | **復号は pipe 時のみ**（resolve 時ではない）という qpdf の境界を保持。`/XRef` early return、V≥4 gate、`/Crypt` の dict/array 両形、Metadata より Crypt 優先、unknown warning + `cf_stream` 書き換えまで 1:1（`docs/qpdf-correspondence.md:317` の ✅ と一致） |
| C15 | `QPDF::decryptString`（parse 時の文字列復号） | `libqpdf/QPDF_encryption.cc:976-1039`, `libqpdf/QPDF.cc:1330-1340` | `crates/flpdf/src/reader/resolver.rs::decrypt_string`（`StringDecrypter` impl, `crates/flpdf/src/reader/resolver.rs:885`） | prod: parse 経路 (`crates/flpdf/src/reader/resolver.rs:3983-3990`) / test: 同ファイル | canonical | `crates/flpdf/src/reader/resolver.rs::decrypt_string` | stream と string で復号タイミングが違うという qpdf の構造を保持（`crates/flpdf/src/reader/resolver.rs:811-812` が明記） |
| C16 | `QPDF::getKeyForObject`（1 object 分の key cache） | `libqpdf/QPDF_encryption.cc:954-974` | `crates/flpdf/src/encryption/state.rs::key_for_qpdf_obj_gen`（`pub(crate)`, `crates/flpdf/src/encryption/state.rs:184-197`） | prod: 3（`crates/flpdf/src/reader/resolver.rs:4903,4906` の stream 復号 2 箇所と、`crates/flpdf/src/encryption/state.rs:145` の `decrypt_object_string_qpdf_obj_gen`——string 復号側）/ test: 2 直接呼び（`crates/flpdf/src/reader/resolver.rs`）+ `key_for_object` 経由 7。`python3 scripts/qpdf-route-callers.py --root . --symbol key_for_qpdf_obj_gen` の集計は直接呼びのみを数えるので prod 3 / test 2 と出る | canonical | `crates/flpdf/src/encryption/state.rs::key_for_qpdf_obj_gen` | cache key が `og` のみで `use_aes` を含まない点まで qpdf 通り（`crates/flpdf/src/encryption/state.rs:176-177` に明記）。**2026-09-20（`.200` citation refresh 中に発見、単なる行ずれ以上の変化）**: `ObjectRef` を取る `key_for_object`（`crates/flpdf/src/encryption/state.rs:178-182`）は現在 `#[cfg(test)]` 専用の thin wrapper で、`QpdfObjGen` を取る `key_for_qpdf_obj_gen` へ委譲するのみ。production の呼び出し元（`pipe_stream_data_from_qpdf_obj_gen`）は最初から `QpdfObjGen` を持っているため後者を直接呼ぶ。canonical owner を `key_for_qpdf_obj_gen` へ更新した — 出力バイトに影響しない「入れ物」の統一で CLAUDE.md 分類 (B) 相当だが、記録が本行にしかなく `docs/qpdf-correspondence.md` には無い。 **2026-09-20 訂正**: この行は当初 prod を 2 と書き、stream 復号だけが production consumer であるかのように読めた。実際は `decrypt_object_string_qpdf_obj_gen`（`encryption/state.rs:145`）も直接呼んでおり、**string 復号と stream 復号の両方**がこの canonical 関数を使う——qpdf の `getKeyForObject` が `decryptString` と `decryptStream` の双方から呼ばれるのと同じ形。test 側も `key_for_object` 経由 7 件に加えて直接呼びが 1 件ある。 |
...
| C22 | `QPDFWriter::willFilterStream` の probe 呼び（`skip_stream_parameters`） | `libqpdf/QPDFWriter.cc:2543-2551` | `crates/flpdf/src/writer/plain/body.rs::canonical_stream_filter_probe`（private, `crates/flpdf/src/writer/plain/body.rs:2414-2472`） | prod: 2 (`crates/flpdf/src/writer/plain/body.rs:2390,2411`) / test: 0 direct（wrapper 経由で `crates/flpdf/src/writer/plain/body.rs:2880,3099` から exercise） | canonical | `crates/flpdf/src/writer/plain/body.rs::canonical_stream_filter_probe` | 2026-09-18（`flpdf-8od1h`、PR #2127）: `writer/plain/body.rs::canonical_stream_will_be_refiltered_with_policy`（`crates/flpdf/src/writer/plain/body.rs:2380-2396`。現在の唯一の production caller は linearization 側の非 modified-stream 分岐、`crates/flpdf/src/linearization/plan.rs:214`）が持っていた `is_data_modified()` なら probe 前に `false` を返す早期 return を撤去し、canonical `canonical_stream_filter_probe` へ統一した。qpdf の `willFilterStream` にこの早期 return は無く、`isDataModified()` は `filter` を **立てる** 入力の一つに過ぎない（`bool filter = (stream.isDataModified() \|\| m->compress_streams \|\| m->stream_decode_level);`、`libqpdf/QPDFWriter.cc:1254`）。modified stream を含む library RED/GREEN テスト（`modified_streams_use_the_canonical_refilter_probe`、`crates/flpdf/src/writer/plain/body.rs:3079`）が probe が一度は raw pipe を踏むことを `source_last_offset` の変化で固定し、qpdf-zlib-compat の byte 比較も維持されている。2026-09-17（`flpdf-q2nka`）: linearized probeはfilter planが無い場合もraw `pipeStreamData`を1回実行し、planがある場合もqpdf同様warning配送を抑止せずprobeする。`broken-lzw.pdf`等10 fixtureでqpdfのlinearize warning pair回数・stderr・qpdf-zlib-compat bytesを一致させる。plain 側の早期 return が解消されたことで、両 route はともに qpdf の `willFilterStream` を 1:1 で辿るため `canonical` へ再分類する。 |
...
| C26 | （C25 の下請け）filter chain の canonical decode | `libqpdf/QPDF_Stream.cc:487-638`（qpdf に whole-buffer 専用 route は無い） | `crates/flpdf/src/object_handle.rs::get_stream_data` / `::pipe_stream_data` | prod: 0 legacy whole-buffer callers / test: 0 legacy whole-buffer callers | canonical | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `.45` が `driver/test_34_41.rs` の qpdf test 36 consumer を raw `PlFlate` pipeline に移行し、`.48.49` が qtest comparator の decoded comparison を `ObjectHandle::get_stream_data(DecodeLevel::Generalized)` へ移行した。旧 whole-buffer decode route は撤去済み |
| C27 | 同上（`ObjectHandle`-native 版） | `libqpdf/QPDF_Stream.cc:487-638` | `crates/flpdf/src/object_handle.rs::get_stream_data` | prod: 0 legacy whole-buffer callers / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::get_stream_data` | `.48.49` が bootstrap xref stream consumer (`libqpdf/QPDF.cc:1051`) を `ObjectHandle::get_stream_data(DecodeLevel::Specialized)` へ移行した。通常 resolver-backed handle と owner-less bootstrap handle が同じ qpdf-shaped pipe boundary を使い、旧 handle-based whole-buffer decode route は撤去済み |
| C28 | 同上（qpdfにない recovering/limit 公開 API） | 対応物なし（qpdf は部分デコード結果・DecodeLimits APIを持たない） | 撤去済み（旧 `filters.rs` の recovery result/event/limit module） | prod: 0 / test: 0 | canonical | `absent` | `.48.96` で qtest consumer cutover後に public recovery API、partial event/result、max-output/TIFF-memory/filter-chain hardening、legacy implementationを削除。canonical `ObjectHandle::pipe_stream_data` と `get_stream_data` は保持する。 |
| C29 | `pipeStreamData` の `qpdf_ef_compress`（deflate 段） | `libqpdf/QPDF_Stream.cc:536-542` | ordinary: `crates/flpdf/src/object_handle.rs::pipe_stream_data`; ObjStm direct stage: `crates/flpdf/src/stream_filter.rs::encode_flate` via `writer/object_streams/emission.rs::wrap_objstm_body_as_handle`（C31） | prod: 0 legacy generic encoder callers / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::pipe_stream_data`（ordinary）/ `crates/flpdf/src/stream_filter.rs::encode_flate`（ObjStm direct stage） | `.48.49` が generic whole-buffer encoder と CLI test callersを撤去。ObjStm は qpdf `QPDFWriter.cc:1659-1665` と同じく `pipeStreamData` を通さず direct Flate を接続する |
...
| C31 | `QPDFWriter::writeObjectStream` の ObjStm 圧縮（`willFilterStream` を通らない） | `libqpdf/QPDFWriter.cc:1659-1665,1719-1721` | `crates/flpdf/src/writer/object_streams/emission.rs::wrap_objstm_body_as_handle`（`pub(crate)`, `crates/flpdf/src/writer/object_streams/emission.rs:183-225`） | prod: 1 (`crates/flpdf/src/linearization/writer.rs:435`) / test: 2 (`crates/flpdf/src/writer/object_streams/emission.rs:240,262`) | canonical | `crates/flpdf/src/writer/object_streams/emission.rs::wrap_objstm_body_as_handle` | linearized container wrapperだけがこの handle-native wrapperを使う。非linearized の plain/legacy ObjStm consumer はそれぞれの sink serializerで直接 `encode_flate` を行う。qpdf と同じく `pipeStreamData` を通さず deflate を直付けし、compressed/uncompressed の payload ownerを handle と sinkで共有する |
...
| C39 | `QPDF_Stream::setFilterOnWrite` / `getFilterOnWrite` / `isDataModified` / `addTokenFilter` | `libqpdf/QPDF_Stream.cc:154-164,320-324,662-666`, `libqpdf/QPDFObjectHandle.cc:1264-1274` | `crates/flpdf/src/object_handle.rs::set_filter_on_write` / `::get_filter_on_write` / `::is_data_modified`（`pub(crate)`）/ `::add_token_filter`（`pub`） | `get_filter_on_write` prod: 1 (`crates/flpdf/src/writer/plain/body.rs:2670`) / test: 4 (`crates/flpdf/src/object_handle.rs:19982,19991,19998,20012`)。`is_data_modified` prod: 3 (`crates/flpdf/src/writer/plain/body.rs:2720,2727,2746`（**2026-09-19 再計測**、旧記載 `:2704,2711,2730` は stale で無関係な行を指していた）) / test: 4 (`crates/flpdf/src/object_handle.rs:11814,11828,11842,11848`)。`add_token_filter` prod: 4 (`crates/flpdf/src/form_field_object_helper/rendering.rs:388`, `crates/flpdf/src/object_handle.rs:6147`, `crates/flpdf/src/page_object_helper.rs:1288`, `crates/flpdf-qtest-tools/src/driver/test_72_79.rs:241`) / test: 4 | canonical | `crates/flpdf/src/object_handle.rs::set_filter_on_write` 他（qpdf と同じく `QPDF_Stream` state の直接アクセサ） | `docs/qpdf-correspondence.md:354`（setFilterOnWrite/getFilterOnWrite、✅）, `docs/qpdf-correspondence.md:1638`（isDataModified/addTokenFilter、✅）と一致（`:262` は無関係の closed-source エントリを指していた誤citationだったため修正）。`is_data_modified` の consumer が writer 3 route に集約されているのは、2026-09-18（`flpdf-8od1h`）で C22 の早期 return が撤去された結果である。撤去直後は `linearization/plan.rs` 側に `is_data_modified()` で `canonical_stream_filter_probe_for_linearization` と `canonical_stream_will_be_refiltered_with_policy(..., true, ...)` を同じ実引数で呼び分けるだけの vestigial な分岐が残っていたが、`flpdf-3yn9.48.184` でその分岐を撤去し `canonical_stream_will_be_refiltered_with_policy(handle, options, true, normalize_content)` の単一呼び出しへ一本化した（出力への影響は無く、C22 の分類にも影響しない） |
...
| C41 | `QPDF::readStream` の `/Length` 検証 + `endstream` 確認 | `libqpdf/QPDF.cc:1361-1399` | `crates/flpdf/src/reader/resolver.rs::read_stream`（private, `crates/flpdf/src/reader/resolver.rs:4229-4347`） | prod: 1 (`crates/flpdf/src/reader/resolver.rs:4123`) / test: 0 | canonical | `crates/flpdf/src/reader/resolver.rs::read_stream` | 3 つのメッセージ（"stream dictionary lacks /Length key" / "/Length key in stream dictionary is not an integer" / "expected endstream"）と `attempt_recovery` 分岐が 1:1（`crates/flpdf/src/reader/resolver.rs:4353-4362`、2026-09-20 citation refresh 時点で "/Length key in stream dictionary is out of range" の 4 件目が追加されている） |
...
- **C4**: `.48.38` で `reader/resolver.rs` の ObjStm consumer は通常の
  `getStreamData(qpdf_dl_specialized)` を呼ぶ canonical routeへ揃え、残っていた
  専用 source error flag / error map を撤去した。qpdf は `QPDF.cc:1792` の例外を
  `QPDF::resolve`（`QPDF.cc:1737-1744`）で扱う。replaced buffer / provider / original
  source と invalid Filter の warning/null/throw 境界を resolver tests で固定した。
- **C8 / C25**: `qpdf_raw_stream_payload`、`stream_payload_for_decode_level`、
  `stream_payload_with_decode_status`、旧 `stream_filter_capabilities` は
...
- **C27**: `xref.rs:3525` の **xref stream**（qpdf `QPDF.cc:1051`）は
  `.48.49` で bootstrap `XrefReadContext` も `ObjectHandle::get_stream_data(DecodeLevel::Specialized)`
  を使うように cutover した。bootstrap owner の bounded source-read policy は resolver に残るが、
  materialized whole-buffer decode helperは撤去済みである。
  `resources.rs:277` の Form pre-pass と `filespec_helper/embedded_file_stream.rs` の `payload` は
  それぞれ別の canonical consumer scope（後者は `.48.42` 済み）として扱う。
- **C26 / C29 cutover**: `.48.49` で qtest comparator の decoded comparison は
  `ObjectHandle::get_stream_data(DecodeLevel::Generalized)`、CLI fixture builders は direct
  `PlFlate`/valid RunLength bytesへ移行した。ordinary stream の compression は
  `ObjectHandle::pipe_stream_data`、ObjStm compression は qpdf と同じ direct `Pl_Flate`（C31）で、
  generic whole-buffer encoder は残さない。
- **C28**: `.48.93` で qtest test 0/1 の stream branch を qpdf の
  `pipeStreamData` 相当（raw pipe、source-read-free filterability probe、filtered pipe）へ
  移行した。`.48.96` で、qpdf に対応物のない recovering result/event、DecodeLimits、
  output/TIFF-memory/filter-chain hardening、legacy moduleを公開面ごと撤去した。
- **C28 warning attribution**: `flpdf-hpq1` で、qtest test 0/1 の非辞書
  ...
  `libqpdf/QPDFValue.cc:14-61`）。この qtest warning attribution は canonical
  `ObjectHandle` pipeへ移行済みで、recovering APIは `.48.96` で撤去済み。
- **C29**: appearance consumer の owner は `QPDFJob::handleUnderOverlay` ではなく
  **`QPDFAcroFormDocumentHelper::adjustAppearanceStream`**
  ...
  `ResourceReplacer` を `addTokenFilter` する（同ファイル `:680-690`）。
  `flpdf-1far` で `overlay_appearance_stream.rs` の appearance consumer を
  `parseAsContents` 後の `ResourceReplacer::addTokenFilter` へ移行し、eager
  decode/re-encode caller を削除した。正常・QDFの overlay出力を qpdf 11.9.0
  と byte 比較し、linearized の二段階 stateful-filter 結果も unit で固定した。
  既存の malformed/truncated appearance warning tests も同じ live route で
  通過する。ObjStm emission の
  `writer/object_streams/emission.rs:171` は qpdf も直接 Flate を接続する別責務
  （C31）なので、ordinary stream の pipe へ一括移行しない。
- **C26 の harness consumer**: `.45` で `driver/test_34_41.rs:551` を
  qpdf `test_driver.cc:1329-1331` の raw stream pipe → bare `Pl_Flate` に移行した。
  `.48.49` で `compare.rs` の decoded comparison も canonical `get_stream_data` へ移行した。
...
**この移行により C7 を `mixed` としていた診断分裂は解消した**。さらに `.48.96` で
C9 の重複 legacy spec readerを削除したため、C7 は `canonical` へ再分類する。
C10/C11 の runtime `registerStreamFilter` は、built-in lookupと同じregistryを読む公開
契約として `.48.46` でcanonical化した。C21 は `writer/object.rs::prepare_stream_dict_entries`（`crates/flpdf/src/writer/object.rs:1709-1712`）が
`/F` / `/FFilter` / `/FDecodeParms` を全分岐で触らない契約を doc comment と
`tests/oracle/qpdf_refiltered_stream_dictionary_probe.cc` で固定しており、`canonical` である。
...
<!-- route-matrix-aggregate: document-tally unit=area-physical file=c-stream-pipeline-encryption.md -->

| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 42 | C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13, C14, C15, C16, C17, C18, C20, C21, C22, C24, C25, C26, C27, C28, C29, C30, C31, C32, C33, C34, C35, C36, C37, C38, C39, C40, C41, C42, C43, C44 |
| mixed | 0 | — |
| bridge | 0 | — |
| unknown | 0 | — |
...
性に乖離は無い。**2026-09-19（`flpdf-3yn9.48.196`）**: C44 は単一 flpdf entrypoint が単一 qpdf 責務
（`getStreamJSON`。内部の `StreamBlobProvider` は別責務ではない）に対応しており、§3 の `mixed` 定義の
いずれの分岐にも該当しないため、README §3 の履歴行例外から canonical へ再分類した。probe 結果は
引き続き production route 重複や全体 parity の証拠には広げない。

`bridge` の判定基準は README §3 の通り **経路（route）に qpdf 対応物が無いこと** で、
責務（responsibility）に qpdf 対応物があるかどうかとは別に問う。本領域の 2 行はこの区別で読む:
C28/C43 は責務のレベルでも qpdf に対応物が無い qpdf-less route だったが、
`.48.95` / `.48.96` で撤去済みである。qpdf 側にも実装にも対応物がある複数実装は
`bridge` ではなく `mixed` に置く。旧C19は`.42`で削除した。
...
| `C28` | `flpdf-3yn9.48.44` | qtest test0/1のwarning attributionをcanonical handleへ移す（完了済みの前段） |
| `C28` | `flpdf-3yn9.48.93` / `flpdf-3yn9.48.96` | qtest test0/1をcanonical pipe/loggerへ移し、qpdf-less recovering result/event・limit APIとlegacy moduleを公開面ごと撤去する |
