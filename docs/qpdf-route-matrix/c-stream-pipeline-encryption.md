...
| C10 | `QPDF_Stream::filter_factories` の built-in factory lookup | `libqpdf/QPDF_Stream.cc:85-94,419-435` | `crates/flpdf/src/stream_filter.rs::stream_filter_for`（`pub(crate)`, `crates/flpdf/src/stream_filter.rs:1802-1813`） | prod: 8 (`crates/flpdf/src/stream_filter.rs:230,254,517,1894`, `crates/flpdf/src/filters.rs:51,673,813`, `crates/flpdf/src/object_handle.rs:6660`) / test: 0 | canonical | `crates/flpdf/src/stream_filter.rs::stream_filter_for` | built-in lookupの集合は qpdf と一致（`Crypt`/`FlateDecode`/`LZWDecode`/`ASCII85Decode`/`ASCIIHexDecode`/`RunLengthDecode`/`DCTDecode`）。`registerStreamFilter` に対応する **runtime 登録 API は存在しない**（`match` registry。`crates/flpdf/src/stream_filter.rs:1790-1797` が CLAUDE.md 分類 (B) の逸脱として自己記録済み、`docs/qpdf-correspondence.md` §1 でも 🔀）。runtime公開契約の未移植は `flpdf-3yn9.48.46` が別に所有する。canonicalはbuilt-in lookupに限定する。この行に `// qpdf-deviation:` マーカーは無い（`stream_filter.rs` 唯一のマーカー `crates/flpdf/src/stream_filter.rs:1773-1777` は registry ではなく `StreamFilter` trait 側に付いている — C11 参照） |
...
| C21 | `QPDFWriter::willFilterStream` の 2 回試行・buffer 保持と `unparseObject` の出力辞書加工 | `libqpdf/QPDFWriter.cc:1287-1315` | `crates/flpdf/src/writer/plain/body.rs::canonical_stream_output_with_rewrite_policy`（private, `crates/flpdf/src/writer/plain/body.rs:756-884`） | prod: 1 (`crates/flpdf/src/writer/plain/body.rs:202` = `canonical_stream_output_with_status`。そこから plain / linearized 両 route が入る) / test: 0 | mixed | `crates/flpdf/src/writer/plain/body.rs::canonical_stream_output_with_rewrite_policy` | attempt 1/2、`filtering_attempted && success`、`stream_data_error` の `"error while getting stream data for …"` 前置（`libqpdf/QPDFWriter.cc:1300-1303`）が 1:1。**責務混在と追加キー削除の差**: `crates/flpdf/src/writer/plain/body.rs:860-865` が `/F` `/FFilter` `/FDecodeParms` も削除するが、qpdf の `unparseObject` は `/Filter` `/DecodeParms` だけを削除する（`libqpdf/QPDFWriter.cc:1451-1455`）。`// qpdf-deviation:` マーカー無し |
...
- **C8 / C25**: `qpdf_raw_stream_payload` と `stream_payload_for_decode_level` の
  root caller は workspace 内では unit tests のみ。`document_json` は既に C24 を呼ぶ。
  旧 API 内部の production 呼び出し 2 本を、未移行の JSON 出力 consumer と数えない。
  closed `flpdf-3yn9.9` は C24 の移植であり、これら旧 API の撤去までは完了していない。
...
| 分類 | 件数 | 行 |
|---|---|---|
| canonical | 26 | C1, C2, C3, C5, C6, C10, C11, C12, C13, C14, C15, C16, C20, C24, C30, C31, C32, C33, C34, C35, C36, C37, C38, C39, C40, C41 |
| mixed | 14 | C4, C7, C8, C9, C17, C18, C21, C22, C25, C26, C27, C29, C42, C44 |
| bridge | 2 | C28, C43 |
| unknown | 0 | なし（C42 の pipe/show-stream EOL subtraction は `flpdf-zvjf` と `flpdf-hj7v` で qpdf parity として解決） |

U3-U4 は「分類は決まっているが、残る実装差と出力への影響を検証する」項目なので
`unknown` 行にはせず、対応する mixed 行（C22 / C17-C18）から参照している。
