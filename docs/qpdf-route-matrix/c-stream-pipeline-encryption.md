...
| C28 | 同上（recovering 版・explicit limits 版の公開 API） | 対応物なし（qpdf は部分デコード結果を返す API を持たない） | `crates/flpdf/src/filters.rs::decode_stream_data_recovering` / `::decode_stream_data_recovering_with_limits` | `decode_stream_data_recovering` prod: 0 / test: 1；`decode_stream_data_recovering_with_limits` prod: 2 (wrapper + qtest exception) / test: 2 | bridge | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `.48.49` では qtest exception 経路を明示的に別セッションへ残す。したがって recovering event/`DecodeLimits` API と test 0/1 caller は未撤去で、今回のPRでC28全体の完了は主張しない |
...
| C42 | `QPDF::recoverStreamLength` | `libqpdf/QPDF.cc:1482-1530` | `crates/flpdf/src/reader/resolver.rs::recover_stream_length`（private） | prod: canonical resolver の stream recovery 呼び出し / test: recovery fixture と EOL metadata tests | mixed | `crates/flpdf/src/reader/resolver.rs::recover_stream_length` | warning 3 種、`endobj` 巻き戻し、全 recovered length の計算は 1:1。`RecoveredStreamEol` は `dump-object` 再シリアライズのframing専用metadataとして残り、C12のAES/RC4 pipeとshow-objectのraw/filtered payloadはqpdfと同じく `length` の全spanをそのまま渡す（`flpdf-zvjf`, `flpdf-hj7v`）。`.3gdi` で dump-object 側の metadata key を `(QpdfObjGen, stream-data parsed offset)` にし、同一 object の `/Prev` 別 offset read による誤trimを防ぐ。 |
| C43 | （なし — flpdf 固有の分類 helper） | 対応物なし | `crates/flpdf/src/filters.rs::passthrough_codec_label`（`pub`） | `passthrough_codec_label` prod: 0 / test: 0 | bridge | `crates/flpdf/src/stream_filter.rs::passthrough_codec_label` | CLI の独自 binary marker は削除し、共通の未デコード filter error 用の codec 分類だけを保持する。qpdf の `filterable` はこの表示用 helper に対応しない。 |
...
- **C28**: qpdf に対応物のない recovering event API は qtest exception session の対象として
...
- **C42 / C43**: EOL metadata は `job/inspection.rs:273` の独自 `dump-object` framing に残る。
  CLI の独自 binary marker は削除し、共通の未デコード filter error 用の分類 helper だけを保持する。
  qpdf `QPDFJob.cc:806-832` は stream dictionary 表示と pipe を使い、shortcut を持たない。
  `.3gdi` はこの qpdf 非対応 extension 自体を削除せず、recovery EOL の所有を
  `QpdfObjGen` と stream-data parsed offset の組にして、旧 `/Prev` read の metadata
  が現行 stream に漏れないことを固定する。
...
| `C42` / `C43` | `flpdf-3yn9.48.48` | dump-object framingとCLI専用 binary-label 迂回をcanonical CLI cutover後に撤去する |
