...
| C12 | `QPDF::Pipe::pipeStreamData` / static `QPDF::pipeStreamData`（original source 読み出し + error boundary） | `include/qpdf/QPDF.hh:819-839`, `libqpdf/QPDF.cc:2476-2562` | `crates/flpdf/src/reader/resolver.rs::pipe_stream_data`（`pub(crate)`, `crates/flpdf/src/reader/resolver.rs:2542-2574`）→ `crates/flpdf/src/reader/resolver.rs::pipe_stream_data_from_input`（private, `crates/flpdf/src/reader/resolver.rs:3664-3808`） | `pipe_stream_data_from_input` prod: 2 (`crates/flpdf/src/reader/resolver.rs:2560` = `ResolverHandle::pipe_stream_data`、`crates/flpdf/src/reader/resolver.rs:770` = C13 の foreign provider) / test: 2 (`crates/flpdf/src/reader/resolver.rs:6322,6351`)。`ResolverHandle::pipe_stream_data` は `DocumentResolver` trait impl（`crates/flpdf/src/reader/resolver.rs:4287-4297`）越しに `crates/flpdf/src/object_handle.rs:6856` から呼ばれる | canonical | `crates/flpdf/src/reader/resolver.rs::pipe_stream_data_from_input` | 復号前置 → seek → 一括 read → write/finish → 失敗時 warning の順序と 2 種の catch arm が 1:1。C42 の EOL 差し引きだけが未マークの追加 |
...
| C19 | 同上（第 3 の実装、呼び出し元ゼロ） | `libqpdf/QPDF_encryption.cc:324-357` | `crates/flpdf/src/encryption/keys.rs::per_object_key`（`pub(crate)`, `crates/flpdf/src/encryption/keys.rs:44-55`） | prod: 0 / test: 0 | mixed | `crates/flpdf/src/encryption/state.rs::compute_data_key` | C17 の「qpdf は 1 実装、flpdf は 3 実装」の 3 本目なので `bridge`（それ自体に qpdf 対応物が無い翻訳層）ではなく `mixed` に分類する — 責務にも実装にも qpdf 対応物（`QPDF::compute_data_key`）がある。残 caller ゼロ = 削除候補（`crates/flpdf/src/encryption/keys.rs:4` の `#![allow(dead_code)]` が dead-code 警告を抑えているため気付かれずに残っている。intra-doc リンクからの参照が `crates/flpdf/src/encryption/standard.rs:884,1520` と `crates/flpdf/src/encryption/state.rs:140` にあるだけ）。truncation が `min(file_key.len() + 5, 16)` で qpdf の `min(連結後入力長, 16)` と式が違う（`crates/flpdf/src/encryption/keys.rs:39-43` が自己申告）。標準ハンドラが admit する `/V`,`/R` の全組で結果は一致するが、qpdf の式ではない |
...
| C23 | `QPDFWriter::willFilterStream` 相当の公開 helper | `libqpdf/QPDFWriter.cc:1238-1315`（qpdf 側は private） | `crates/flpdf/src/writer.rs::apply_stream_compress_policy`（**`pub`**, `crates/flpdf/src/writer.rs:5489-5497`）→ `crates/flpdf/src/writer.rs::apply_stream_compress_policy_with_decode_level`（private, `crates/flpdf/src/writer.rs:5499-5599`） | prod: **0**（`crates/flpdf/src/lib.rs:236` は `pub use` の再輸出行のみ） / test: 2 (`crates/flpdf/src/writer.rs:5980,5996`) + 3 (private 版 `crates/flpdf/src/writer.rs:6026,6044,6059`) | bridge | `crates/flpdf/src/writer/plain/body.rs::canonical_stream_filter_plan` | **route と責務を分けて読む**: 責務（`willFilterStream`）には qpdf 対応物があるが、`bridge` が問うのは **経路** の対応物であり、この経路 — `pub` の whole-buffer wrapper — には qpdf 側に対応物が無い（qpdf の `willFilterStream` は公開されず、writer の内部からしか呼ばれない）。production caller ゼロ。`pipe_stream_data` を通らず `get_raw_stream_data` → `filter_chain_is_decodable`（`crates/flpdf/src/writer.rs:5621`、qpdf に対応物なし）→ `filters::decode_stream_data` → `filters::encode_stream_data` の whole-buffer route。CLAUDE.md 分類 (B) 根拠 8 の 3 根拠すべてに該当しない: (1) qpdf 側 `willFilterStream` は `include/qpdf/QPDFWriter.hh:440` の `private:` 配下、(2) flpdf-cli からの呼び出し無し、(3) `crates/flpdf/src/lib.rs` 冒頭 `//!` doc に記載なし（`merge_documents`/`extract_pages` は記載あり）。cutover 最小単位: この `pub` を落とせば `apply_stream_compress_policy_with_decode_level` / `filter_chain_is_decodable` / `stream_dictionary_copy` 経由の subtree ごと外せる |
...
| C26 | （C25/C23 の下請け）filter chain の whole-buffer decode | `libqpdf/QPDF_Stream.cc:487-638`（qpdf に whole-buffer 専用 route は無い） | `crates/flpdf/src/filters.rs::decode_stream_data`（`pub`, `crates/flpdf/src/filters.rs:114-121`） | prod: 5 (`crates/flpdf/src/json_inspect.rs:274`, `crates/flpdf/src/writer.rs:5527`, `crates/flpdf-qtest-tools/src/compare.rs:85,89`, `crates/flpdf-qtest-tools/src/driver/test_34_41.rs:519`。後ろ 3 件は parity harness だが、`#[cfg(test)]`（`compare.rs:299` / `test_34_41.rs:814`）より前にあるので production に数える) / test: 3 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | flpdf 本体の残 production caller は C25（json_inspect）と C23（caller ゼロの `pub` helper）の 2 つだけ。source dispatch も復号も token filter も持たず、呼び出し側が別途取得した `&[u8]` を受け取る |
| C27 | 同上（`ObjectHandle`-native 版） | `libqpdf/QPDF_Stream.cc:487-638` | `crates/flpdf/src/filters.rs::decode_stream_data_from_handle`（`pub(crate)`, `crates/flpdf/src/filters.rs:426-438`） | prod: 4 (`crates/flpdf/src/filespec_helper/embedded_file_stream.rs:273`, `crates/flpdf/src/resources.rs:269`, `crates/flpdf/src/xref.rs:752,3266`) / test: 0 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | `docs/qpdf-correspondence.md:312` の「production 経路の `decode_stream_data`/`encode_stream_data` 呼び出しはテストコードのみに残る」に反する現存 production caller。`crates/flpdf/src/xref.rs:752,3266` は xref stream 読み出しで、qpdf 側の対応は `libqpdf/QPDF.cc:1051` の `getStreamData(qpdf_dl_specialized)` |
| C28 | 同上（recovering 版・limits 版の公開 API） | 対応物なし（qpdf は部分デコード結果を返す API を持たない） | `crates/flpdf/src/filters.rs::decode_stream_data_recovering`（`pub`, `crates/flpdf/src/filters.rs:187-192`）/ `::decode_stream_data_recovering_with_limits`（`crates/flpdf/src/filters.rs:202-208`）/ `::decode_stream_data_with_limits`（`crates/flpdf/src/filters.rs:313-324`） | prod: 1 (`crates/flpdf-qtest-tools/src/driver/test_0_1.rs:332` = parity harness のみ) / test: 4 (`crates/flpdf/tests/tiff_predictor_memory_tests.rs`, `crates/flpdf/tests/filters_route_cutover_tests.rs`) | bridge | `crates/flpdf/src/object_handle.rs::pipe_stream_data` | **route と責務を分けて読む**: この経路は責務のレベルでも qpdf に対応物が無い（qpdf は部分デコード結果を返す API を持たない）ので、C23 より単純な `bridge`。flpdf 本体に production caller なし。`DecodeLimits`（`crates/flpdf/src/filters.rs:221-235`）は qpdf に対応物のない hardening budget |
| C29 | `pipeStreamData` の `qpdf_ef_compress`（deflate 段） | `libqpdf/QPDF_Stream.cc:536-542` | `crates/flpdf/src/filters.rs::encode_stream_data`（`pub`, `crates/flpdf/src/filters.rs:349-351`）/ `::encode_stream_data_from_handle`（`pub(crate)`, `crates/flpdf/src/filters.rs:373-381`） | `encode_stream_data` prod: 2 (`crates/flpdf/src/writer.rs:5575` = C23 の下、`crates/flpdf-qtest-tools/src/driver/test_02_09.rs:516`) / test: 4。`encode_stream_data_from_handle` prod: 3 (`crates/flpdf/src/overlay_appearance_stream.rs:172,183`, `crates/flpdf/src/writer/object_streams/emission.rs:171`) / test: 0 | mixed | `crates/flpdf/src/object_handle.rs::pipe_stream_data`（ordinary stream）/ `crates/flpdf/src/writer/object_streams/emission.rs::wrap_objstm_body_as_handle`（ObjStm、C31） | `emission.rs:171` の利用は qpdf 側でも `pipeStreamData` を通らない ObjStm 経路（`libqpdf/QPDFWriter.cc:1659-1665`）なので責務としては正しい。`overlay_appearance_stream.rs:172,183` は qpdf の `handleUnderOverlay` が `getRawStreamData()` + `replaceStreamData` で済ませる箇所（`libqpdf/QPDFJob.cc:1993`）に対する再エンコード追加で、qpdf に対応する encode 呼び出しが無い |
...
| C42 | `QPDF::recoverStreamLength` | `libqpdf/QPDF.cc:1482-1530` | `crates/flpdf/src/reader/resolver.rs::recover_stream_length`（private, `crates/flpdf/src/reader/resolver.rs:3381-3426`） | prod: 2 (`crates/flpdf/src/reader/resolver.rs:3278,3302`) / test: 2 | unknown | `crates/flpdf/src/reader/resolver.rs::recover_stream_length` | warning 3 種と `endobj` 巻き戻しは 1:1。**qpdf に対応物のない追加**: 復元 length 末尾の EOL を `RecoveredStreamEol` として記録し（記録は `crates/flpdf/src/reader/resolver.rs:3404-3410`、検出と accessor は `crates/flpdf/src/reader/resolver.rs:3428-3462`）、C12 の復号分岐だけがその長さを `length.saturating_sub(recovered_stream_eol_length)` で差し引く（`crates/flpdf/src/reader/resolver.rs:3778,3793`）。非暗号化分岐は差し引かない = 同一 stream でも暗号化の有無で pipe されるバイト数が変わる。qpdf は `libqpdf/QPDF.cc:2496-2501` で常に `length` バイトを読む。`// qpdf-deviation:` マーカー無し。**この差が補正なのか純粋な追加なのかは source だけでは決められないため `unknown`**（必要な probe は U1）。flpdf 側の実装は 1 本だけで、経路が割れているわけではない |
...
|---|---|---|
| U1 | C42 の EOL 差し引き（`crates/flpdf/src/reader/resolver.rs:3778,3793`）は、flpdf の read 経路のどこかで打ち消される補正なのか、それとも qpdf に対して純粋な追加なのか。打ち消しがあるなら byte 影響なし、無ければ暗号化 + 破損 `/Length` の組み合わせで qpdf と復号バイト列が食い違う | `/Length` を壊した RC4 と AES-128/256 の暗号化 PDF を 1 つずつ作り、`qpdf --password=… --decrypt --static-id in.pdf out-qpdf.pdf` と flpdf の同等呼び出しの stream バイト列を比較する（`qpdf --show-object=N --raw-stream-data` でも可）。加えて `crates/flpdf/src/reader.rs::canonical_recovered_stream_eol`（`crates/flpdf/src/reader.rs:333-356`）の全 consumer を洗い、writer 側で EOL を再付与している箇所があるかを確認する。領域 B（recovery）と跨るため、そちらの担当と突き合わせて決める |
...
| mixed | 12 | C4, C8, C9, C17, C18, C19, C22, C25, C26, C27, C29, C44 |
| bridge | 3 | C23, C28, C43 |
| unknown | 1 | C42（EOL 差し引きが補正か追加かを source で決められない。probe は U1） |

U2-U4 は「分類は決まっているが、その分類が示す差が出力バイトに出るかを未検証」の項目なので
`unknown` 行にはせず、対応する mixed 行（C44 / C22 / C17-C19）から参照している。

`bridge` の判定基準は README §3 の通り **経路（route）に qpdf 対応物が無いこと** で、
責務（responsibility）に qpdf 対応物があるかどうかとは別に問う。本領域の 3 行はこの区別で読む:
C28 / C43 は責務のレベルでも qpdf に対応物が無く、C23 だけは責務（`QPDFWriter::willFilterStream`）に
対応物がある一方で **経路**（公開された whole-buffer wrapper）に対応物が無い。
qpdf 側にも実装にも対応物がある複数実装は `bridge` ではなく `mixed` に置く（C19 をここへ移した）。
