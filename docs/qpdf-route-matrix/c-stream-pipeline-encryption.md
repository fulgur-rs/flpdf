...
| C4 | 同上（ObjStm resolve 経路） | `libqpdf/QPDF.cc:1792`（`getStreamData(qpdf_dl_specialized)` を catch せず呼ぶ） | `crates/flpdf/src/object_handle.rs::get_stream_data`（`pub`, `crates/flpdf/src/object_handle.rs:6180-6213`）→ `crates/flpdf/src/reader/resolver.rs::resolve_object_stream_with_failure_kind` | prod: 1 (`crates/flpdf/src/reader/resolver.rs`) / test: 0 | canonical | `crates/flpdf/src/object_handle.rs::get_stream_data` | qpdf は ObjStm でも通常の `getStreamData` を呼ぶだけで、codec error 回復の分岐を持たない。unfilterable の typed `QpdfExc` fields は `get_stream_data` が保持する。whole-buffer/ObjStm compatibility の残差は `.49` scopeで追跡する |
