...
> ブランチは保護しない（§7 cascade の手動 rebase が
...
| 現 bottom (cursor) | bottom-up で最小の「未 MERGED **かつ** ブランチが origin/main より先行(`git merge-base --is-ancestor origin/<sub> origin/main` が偽)」な sub。**OPEN だけで判定しない** |
| CLOSED-not-MERGED 中間 sub | PR が CLOSED/draft でもブランチが main より先行なら作業が宙吊り。次 PR の base もそれを指す。`gh pr reopen`→`gh pr ready`→(旧 base 削除済なら)`gh pr edit --base main`→再 derive。reopen 失敗→人間フラグ |
| cascade 残あり? | 新 cursor で `git merge-base --is-ancestor origin/main origin/<cursor-branch>` が偽 ⇒ §7-2 手動 rebase 要 |
| cascade 中断? | `.git/rebase-merge` / `.git/rebase-apply` が存在 ⇒ 中断 rebase。`git rebase --abort` で原状復帰 → 人間フラグ |
| cleanup 残あり? | `delete_branch_on_merge=true` でリモートはサーバ側自動削除。ローカル追跡は `git fetch --prune` |
...
> **重要前提**: このリポジトリの epic/sub-* PR は gh-stack のスタック
> として**登録されていない**（base ポインタで連鎖しているだけ。
> `gh stack view` は "not part of a stack" エラー）。よって
> `gh stack merge` / `gh stack sync` は**使えない**。通常 PR マージ
> + 手動 cascade で行う。

1. **Merge（通常 PR、方式 B）**: ゲート全 true の bottom（base は必ず
   main）に `gh pr merge <bottom> --rebase --delete-branch`。**`--rebase`**
   を使う（`--squash` ではない）。`--rebase` は PR のコミットを
   コミット単位で main に乗せるため、次ブランチの **plain**
   `git rebase origin/main` が patch-id 一致でクリーンに drop できる。
   `--squash` は1コミットに潰れ patch-id が個別コミットと一致せず
   plain rebase が壊れる（squash を使うなら cascade を
   `git rebase --onto origin/main "$(gh pr view <pr> --json commits
   --jq '.commits[0].oid')^"` にする＝方式 A）。本リポジトリは
   **方式 B（`--rebase` + plain rebase）に統一**。`--delete-branch` で
   GitHub が次 PR の base を main へ auto-retarget（base ポインタのみ。
   ブランチ内容は下記 rebase 要）。`git push origin main` は ruleset
   不可、必ず `gh pr merge` 経由
2. **Cascade（手動、`gh stack sync` の代替）**: 直近の新 cursor のみ
   今 rebase すればよい（さらに上位はまだ存在するブランチを指すので、
   各々が cursor になった時に遅延 rebase）。新 cursor ブランチ B:
   - `git fetch origin --prune`
   - `git checkout B && git rebase origin/main`
     （親が `--rebase` でマージ済なので、git は親の main 済みコミット
     を patch-id 一致で drop し、B 固有コミットのみ replay。実証済:
     本設計の PR 自体が `--rebase` マージ後に main へ無衝突 rebase）
   - `git push --force-with-lease origin B`
   - rebase 中コンフリクト → `git rebase --abort` → **人間フラグ**
     （手動 rebase は atomic でない。half-rebased で放置せず、abort
     で原状復帰してから停止）
3. cascade の force-push が B の head SHA を変える → CodeRabbit
   再レビュー + roborev 再キックが必要 → §5 状態機械 + §4-3 が処理
   （pause/rate-limit はここで頻発する前提）

> **PR auto-retarget の経路に注意**: bottom がマージされ
> `delete_branch_on_merge=true` でブランチ削除されると、GitHub は
> 次 PR の base を main へ auto-retarget する。これは §7-2 の手動
> rebase/force-push とは**別経路**で、retarget 直後（rebase 前）は
> 次 PR の diff が親の済みコミット分だけ見かけ上膨張する。手動 rebase
> 完了で解消。auto-retarget を CodeRabbit が全再レビューするか無反応
> かは挙動依存（§11 で実機確認）。retarget 直後の CodeRabbit 状態は
> §5 状態機械で必ず分類し直す。

ロールバック: `gh pr merge` は API 完了で原子的（部分マージ無し）。
**手動 cascade rebase は atomic でない**ため、中断時はブランチが
rebase 進行中で残り得る → derive_state が `.git/rebase-merge` /
`rebase-apply` の存在を検出し、`git rebase --abort` で原状復帰してから
人間フラグ。明示 rollback は不要 — **冪等再実行 + abort** が安全策。
...
- 停止して人間へ:
  - Resolve が反復上限で非収束
  - CI failure が修正で直らない
  - 手動 cascade rebase コンフリクト（`git rebase --abort` 後）
  - Compat matrix baseline drift で要判断
  - トリアージで「defer 可否不明」が出た finding
  - CLOSED-not-MERGED 中間 sub の `gh pr reopen` 失敗
