...
1. **Merge（通常 PR）**: ゲート全 true の bottom（base は必ず main）
   に `gh pr merge <bottom> --squash --delete-branch`。`--squash` で
   PR 1本=main 上1コミット（テンプレの PR 単位と一致）。
   `--delete-branch` でリモートブランチ削除 → GitHub が次 PR の base
   を main へ auto-retarget。`git push origin main` は ruleset で
   不可、必ず `gh pr merge` 経由（API がルールを尊重）
2. **Cascade（手動、`gh stack sync` の代替）**: 直近の新 cursor のみ
   ...
   - `FIRST=$(gh pr view <B-pr> --json commits --jq '.commits[0].oid')`
     （= PR 固有の最初のコミット。GitHub から導出 = 台帳不要を維持）
   - `git checkout B && git rebase --onto origin/main "$FIRST^" B`
     （B 固有コミットのみ新 main へ。squash で親コミットの hash が
     変わっても親の済みコミットは drop される）
