...
- [ ] **Step 1: Write the canonical-resolver RED test.**

Use a named resolver over `1 0 obj\n<< /L1 << /L2 << /Value 7 >> >> >>\nendobj\n`, resolve object `1 0` through `get_object_handle`, and assert the canonical root description is `input.pdf, object 1 0 at offset 10`. Assert the L1 dictionary, L2 dictionary, and scalar descriptions use the same template with offsets `17`, `24`, and `32`; assert both nested dictionaries and the scalar still emit through the owning resolver.

- [ ] **Step 2: Write the explicit-parse RED test.**

Parse `<< /Value 7 >>` through `ObjectHandle::parse`, assert the direct child description is qpdf's contextless template `parsed object,  at offset 10`, and assert `object_warning` still returns the bare contextless error rather than routing through a document sink.

- [ ] **Step 3: Run only the new tests and verify the failure is the missing description.**
...
- [ ] **Step 1: Add the optional template seam with contextless defaults.**
...
- [ ] **Step 2: Snapshot the template once per parser invocation.**
...
- [ ] **Step 3: Stamp every non-null direct parser value.**
...
- [ ] **Step 4: Build the canonical resolver template from qpdf's input/object description.**

Add a `ResolverHandle` helper that snapshots `ResolverCore::description` and returns:

```rust
if input_description.is_empty() {
    format!("object {} {} at offset $PO", object_ref.number, object_ref.generation)
} else {
    format!(
        "{}, object {} {} at offset $PO",
...
- [ ] **Step 5: Preserve the top-level parser description during canonical resolution.**
...
- [ ] **Step 6: Run the RED tests GREEN, then refactor only if all remain green.**
...
- [ ] **Step 1: Run focused parser and object-description regressions.**
...
- [ ] **Step 2: Run formatting and workspace quality gates.**
