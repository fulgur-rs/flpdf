...
**Files:**
- Modify: `crates/flpdf/src/writer.rs` in the private writer test module replacing the old selector tests.

- [x] **Step 1: Add the empty-source default test**
...
- [x] **Step 1: Add full-rewrite empty-ID tests**
...
- [x] **Step 2: Add linearized empty-ID tests**
...
- [x] **Step 3: Add encrypted output ID-consistency coverage**
...
- [x] **Step 1: Extract the one supported source-ID predicate**
...
- [x] **Step 2: Implement one complete ordinary/static ID generator**
...
- [x] **Step 3: Make the generator pass the RED unit tests**
...
- [x] **Step 1: Replace incremental ordinary/static mutations**
...
- [x] **Step 2: Compute the plain-pipeline ordinary/static array once**

In `PlainWritePlan::build`, before calling `apply_encrypt_trailer_entries`, create the same ordinary/static generated array for the eligible non-deterministic, non-copy route and pass it to the trailer helper. This ensures the new plain full-rewrite pipeline does not retain its own old selector.
...
- [x] **Step 5: Reuse the complete array at every full-rewrite trailer site**
...
- [x] **Step 6: Run full-rewrite RED tests until GREEN**

Run:

```bash
cargo test -p flpdf writer::tests --lib
```

The old unit tests were rewritten to assert the new qpdf behavior rather than preserve deleted helper names. The focused writer set passed 210/210.
...
- [x] **Step 1: Replace ordinary/static linearization branches**
...
- [x] **Step 2: Verify linearized encryption consumes the emitted first element**
...
- [x] **Step 3: Remove only unreachable private selector routes**
...
- [x] **Step 4: Run linearization focused tests**
...
Expected result: default/static empty-ID tests, encrypted empty-ID tests, existing deterministic tests, and existing linearization tests pass together. The focused linearization set passed 96/96.
...
- [x] **Step 1: Run the affected crate tests**
...
- [x] **Step 2: Run qpdf live checks on the empty-ID fixtures**
...
- [x] **Step 4: Check the cutover and diff**
