...
- [x] **Step 1: Write the failing parser tests.** Replace the old scalar-only assertions with tests that name and assert each qpdf behavior:
...
- [x] **Step 2: Run the parser test to verify the expected RED result.**
...
- [x] **Step 3: Implement the minimal qpdf parser.** Add the associated public job entry point and a private byte-oriented helper. The helper must:
...
- [x] **Step 4: Run the parser test to verify GREEN and preserve the existing lifecycle suite.**
...
- [x] **Step 1: Change the existing empty-selection test into a qpdf-shaped failing expectation.** Rename it to `empty_selection_rebuilds_an_empty_page_tree`, assert `new_kids.is_empty()`, `removed_pages` contains the original leaves, and resolve the rebuilt `/Pages` root to assert `/Count` is zero and `/Kids` is an empty array.

- [x] **Step 2: Run the test to verify the expected RED result.**
...
- [x] **Step 3: Remove only the obsolete empty-selection rejection and update the public error documentation.** Leave the existing root/page-tree preparation and root rewrite path intact so the empty vector follows exactly the same qpdf-shaped rebuild lifecycle as non-empty selections.

- [x] **Step 4: Run the empty-selection test and all tree-rebuild tests.**
...
- [x] **Step 5: Commit the page-tree primitive.**
...
- [x] **Step 1: Write failing page-job tests.** Add tests for `select_single_source_pages` and `handle_page_specs` covering:
...
- [x] **Step 2: Run the new page-job tests to verify RED.**
...
- [x] **Step 3: Implement qpdf's vector logic.** Remove scalar zero guards, call the shared cardinality helper after plans are built, use `values[spec_index]` for each round, and run the round-robin only when there is more than one specification. Remove the merged helper's unconditional empty-selection error. Keep source grouping, label capture, AcroForm replay, warning attribution, and `PageSpecJobOutput` unchanged.

- [x] **Step 4: Update existing scalar tests and lifecycle callers to pass borrowed slices.** Use `Some(&[1])` for the previous scalar behavior and `configuration.collate.as_deref()` in the JSON job path. Ordinary `None` calls remain unchanged.

- [x] **Step 5: Run page-job tests to verify GREEN.**
...
- [x] **Step 6: Commit the canonical page-job vector route.**
...
- [x] **Step 1: Add failing qpdf differential CLI tests.** In `page_ops_qpdf_matrix.rs`, add tests that run the same command through qpdf and flpdf and compare page order using the existing `media_boxes_of` helper:
...
- [x] **Step 2: Run the new integration tests to verify RED.**
...
- [x] **Step 5: Commit the CLI/JSON integration.**
...
- [x] **Step 1: Verify callers before deletion.**
...
- [x] **Step 2: Delete the unreachable bridge and update exports/docs.** Preserve `CombinedPlan::build`/`from_specs` where their distinct generic responsibilities remain used, and do not add a replacement free collate function.

- [x] **Step 3: Run the route-lock and documentation/deviation checks.**
...
- Modify: `/home/ubuntu/flpdf-qtest/parity/qtest-11.9.0.jsonl` only for rows promoted by the same full run; no qtest source/fixture changes.
- Use the existing `/home/ubuntu/flpdf-qtest` harness and release binary.

- [x] **Step 1: Run the local quality gates from the feature worktree.**
...
- [x] **Step 2: Run qpdf compatibility and same-run qtest evidence.** Build the release binary, run the literal collate target in `/home/ubuntu/flpdf-qtest`, preserve `harness.log` and `qtest-results.xml`, and report target testcase outcomes separately from any pre-existing global manifest failures. The acceptance record must contain zero allowlist regressions and zero manifest validation errors for the same run.

- [x] **Step 3: Run fresh changed-line coverage and inspect the diff.** Use the repository's full workspace coverage command with `--features qpdf-zlib-compat` and `scripts/patch-coverage.sh --base origin/main --lcov`; inspect `git diff --check`, `git status --short`, and all untracked files before committing or publishing.

- [x] **Step 4: Rebase only if the parent PR moved, then push the feature branch.** Confirm PR #1386's live head/base first. If its base remains `main` and the feature branch is based on the current remote hxmj head, push with `git push -u origin flpdf-egzr-8-11-collate`. Do not touch the dirty/local hxmj worktree.

- [x] **Step 5: Create or update the Draft PR and read it back.** Parent PR #1386 merged while this slice was running, so publish `gh pr create --draft --base main --head flpdf-egzr-8-11-collate`; verify the exact base/head, body, checks, and clean merge state. Do not claim Ready until required CI and review gates are green.

- [x] **Step 6: Append implementation evidence, close the completed Beads issue, and push Dolt.** Read the issue before mutation; append the source citations, commit/PR, focused/full tests, qtest target results, patch coverage, and any known unrelated global failures. Then run `bd close flpdf-egzr.8.11 --reason="qpdf comma-separated collate implemented and verified"`, read it back, run `bd dep cycles`, and finish with `bd dolt push` reporting `Push complete.`.
