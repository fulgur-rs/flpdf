...
- ObjStm members have generation zero and are not streams, xref objects,
  encryption dictionaries, or other forbidden structural objects;
...
Before implementation begins, `flpdf-9hc.20.29` and `.30` must be reconciled
against current committed tests. They may be linked to this stack only for
acceptance criteria that remain unmet. If their criteria are already
satisfied, they are closed with the exact committed tests as evidence instead
