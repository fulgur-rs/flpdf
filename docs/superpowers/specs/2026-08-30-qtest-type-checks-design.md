...
Port the exercised array/dictionary iterator behavior at the ObjectHandle boundary rather than replacing it with a test-only vector walk. qpdf's iterator stores an internal `ivalue` and its C++ `operator*` returns a reference to that member; a copied `QPDFObjectHandle` shares the selected child identity but does not follow later assignments when the iterator moves. `current()` returns `ObjectHandle` by value, so flpdf follows the copied-handle behavior and callers read a fresh value for later positions. Dictionary cursors snapshot qpdf's visible `getKeys()` set, excluding null-valued entries. At a non-end key position, qpdf's `getKey` supplies an initialized null for a removed key and, with a warning context, for a non-dictionary receiver; without context its type warning throws. Only the key-set end position uses the default uninitialized iterator value. Add an ObjectHandle initialized-state query and preserve the existing unresolved/reserved/destroyed states as distinct from uninitialized.
