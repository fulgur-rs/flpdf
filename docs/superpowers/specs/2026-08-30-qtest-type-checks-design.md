...
Port the exercised array/dictionary iterator behavior at the ObjectHandle boundary rather than replacing it with a test-only vector walk. The Rust cursors use a stable, safe `ivalue` cell that is rebound as the cursor moves, so handles retained from `current()` observe the same end/next-item transitions as qpdf's iterator reference. Dictionary cursors snapshot qpdf's visible `getKeys()` set, excluding null-valued entries. Iterator end values are represented as an explicit uninitialized iterator value, not as a null object. Add an ObjectHandle initialized-state query and preserve the existing unresolved/reserved/destroyed states as distinct from uninitialized.
