...
| `crates/flpdf/src/object_handle.rs` | correspondence | \`QPDFObjectHandle\`, \`QPDFObject\`, and \`QPDFValue\` identity and payload ownership, \`QPDFObjectHandle::copyStream\`/\`QPDF::copyStreamData\` stream-copy primitives, \`QPDF::setImmediateCopyFrom\`, plus \`QPDFWriter.cc\` \`unparseObject\`/\`writeTrailer\` writer-emission primitives (\`unparse_object\`/\`unparse_object_qdf\`/\`unparse_stream_body\`/\`unparse_stream_body_qdf\`/\`unparse_trailer\`) |
